...
1. 작성자로 로그인한 후 왼쪽 창에서 &#39;콘텐츠 라이브러리&#39;를 클릭하고 &#39;추가&#39;를 클릭합니다.
   ...
   또는, 시작 페이지에서 &#39;콘텐츠 생성&#39;을 클릭할 수 있습니다.

1. 이름 필드에서 업로드하려는 콘텐츠의 이름을 지정합니다.
1. 설명 필드에 콘텐츠 설명을 입력합니다. 의미 있는 설명을 입력해야 합니다. 글자수 제한은 400자입니다.
...
1. 지속 시간 필드에서 학습자는 이 모듈에서 보낼 예상 시간을 선택적으로 입력할 수 있습니다. 지속 시간은 분 단위 입니다.

   학습자의 학습 소요 시간은 학습자가 강의를 완료로 표시한 경우 지정된 지속 시간에 따라 계산됩니다. 학습자가 플레이어에서 콘텐츠를 사용한 경우 플레이어에서 소요한 시간이 학습 소요 시간에 추가됩니다. 실제 콘텐츠 시간이 지정된 지속 기간보다 짧은 경우 플레이어는 항상 콘텐츠 표시 시간을 인식하므로 아무 동작도 수행되지 않습니다.

1. 콘텐츠 태그 필드에 콘텐츠를 쉽게 검색할 수 있도록 업로드된 콘텐츠의 태그를 입력하십시오.
...
WebVTT는 다음에 대해 지원되지 않습니다.
   ...
   1. 비디오 공지.
   1. eLearning 콘텐츠 내에서 재생되는 비디오. 이는 콘텐츠에서 구동합니다.
   1. 소셜 학습에 업로드된 비디오.
...
1. 지속 시간 필드에서 학습자는 이 모듈에서 보낼 예상 시간을 선택적으로 입력할 수 있습니다. 지속 시간은 분 단위 입니다.
1. 콘텐츠 태그 필드에 콘텐츠를 쉽게 검색할 수 있도록 업로드된 콘텐츠에 대한 태그를 입력하십시오.
