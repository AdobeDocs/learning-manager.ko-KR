 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    <p><b>중요: 학습자 지원 및 SCORM 기반 평가</b></p>
    <p>계정에 학습자 도우미가 활성화되어 있고 SCORM/xAPI 패키지를 학습자 도우미 콘텐츠 소스로 구성된 카탈로그에 업로드하는 경우 다음 사항에 주의하십시오.</p>
    <ul>
      <li>학습자는 학습자 도우미를 사용하여 해당 패키지의 콘텐츠에 대해 질문할 수 있습니다.</li>
      <li>보조자는 SCORM 과정에 포함된 퀴즈 질문에 대한 답변을 효과적으로 표시하는 설명 또는 요약을 생성할 수 있습니다(예: 피드백을 다시 작성하거나 "정답" 옵션을 설명함).</li>
   </ul>
   <p>높은 이해관계 또는 감독된 평가(예: 규정 준수 또는 인증 시험)의 경우, Adobe은 다음을 권장합니다.</p>
   <ul>
      <li>학습자 도우미 소스로 구성되지 않은 카탈로그에서 평가가 많은 SCORM 콘텐츠를 유지 또는</li>
      <li>단순히 학습 자료를 재해석해서 정답이 드러나지 않도록 평가를 설계한다.</li>
   </ul>
   <p>예상된 동작입니다. Adobe Learning Manager에서는 학습자 도우미가 명시적으로 AI 소스로 선택한 카탈로그에 있는 콘텐츠를 설명할 수 있습니다.</p>
    </td>
   <td>
    <p>zip</p></td>
  </tr>
...
## 강의 콘텐츠 미리 보기 {#previewcontentlibrary}

강의 콘텐츠가 콘텐츠 라이브러리에 추가되면 이를 미리 볼 수 있습니다. 미리 보려면 각 강의 행 끝에 있는 눈 아이콘을 선택합니다.
...
1. **작성자**(으)로 로그인한 후 왼쪽 창에서 **[!UICONTROL 콘텐츠 라이브러리]**&#x200B;를 선택하고 **[!UICONTROL 추가]**&#x200B;를 선택합니다.
...
1. Adobe Learning Manager은 Adobe Learning Manager에 업로드된 비디오 콘텐츠에 대한 캡션을 지원합니다. 이제 작성자는 비디오 파일과 함께 캡션이 있는 파일을 업로드할 수 있습니다.
   ...
   그러면 자막이 Fluidic Player에 나타납니다. 또한 캡션은 [WCAG2.0 표준](https://www.w3.org/TR/WCAG20/)을 준수합니다.
   ...
   *webvtt 파일 추가*
   ...
   이 콘텐츠를 과정에 추가한 후에 학습자로 과정을 미리 보기하면 비디오에서 캡션을 볼 수 있습니다.
...
판매자 계정에서 과정이 있는 카탈로그를 공유하고 과정에 모듈, 오디오 또는 자막이 있는 비디오가 포함된 경우 과정이 구매자 계정에서 동일하게 작동되어야 합니다.
...
* 글로벌 교육: 다국어 학습자에게 자막이 제공되는 비디오 콘텐츠를 다국어로 제공합니다.
* 접근성 규정 준수: 청각 장애가 있는 사용자를 위해 자신이 선호하는 언어로 캡션을 제공합니다.
* 빠른 지역화: VTT 파일을 자동으로 생성하고 번역하여 수작업을 줄이고 콘텐츠 출시를 가속화합니다.
* 일관된 환경: 언어에 관계없이 모든 학습자가 동일한 정보를 받도록 합니다.

#### 주요 기능

* 자동 VTT 생성: 비디오 또는 오디오 파일을 업로드하고 원본 언어로 VTT 캡션을 자동 생성합니다.
* 다국어 번역: 캡션을 지원되는 10가지 영어 이외의 언어로 번역합니다.
* 인앱 검토 및 편집: 게시하기 전에 VTT 파일을 검토, 편집 및 다운로드합니다.
* 알림: VTT 생성 및 번역이 완료되면 인앱 알림을 받습니다.
* 원활한 게시: Publish은 학습자가 선택한 언어로 액세스할 수 있도록 캡션을 완성했습니다.

#### 콘텐츠 업로드 및 VTT 생성

1. 콘텐츠 라이브러리로 이동하여 [!UICONTROL 콘텐츠 추가]를 선택합니다.
2. MP3 또는 MP4 파일을 업로드합니다.
3. 업로드 대화 상자에서 **번역 생성** 옵션을 선택합니다.
4. 원래 내용 언어를 선택합니다(기본값은 파일의 언어).
5. 번역을 위한 추가 대상 언어를 선택합니다(최대 10개 지원).
6. [!UICONTROL 저장]을 선택합니다. 시스템이 VTT 파일을 생성하고 번역하기 시작합니다.
...
* **지원되는 언어:** Adobe Learning Manager에서 지원하는 비영어 언어(프랑스어, 독일어, 스페인어, 이탈리아어, 포르투갈어, 네덜란드어, 스웨덴어, 인도네시아어, 노르웨이어 보크몰 및 덴마크어)는 10가지입니다.
...
예를 들어 기관에서 콘텐츠를 완전히 제어하기를 원하고 콘텐츠를 못 본 일부 사용자는 모든 콘텐츠에 대한 액세스 권한을 가져야 합니다. 동시에 에이전시의 콘텐츠 작성자는 자신의 콘텐츠에만 액세스할 수 있어야 하며, 경우에 따라 다른 사람의 콘텐츠에도 액세스할 수 있어야 합니다.

기존 콘텐츠(즉, 콘텐츠 폴더를 구성하기 전에 업로드된 콘텐츠)가 있는 콘텐츠 라이브러리는 **공개 폴더**&#x200B;로 정의됩니다. 이 폴더는 중단하거나 삭제할 수 없습니다. 공개 폴더의 일부인 콘텐츠에는 모든 유형의 작성자가 액세스할 수 있습니다. 콘텐츠 폴더가 구성되면 표준 작성자 및 사용자 정의 작성자는 새 콘텐츠를 업로드하는 동안 콘텐츠를 배치해야 할 폴더를 선택해야 합니다.
...
**공개**&#x200B;를 선택하면 콘텐츠가 모든 작성자에게 표시될 수 있습니다. 폴더에 속하지 않은 계정의 모든 콘텐츠는 기본적으로 공개 폴더에 있게 됩니다.

콘텐츠 폴더는 콘텐츠를 연결할 단순한 가상 구획입니다. 하나의 콘텐츠가 두 폴더에 있는 경우 콘텐츠 파일은 한 개지만 여러 폴더에 연결되어 있다는 의미입니다. 또한 사용자 정의-폴더-1에 대한 액세스 권한이 있는 사용자 정의-작성자-1이 콘텐츠를 업데이트하는 경우 업데이트된 동일한 콘텐츠가 사용자 정의-작성자-2가 액세스한 사용자 정의-폴더-2에도 반영됩니다.
...
새 콘텐츠를 업로드할 때 업로드 프로세스 중에 저장할 폴더를 선택합니다.

1. 작성자로 로그인하고 **관리** > **콘텐츠 라이브러리**&#x200B;로 이동하거나 홈 페이지에서 **콘텐츠 만들기**&#x200B;를 선택합니다.

2. 페이지의 오른쪽 상단에서 **추가** > **콘텐츠**&#x200B;를 선택합니다.

3. 콘텐츠의 이름 및 설명을 입력합니다.
...
7. 필요에 따라 나머지 필드 **기간**, **태그**, **콘텐츠 고유 ID** 및 **만료 날짜**&#x200B;를 완료하십시오.
...
**콘텐츠 라이브러리**&#x200B;에서 각 콘텐츠 파일에는 전체 폴더 경로가 표시됩니다(예: **영업 교육** / **자가 진행식 모듈** / **PDF 에셋**). 이 경로는 계층 구조에서 파일이 있는 위치를 정확하게 알려줍니다.

* 파일이 두 개 이상의 폴더에 있는 경우 모든 경로가 쉼표로 구분되어 표시됩니다.

* 경로가 길면 처음부터 줄임표(...)로 잘리고 가장 깊은 폴더 이름이 항상 표시됩니다.

* 게시, 중단 및 초안 컨텐츠는 모두 컨텐츠 라이브러리에 표시되며 해당 폴더 경로를 표시합니다.

![](assets/cl1.png)

## 폴더별로 콘텐츠 필터링

콘텐츠 라이브러리의 **폴더** 필터를 사용하면 특정 폴더에 저장된 콘텐츠만 표시하도록 목록 범위를 좁힐 수 있습니다.

1. **콘텐츠 라이브러리**&#x200B;에서 열 머리글의 **폴더** 드롭다운을 선택합니다.
   ![](assets/cl5.png)
   드롭다운에 공개 폴더와 비공개 폴더 목록이 표시됩니다. 공용 폴더와 개인 폴더는 동시에 선택할 수 없습니다. 하나를 선택하면 다른 하나는 선택 해제됩니다.

2. 특정 개인 폴더로 필터링하려면 다음을 수행합니다.
   * **수준 1 폴더**&#x200B;를 선택하면 그 아래에 있는 모든 수준 2 및 수준 3 하위 폴더가 자동으로 선택됩니다. 이러한 하위 폴더는 선택된 것으로 나타나지만 개별적으로 선택 해제할 수 없습니다. 이 필터는 수준 1 폴더와 그 하위 폴더의 모든 콘텐츠를 반환합니다.
   * **수준 2 폴더**를 선택하면 그 아래에 있는 모든 수준 3 하위 폴더가 자동으로 선택됩니다.
     ![](assets/cl6.png)
...
콘텐츠를 이동하면 현재 폴더에서 삭제되어 대상 폴더에 저장됩니다. 대화 상자에서 여러 개인 폴더를 선택하여 콘텐츠를 이동할 수 있습니다. 그러나 비공개 폴더와 공개 폴더는 동시에 선택할 수 없습니다. 여러 개의 비공개 폴더 또는 단일 공개 폴더를 선택할 수 있습니다.

1. **콘텐츠 라이브러리**&#x200B;에서 이동할 각 콘텐츠 파일 옆의 확인란을 선택합니다.
2. **작업** > **콘텐츠 구성** > **콘텐츠를 폴더로 이동**&#x200B;을 선택합니다. **폴더로 콘텐츠 이동** 패널이 열립니다.
3. 콘텐츠를 이동할 대상 폴더를 선택합니다.
4. **콘텐츠 이동**&#x200B;을 선택합니다. 콘텐츠 라이브러리가 업데이트되어 새 폴더 경로가 표시됩니다.

>[!IMPORTANT]
>
>공용 폴더에서 개인 폴더로 또는 그 반대로 콘텐츠를 이동할 수 있습니다. 그러나 동일한 작업에서 콘텐츠를 공개 폴더와 비공개 폴더 모두로 이동할 수는 없습니다. 공개 폴더를 선택하면 비공개 폴더를 선택할 수 없으며, 그 반대의 경우도 마찬가지입니다.
...
복사하면 현재 위치에서 해당 콘텐트를 제거하지 않고 추가 폴더에 있는 해당 콘텐트에 대한 링크가 만들어집니다. 컨텐트 파일 자체는 복제되지 않습니다.

1. **콘텐츠 라이브러리**&#x200B;에서 복사할 각 콘텐츠 파일 옆의 확인란을 선택합니다.
2. **작업** > **콘텐츠 구성** > **폴더에 콘텐츠 복사**&#x200B;를 선택합니다. **폴더로 콘텐츠 복사** 패널이 열리고 폴더 계층 구조가 표시됩니다.
3. 콘텐츠를 표시할 대상 폴더를 선택합니다.
4. **콘텐츠 복사**&#x200B;를 선택합니다. 복사된 콘텐츠의 새 폴더 경로를 표시하도록 콘텐츠 라이브러리가 업데이트됩니다.

>[!NOTE]
>
>비공개 폴더에서 공개 폴더로 또는 공개 폴더에서 비공개 폴더로 콘텐츠를 복사할 수 없습니다.
...
링크 해제는 선택한 폴더에서 콘텐츠를 제거한다는 의미입니다.

다른 폴더와도 연결된 경우 지정한 폴더 **ONLY**&#x200B;에서 콘텐츠를 연결 해제할 수 있습니다. 링크 해제 중인 콘텐츠가 하나의 폴더에만 연결되었다면 대신 이동 작업을 사용하는 것이 좋습니다.
...
강의에 자가 진행식 모듈을 추가하는 경우 폴더 계층 구조를 검색하여 이름으로 검색하는 대신 적절한 콘텐츠 파일을 찾을 수 있습니다.

1. 작성자로 로그인하고 왼쪽 탐색에서 **과정**&#x200B;으로 이동합니다.

2. **추가**&#x200B;를 선택하여 새 강의를 만들거나 편집 모드에서 기존 강의를 엽니다.

3. **모듈** 섹션으로 이동하고 **콘텐츠** > **모듈 추가**&#x200B;를 선택합니다.

4. **모듈 유형 선택** 대화 상자에서 모듈 유형으로 **자가 진행식**&#x200B;을 선택합니다. **자가 진행식 모듈** 대화 상자가 열립니다.

5. **폴더 찾아보기**&#x200B;를 선택하여 폴더 계층 구조를 엽니다.

6. 폴더 레벨을 탐색하여 추가할 모듈을 찾습니다. 콘텐츠 파일을 선택합니다.

7. **추가**&#x200B;를 선택합니다. 모듈이 강의에 추가됩니다.

8. 나머지 강의 필드를 완료하고 **저장**&#x200B;을 선택하십시오.
...
Adobe Learning Manager에서 콘텐츠 라이브러리는 업로드된 콘텐츠의 버전 관리도 유지합니다. 콘텐츠(예: PowerPoint 프레젠테이션)에 변경 사항이 있는 경우 프레젠테이션을 라이브러리에 다시 업로드하면 버전 숫자가 1만큼 증가합니다. 이를 통해 콘텐츠의 변경 사항을 추적할 수 있습니다. 새로운 버전의 콘텐츠를 모든 학습 객체에 동시에 적용하거나 각 강의마다 개별 업데이트를 적용할 수 있습니다.
+++
