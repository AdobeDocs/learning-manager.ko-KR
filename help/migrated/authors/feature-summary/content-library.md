...
이전 섹션에서 언급된 대로 대화형 및 정적 콘텐츠 모두를 라이브러리에 업로드할 수 있습니다.

## 정적 콘텐츠 추가 {#addstaticcontent}

1. 선택 **[!UICONTROL 콘텐츠 라이브러리]** (으)로 로그인한 후 왼쪽 창에서 **작성자** 을 클릭하고 **[!UICONTROL 추가]** > **[!UICONTROL 내용]**.

>[!NOTE]
>
>선택 **[!UICONTROL 추가]** 이(가) 두 가지 옵션을 표시합니다. **[!UICONTROL 내용]** 및 **[!UICONTROL 퀴즈]**: 참조 [퀴즈 만들기](content-library.md#createaquiz) 를 참조하십시오.
...
1. **Learning Manager** learning Manager에 업로드된 비디오 콘텐츠에 대한 캡션을 지원합니다. 이제 작성자는 비디오 파일과 함께 캡션이 있는 파일을 업로드할 수 있습니다.
   ...
   라이브러리에 비디오 콘텐츠를 추가할 때 유효한 파일이어야 하는 VTT 파일을 추가할 수도 있습니다.
   ...
   *webvtt 파일 추가*
   ...
   플레이어에서 Fluidic 플레이어의 CC 버튼을 전환하여 캡션을 표시하거나 숨깁니다.
...
콘텐츠를 업로드하면 다음 옵션을 선택하여 알림을 볼 수 있습니다. **벨** 아이콘을 클릭합니다(페이지 오른쪽 상단 모서리). 콘텐츠를 수정 및 다시 업로드할 때마다 알림이 전송됩니다. 변경 사항이 있는 경우 귀하에게만 알림이 전송되며 다른 작성자에게는 전송되지 않습니다.

## 퀴즈 만들기 {#createaquiz}

새로운 퀴즈 생성 도구를 사용하여 Adobe Learning Manager 내에서 평가를 생성합니다. **[!UICONTROL 콘텐츠 라이브러리]** 페이지. 생성된 평가는 다음 항목의 일부가 됩니다. **콘텐츠 라이브러리** 강의 재사용 가능성을 위해 &#39;공용&#39; 폴더에 추가할 수 있습니다.

1. 왼쪽 패널에서 **[!UICONTROL 콘텐츠 라이브러리]**&#x200B;를 선택합니다.
1. 화면의 오른쪽 상단 모서리에서 **추가 > 퀴즈**.
1. (으)로 **[!UICONTROL 퀴즈 만들기]** 페이지에서 퀴즈의 이름과 설명을 입력합니다.
1. (으)로 **[!UICONTROL 퀴즈 콘텐츠]** 섹션, **퀴즈 질문 추가**.
1. (으)로 **[!UICONTROL 퀴즈 질문]** 대화 상자에서 질문 유형을 선택합니다. 질문의 유형에는 세 가지가 있습니다.
   * 객관식 질문
   * 참 또는 거짓
   * 공백 채우기
...
책임자가 콘텐츠 폴더를 만든 후에는 작성자가 콘텐츠를 콘텐츠 폴더에 업로드하여 해당 콘텐츠를 본인이나 계정에서 선택한 작성자 그룹만 볼 수 있도록 할 수 있습니다. 콘텐츠를 공개로 설정하고 계정의 모든 작성자에게 표시되도록 설정할 수도 있습니다.
...
>[!NOTE]
>
>공용 폴더와 개인 폴더는 함께 사용할 수 없습니다. 이것은 콘텐츠가 **할 수 없음** 동시에 공용 폴더와 전용 폴더와 연결되어야 합니다. 공용 폴더에 연결할 수도 있고 **또는** 언제든지 하나 이상의 비공개 폴더와 연결할 수 있습니다.
...
1. 콘텐츠를 이동할 폴더를 선택합니다. 선택 **[!UICONTROL 이동]**.
...
1. 다른 언어에 대한 콘텐츠를 추가하려면 **새 언어 추가** 을 누르고 필요한 언어를 선택합니다. 이 접근법을 사용하면 콘텐츠에 대한 다중 언어 지원을 추가할 수 있습니다.
