...
1. 변경 사항을 저장하려면 **[!UICONTROL 저장]**&#x200B;을 클릭합니다. 강의를 게시하려면 **[!UICONTROL 게시]**&#x200B;를 클릭합니다.

## 강의 생성 - 고급 워크플로우 {#createacourseadvancedworkflow}

1. 작성자로 Adobe Learning Manager에 로그인합니다. 작성자에게만 강의를 생성할 권한이 있습니다. 이제 홈 페이지에서 **[!UICONTROL 강의 만들기]**&#x200B;를 선택합니다. 강의 카탈로그 페이지가 열리고 강의 목록이 표시됩니다.
2. 오른쪽 상단에서 **추가**&#x200B;를 선택합니다. 새 강의를 생성할 수 있는 페이지가 열립니다.
3. 강의 이름과 간단한 설명을 입력합니다. 이 설명은 강의 카드에 표시됩니다. 해당 설명은 140자를 넘지 않아야 합니다. 그런 다음 강의에 대한 자세한 개요(강의 세부 사항 페이지에 표시)를 입력합니다. 해당 설명은 1500자를 초과하지 않아야 합니다.
4. 강의에 다른 언어를 제공하려면 창 왼쪽 상단 모서리의 &#39;새 언어 추가&#39;를 클릭합니다. 강의에 적용할 하나 이상의 언어를 선택합니다. **[!UICONTROL &#39;저장&#39;]**&#x200B;을 클릭합니다. 자세한 내용은 [다른 언어용 콘텐츠 추가](/help/migrated/authors/feature-summary/content-library.md)를 참조하십시오.
5. **강의 설정 수정**-

   a. 강의 설정 페이지에서 강의를 위한 스킬을 선택합니다. 스킬 드롭다운 목록에서 필요한 스킬을 선택합니다. 그런 다음 레벨 드롭다운 목록에서 필요한 레벨을 선택합니다.
   b. 강의 스킬, 레벨을 선택한 다음 스킬에 대한 점수를 설정합니다. 필요한 경우 스킬을 더 추가합니다.
   c. 필요한 경우, 사용자 정의 준수 레이블을 강의에 추가합니다. [강의/학습 경로/인증에 준수 레이블 추가](/help/migrated/authors/feature-summary/courses.md#add-compliance-labels-to-courselearning-pathcertification)를 참조하십시오.
   d. **등록 유형** 드롭다운 목록에서 등록 유형을 선택합니다.
...
6. 강의의 가격을 설정할 것인지 무료로 제공할 것인지 선택합니다. 강의를 유료로 제공하려면 **[!UICONTROL 유료]** 옵션을 선택하고 가격을 지정합니다. 그러면 강의 카드와 학습자용 강의 개요 페이지에 가격이 표시됩니다.

   >[!NOTE]
   >
   >이 옵션은 Adobe Commerce 커넥터가 구성된 경우에만 사용할 수 있습니다.

7. 학습자가 등록한 강의를 직접 취소할 수 있게 하려면 **학습자가 직접 등록을 취소할 수 있음** 확인란을 선택합니다.
...
8. **인스턴스 구성**
   ...
   * 활동
   ...
   >[!NOTE]
   >
   >강의를 복제하는 동안 원 강의에서 인스턴스 구성 옵션을 활성화한 경우에는 대상 강의에서 옵션이 비활성화된 상태로 유지됩니다.
...
9. **여러 등록**
   ...
   이를 통해 하나 이상의 기간에 둘 이상의 강의 인스턴스에 학습자를 등록할 수 있습니다.

   학습자의 다양한 강의 등록 간에 전환하려면 **다중 등록** 토글을 활성화합니다. 인스턴스 전환을 활성화한 경우 다중 등록을 사용할 수 없습니다.

10. 강의를 수강하기 전에 완료해야 하는 전제 조건 강의를 선택합니다. 강의 필드를 클릭한 다음 강의 목록에서 선택합니다.
11. 강의 전제 조건을 필수로 설정하려면 **사전 전제 조건** **활성화** 확인란을 선택합니다.
12. 강의와 관련된 키워드를 태그로 추가합니다. 학습자는 태그를 통해 검색 중 귀하의 강의를 쉽게 찾을 수 있습니다. 이 모든 태그는 추가한 모듈에 따라 자동으로 추가됩니다. 이 강의에 추가하려는 다른 태그가 있으면 계속 입력할 수 있습니다.
13. 강의와 관련된 키워드를 태그로 추가합니다. 학습자는 태그를 통해 검색 중 귀하의 강의를 쉽게 찾을 수 있습니다. 이 모든 태그는 추가한 모듈에 따라 자동으로 추가됩니다. 이 강의에 추가하려는 다른 태그가 있으면 계속 입력할 수 있습니다.
14. 자동 사용 중지 필드에서 강의를 중단할 날짜를 선택합니다. 책임자는 먼저 자동 중단 옵션을 활성화해야 합니다.
15. 변경 내용을 저장하려면 **[!UICONTROL 저장]**&#x200B;을 선택하세요. 강의를 게시하려면 **[!UICONTROL 게시]**&#x200B;를 클릭합니다.

## 강의에 대한 성적 증명서 구성 {#configuregradebookforacourse}
...
* **Gradebook** 토글을 선택하여 활성화합니다. 그 아래에 두 가지 옵션이 나타납니다. 기본적으로 둘 다 켜져 있습니다.
  ...
  * **최종 등급에 기여하지 않는 모듈 포함:** 점수를 매길 수 없는 모듈(PDF, 비디오, 오디오 등)이 성적 증명서에 표시됩니다. 점수를 매길 수 없는 모듈은 학습자의 최종 점수에 기여하지 않습니다.
...
1. **모듈 추가**&#x200B;를 선택합니다.
2. 모듈 선택기에서 추가할 모듈을 선택하고 **추가**&#x200B;를 선택합니다. 모듈은 **콘텐츠** 섹션에 표시됩니다. 점수를 매길 수 있는 모듈, SCORM, Captivate 콘텐츠, AICC, xAPI, 기본 퀴즈, 활동 모듈, 강의실 세션 및 가상 강의실 세션은 **가중치** 입력 필드를 표시합니다. 점수를 매길 수 없는 모듈은 가중치 열에서 대시를 표시합니다.
3. 점수를 매길 수 있는 각 모듈에 대해 **가중치** 필드에 백분율 값을 입력합니다. **총 가중치** 표시기는 입력할 때 업데이트되며, 저장하기 전에 정확히 **100%**&#x200B;에 도달해야 합니다.

   ![](assets/image_0004.png)

4. 여러 배달 형식이 있는 모듈의 경우: 모듈 지원 점수 매기기 중 **모두** 배달 형식이 있는 경우에만 가중치를 할당할 수 있습니다. 하나의 납품 유형이 점수부여를 지원하지 않는 경우 전체 모듈을 가중화할 수 없습니다.
...
## 인스턴스 만들기 {#createaninstance}

인스턴스를 만들려면 다음 작업을 수행하십시오.

1. **작성자**(으)로 로그인합니다.
2. 인스턴스를 생성하려는 강의로 이동하여 강의를 엽니다.
3. 왼쪽 탐색 패널에서 **인스턴스**&#x200B;를 선택합니다. **인스턴스** 페이지가 열립니다. 모든 강의에는 기본적으로 인스턴스가 있습니다. 기본 인스턴스를 수정하거나 인스턴스를 추가할 수 있습니다. 이 강의 인스턴스는 삭제할 수 없습니다.
4. 인스턴스를 생성하려면 강의 정보의 오른쪽 상단 모서리에 있는 **[!UICONTROL 새 인스턴스 추가]**&#x200B;를 클릭합니다. 강의의 새 인스턴스가 표시됩니다.
5. 인스턴스의 속성을 입력합니다.

   * **[!UICONTROL 인스턴스 이름]** 필드에 강의와 연결할 인스턴스의 이름을 입력합니다. 인스턴스의 이름은 고유해야 합니다.
   * **시작 날짜:** 과정의 시작 날짜입니다. 두 개의 미리 알림 전자 메일은 선택된 두 개의 날짜에 이 시작 날짜 이전에 학습자에게 전송될 수 있습니다. 시작 날짜는 완료 날짜보다 이전 날짜여야 합니다. 시작 날짜를 선택하면 이 날짜는 인스턴스의 **알림 경고** 섹션에 있는 달력에서 파란색으로 강조 표시됩니다.
   ...
   * **[!UICONTROL 기타 옵션 표시]**&#x200B;를 선택하여 다른 기한 옵션을 표시합니다.
...
### 시작 날짜 이전에 전자 메일 미리 알림 설정

전체 인스턴스 정보를 저장한 후에만 전자 메일 미리 알림을 설정할 수 있습니다. **저장**&#x200B;을 선택하면 인스턴스가 자동으로 최소화되어 편집 모드로 전환됩니다.

1. **알림 경고** 상자를 선택합니다.
2. 달력에서 첫 번째 미리 알림에 대한 인스턴스의 시작 날짜 이전의 날짜를 선택합니다.
3. 오른쪽에서 **미리 알림 추가**&#x200B;를 선택합니다.
4. **전자 메일 템플릿:** 드롭다운 목록에서 템플릿을 선택합니다.
5. **되풀이:** 드롭다운 목록에서 되풀이의 유형을 선택합니다. 오른쪽에 있는 파란색 체크 표시를 선택하여 알림 날짜를 저장합니다. 각 알림 날짜는 별도로 저장되어야 합니다.
   ![](assets/start-date-reminder1.png)
6. 두 번째 미리 알림에 대한 강의 시작 날짜 이전의 다른 날짜를 선택하고 프로세스를 반복한 다음 오른쪽에 있는 파란색 체크 표시를 선택하여 이 미리 알림 날짜도 저장합니다.
   ![](assets/start-date-reminder2.png)

## 인스턴스의 속성 보기 {#viewpropertiesoftheinstance}

1. **모듈:** 강의 작성자가 생성한 모듈의 개수입니다.
2. **세션:** 과정의 가상 강의실 및 강의실 모듈 수
3. **도전 과제:** 학습자의 도전 과제
4. **알림 경고:** 강의 시작일 전에 두 개의 알림 전자 메일을 보내는 옵션

### 액션 {#actions}

* **강의 설정에 저장:** 인스턴스를 만든 강의에 이 인스턴스의 설정을 저장합니다.
* **기본 설정 적용:** **구성** > **인스턴스 기본값** 페이지에 있는 기본 설정을 적용합니다.
* **인스턴스 사용 중지:** 인스턴스를 사용 중지했습니다. 인스턴스를 더 이상 사용할 수 없습니다.
* **인스턴스 삭제:** 특정 인스턴스를 삭제합니다.


>[!NOTE]
>
>기본 인스턴스나 기본 인스턴스의 사본은 삭제할 수 없습니다. 이러한 경우 인스턴스 삭제 옵션은 작업 드롭다운 목록 아래에 나타나지 않습니다.

* **대기자 명단에 등록된 학습자에게 알리기:** 대기자 명단에 등록된 학습자에게 등록할 수 있는 다른 인스턴스가 있음을 알립니다.
* **등록된 학습자에게 전자 메일 보내기:** 중요한 공지사항에 대해 학습자에게 전자 메일을 보냅니다.
...
강사 및 강의실 예약의 충돌을 관리합니다. 강사를 강의에 할당하기 전 강사가 가능한 시간과 날짜를 알고 싶으시다면 스케줄링 도우미를 사용하십시오.

강의를 생성하는 동안 VC 또는 CR 강의에 스케줄링 도우미를 클릭합니다.
...
스킬 레벨이 아닌 스킬 이름의 일부 또는 전체 이름으로만 검색할 수 있습니다.

도우미에서 강사 이름과 위치 및 인원 제한을 입력합니다.

또한 강사 검색 상자의 오른쪽에 있는 필터 아이콘을 클릭하면 표시되는 스킬을 검색할 수도 있습니다. 아래 스크린샷에 버튼이 표시됩니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>
    <p>"<b>시도 간 모듈 잠금 0:0:1 형식: 일/시간/분</b>" 확인란을 활성화하여 시도 간 특정 시간 동안 모듈을 잠글 수 있습니다. 모듈이 잠기면 학습자는 지정된 잠금 기간 동안 모듈에 진입할 수 없습니다. </p>
...
1. **[!UICONTROL 모듈 추가]**&#x200B;를 클릭합니다. 모듈을 추가하는 네 가지 옵션이 표시됩니다. 첫 번째 옵션은 자가 진행식 모듈을 추가하는 것입니다. 이 모듈을 통해 Adobe Learning Manager에서 모듈 라이브러리를 만들고 추가할 수 있습니다. 두 번째 옵션은 가상 강의실을 설정하는 것입니다. 세 번째 옵션은 강의실 모듈을 설정하는 것이며 네 번째는 활동 모듈입니다.
   ...
   **참고:** 학습자는 Microsoft Teams를 사용할 수 있어야 합니다. 그러나 학습자는 Learning Manager에 게스트로 참여할 수 있습니다.
   ...
   책임자가 ALM 계정에 대해 라이브 허브 를 활성화한 경우 라이브 허브 과정을 만들 수도 있습니다. Live Hub를 사용하여 VC 세션을 만들려면 [Live Hub 세션 만들기](../../getting-started-with-live-hub/create-a-live-hub-session.md)를 참조하세요
   ...
   **강의실 모듈:** 이 모드에서는 학습자가 직접 수업에 출석하며 훈련된 강사의 지원을 받습니다. 제목, 설명을 입력하고 세션 기간을 설정하십시오. 강의실 위치와 강사를 지정하여 세션을 진행할 수도 있습니다. 변경 사항을 저장하려면 **[!UICONTROL 완료]**&#x200B;를 클릭합니다.
   ...
   >[!NOTE]
   >
   >여기에서 새 위치를 만들 수 있는지 여부는 책임자가 구성한 [작성자가 위치를 만들 수 있도록 허용] 설정에 따라 달라집니다. 비활성화되어 있으면 책임자가 이미 추가한 위치만 검색하고 선택할 수 있습니다. 자세한 내용은 [강의실 위치 추가](../../../migrated/administrators/feature-summary/classroom.md)를 참조하세요.
   ...
   **참고:** 학습자가 Microsoft Teams에 게스트로 참석하려는 경우, 전자 메일을 입력해야 합니다. 전자 메일이 Learning Manager에 있어야 합니다.
...
여러 언어로 체크리스트를 만들 수 있습니다.

### 여러 언어로 체크리스트 만들기 {#createachecklist}

작성자만 체크리스트를 만들 수 있습니다. 체크리스트는 활동 모듈의 유형입니다. 활성 모듈을 설정하는 동안 아래에 표시된 대로 작성자는 활동을 **체크리스트**&#x200B;로 선택할 수 있습니다.

![](assets/checklist-option.png)

*검사 목록 만들기*

1. **체크리스트** 옵션을 선택하면 몇 가지 추가 옵션이 표시됩니다.

2. **체크리스트 유형:** **예/아니요** 또는 **1-5** 옵션을 선택합니다. 예/아니요를 선택하는 경우 체크리스트에는 예 또는 아니요로만 대답할 수 있는 질문이 포함됩니다. 1-5에서 선택한 경우 Likert 체크리스트를 볼 수 있고 여기에서 5점 범위의 질문을 평가할 수 있습니다.

3. **구성**&#x200B;을 선택합니다. 체크리스트 창이 나타납니다.
4. 원하는 만큼 질문을 입력합니다. 동일한 질문을 추가 언어로 입력할 수도 있습니다.

   ![](assets/multi-lang-support-for-checklists2.png)
   *추가 언어를 사용할 수 있음*

   ![](assets/multi-lang-support-for-checklists3.png)
   *예를 들어 프랑스어로도 질문을 입력합니다*

   >[!NOTE]
   >
   >ALM은 이러한 추가 언어로 질문을 자동으로 번역하지 않습니다. 질문은 해당 언어로 번역해야 합니다.

5. **저장**&#x200B;을 선택합니다. 질문은 모든 언어로 저장됩니다.
6. 다른 모든 관련 옵션을 선택하고 **추가**&#x200B;를 선택합니다. 체크리스트는 해당 과정이 제공되는 모든 언어로 작성됩니다.
...
* 체크리스트에서 검토자 주석 활성화
* 학습자에게 댓글을 표시할지 여부 제어
* 검토자의 이름을 학습자에게 표시할지 여부 제어
* 검토자 주석이 학습자 성적 증명서 및 보고서에 캡처되는지 확인

#### 사용 사례

#### 성능 평가

실제 또는 역할 기반 평가 시 관찰된 강점, 격차 및 개선 영역에 대한 명확한 피드백을 제공합니다.

#### 강사 주도 및 VILT 교육

강사가 학습자가 체크리스트 기반 평가를 통과하거나 실패한 이유를 설명할 수 있도록 허용합니다.

#### 규정 준수 및 감사 시나리오

검사 목록 평가 결정을 설명하는 감사 가능한 추적을 유지합니다.

#### 다중 검토자 환경

학습자가 평가한 학습자를 식별하고 세부 사항을 설명할 수 있도록 후속 조치를 취합니다.

#### 체크리스트 주석 달기 활성화

1. 기존 강의를 열거나 새 강의를 생성합니다.
2. 체크리스트 모듈을 추가하거나 기존 체크리스트 모듈을 열어 편집합니다.
3. 체크리스트 구성 패널에서 다음을 수행합니다.
   1. **검토자 의견**&#x200B;을 선택합니다. 체크리스트를 검토하는 동안 검토자는 발언이나 피드백을 추가할 수 있습니다. 게시된 뒤에는 설정을 변경할 수 없습니다.
   2. (선택 사항) **학습자에게 검토자 이름 표시**&#x200B;를 선택합니다. 그러면 학습자는 체크리스트 모듈을 검토한 검토자의 이름을 확인할 수 있습니다.
4. 체크리스트 구성을 저장합니다.
5. Publish 과정을 통해 평가 시 체크리스트를 사용할 수 있습니다.

체크리스트는 이제 학습자 평가 중 검토자 의견을 지원합니다.

#### 평가 중 수행되는 작업

작성자는 평가를 수행하지 않지만 다음과 같이 구성이 사용되는 방식을 이해하는 것이 좋습니다.

* 검토자가 학습자를 평가하는 동안 **주석 필드**&#x200B;를 확인합니다
* 검토자는 제출 중에 상황에 맞는 주석을 추가할 수 있습니다
* 활성화된 경우:
  * 학습자가 리뷰어 주석을 확인합니다.
  * 학습자가 검토자의 이름을 확인합니다.
...
* 학습자가 완료된 체크리스트를 열면 다음과 같이 표시됩니다.
* 검사 목록 결과(합격/불합격 또는 점수)가 표시됩니다.
* 검토자 주석이 표시됩니다(활성화된 경우).
* 검토자의 이름이 표시됩니다(활성화된 경우).

이는 학습자가 평가 결과와 다음 단계를 명확히 이해하는데 도움을 준다.

#### 작성자를 위한 주요 참고 사항

* 검사 목록 주석은 **선택 사항**&#x200B;이며 명시적으로 사용하도록 설정해야 합니다.
* 주석 및 검토자 ID의 표시 여부는 **완전히 작성자 제어**&#x200B;입니다.
* 작업 대상:
  * 단일 또는 다중 검토자
  * 점수 기반, 예/아니요 및 비율 조정된 체크리스트
* 작성자가 업데이트하지 않으면 기존 체크리스트에 영향을 주지 않습니다.

### 다국어 검사 목록 만들기

다국어 지원 체크리스트를 통해 작성자는 **여러 언어로 제공되는 질문이 있는 단일 체크리스트 모듈**&#x200B;을 만들 수 있습니다.\
활성화되면 검토자가 **선택한 콘텐츠 언어**&#x200B;로 체크리스트 질문을 자동으로 볼 수 있으므로 전 세계 팀에서 일관되고 지역화된 평가를 수행할 수 있습니다.

이렇게 하면 **언어별로 별도의 검사 목록**&#x200B;을 만들고 관리할 필요가 없으므로 중복이 줄어들고 과정 유지 관리가 단순화됩니다.

다국어 검사 목록을 지원하여 작성자는 다음을 수행할 수 있습니다.

* 동일한 체크리스트 내에서 여러 언어로 체크리스트 질문 추가
* 체크리스트 질문 전체에 걸쳐 언어 패리티 유지
* 검토자가 콘텐츠 언어에 맞게 자동으로 현지화된 체크리스트 질문을 볼 수 있도록 합니다
* 선택한 언어로 체크리스트 보고서 다운로드

#### 사용 사례

#### 글로벌 인력 평가

모든 지역의 관리자와 강사가 현지화된 체크리스트 질문을 사용하여 학습자를 평가할 수 있습니다.

#### 지역 간 표준화된 평가

단일 평가 구조를 유지하면서 다른 지역에 대해 여러 언어를 지원합니다.

#### 규정 준수 및 운영 교육

평가자가 모국어로 된 체크리스트 기준을 명확하게 이해하도록 하여 해석 오류를 줄입니다.

#### 컨텐츠 복제 감소

다른 언어용으로 여러 체크리스트 모듈을 만들고 유지하지 마십시오.

#### 다국어 체크리스트 생성 방법

1. 기존 강의를 열거나 새 강의를 생성합니다.
2. 체크리스트 모듈을 추가합니다.
3. 강의 설정에서 필요한 **콘텐츠 언어**(예: 영어, 프랑스어, 독일어)를 추가합니다.
4. 강의 구성을 저장합니다.

#### 여러 언어로 체크리스트 질문을 구성하는 방법

1. 체크리스트 모듈 구성을 엽니다.
2. **질문 구성** 섹션으로 이동합니다.
3. 사용 가능한 언어 간에 전환하려면 **언어 탭**&#x200B;을 사용하십시오.
4. 각 언어에 대해:
   * 각 체크리스트 질문의 번역된 버전 추가
   * 언어 간에 동등한 의미와 평가 의도를 유지합니다.
5. 필요에 따라 체크리스트 기준(예/아니요, 비율 기준 또는 점수 기준)을 정의합니다.

각 언어 버전은 별도의 모듈이 아닌 동일한 체크리스트를 나타냅니다.

체크리스트 구성을 저장하고 강의를 게시합니다. 이제 체크리스트를 구성된 모든 언어로 사용할 수 있습니다.

#### 검토자의 경험

* 검토자는 **선택한 콘텐츠 언어**&#x200B;에서 체크리스트 질문을 봅니다.
* 평가 중에는 수동 언어 전환이 필요하지 않습니다.
* 리뷰어가 콘텐츠 언어를 변경하면 체크리스트 질문이 자동으로 조정됩니다

### 체크리스트 질문에 가중치 지정

체크리스트 질문 가중치는 작성자가 개별 체크리스트 질문에서 **다른 최대 점수**&#x200B;를 가질 수 있는 **점수 기반 체크리스트**&#x200B;를 만들 수 있도록 합니다.

작성자는 모든 체크리스트 항목을 동일하게 취급하는 대신 가중치 질문을 정의하고 학습자가 합격하는 데 필요한 **최소 총 점수**&#x200B;를 설정할 수 있습니다.

이 기능을 사용하면 특히 **성능 기반, 평가 기반 및 규정 준수 중심 교육**&#x200B;에 대한 보다 정확하고 유연한 평가를 수행할 수 있습니다.

체크리스트 질문 가중치로 작성자는 다음을 수행할 수 있습니다.

* **점수 기반 검사 목록 유형** 만들기
* 개별 검사 목록 질문에 다른 **최대 점수** 할당
* 체크리스트 통과에 필요한 **최소 총 점수**&#x200B;를 정의합니다.
* 확장된 점수부여 척도 사용(최대 10까지 구성 가능 범위)
* 보고서에서 달성된 점수 및 최대 점수 보기

#### 사용 사례

#### 강사 주도 평가

단순한 예/아니요 결과가 아닌 자세한 채점 기준을 사용하여 학습자를 평가한다.

#### SOP 및 운영 교육

중요한 단계에는 높은 가중치를, 작은 작업에는 낮은 가중치를 할당합니다.

#### 규정 준수 및 인증 준비 상태

학습자가 합격하기 전에 정의된 최소 역량 임계값을 충족하는지 확인합니다.

#### 기술 및 성능 벤치마킹

부분 완료가 여전히 전체 점수에 기여하는 구조화된 평가를 지원합니다.

#### 가중치 체크리스트 생성 방법

1. 기존 강의를 열거나 새 강의를 생성합니다.
2. 체크리스트 모듈을 추가하거나 기존 체크리스트 모듈을 열어 편집합니다.
3. 체크리스트 구성 패널에서 **사용자 지정 점수** 체크리스트 유형을 선택합니다.
4. 이렇게 하면 질문별 점수 및 총 점수 계산이 가능합니다.
5. 필요에 따라 체크리스트 질문을 추가합니다.
6. 각 질문에 대해:
   * **최대 점수** 정의(예: 5, 10, 20)
7. 질문 점수가 상대적 중요도를 반영하는지 확인합니다.

질문마다 동일한 체크리스트 내에서 최대 점수가 다를 수 있습니다.

#### 합격 기준 정의

1. 체크리스트 통과에 필요한 **최소 총 점수**&#x200B;를 지정하십시오. 이 시스템은 질문 가중치에 따라 총 가능한 점수를 자동으로 계산합니다.
2. 체크리스트 구성을 저장하고 강의를 게시합니다.

이제 체크리스트가 강사 평가에 대한 가중치 적용으로 활성화됩니다.

#### 검토자의 경험

* 검토자는 체크리스트 질문 각각의 **최대 점수**&#x200B;를 봅니다.
* 검토자는 평가 중에 질문당 점수를 할당합니다
* 시스템이 계산하는 내용은 다음과 같습니다.
  * 달성한 총 점수
  * 정의된 기준에 따라 합격 또는 불합격 상태

#### 학습자 경험

* 학습자는 체크리스트 완료 상태(합격/불합격)를 확인합니다
* 활성화된 경우 학습자에게 다음이 표시될 수도 있습니다.
  * 검토자 주석
  * 검토자 이름
* 채점 논리는 평가 간에 일관되게 적용됩니다

#### 작성자용 참고 사항

* **형식 평가**&#x200B;에는 가중치 체크리스트가 가장 적합합니다.
* 문제 점수는 왜곡된 결과를 피하기 위해 신중하게 교정되어야 한다
* 작업 대상:
  * 단일 또는 다중 검토자
  * 검토자 주석(활성화된 경우)
* 명시적으로 업데이트하지 않으면 기존 체크리스트는 영향을 받지 않습니다.
...
7. **[!UICONTROL 강사가 성공을 표시할 수 있도록 허용]** 옵션을 선택합니다.
   ...
   ![강사가 성공을 표시할 수 있도록 허용하시겠습니까?&quot; 확인란이 강조 표시되어 강사가 모듈에 대한 학습자 성공 상태를 기록할 수 있도록 합니다.](/help/migrated/authors/feature-summary/assets/allow-instructor-mark-success.png)
...
1. 책임자 모드에서 왼쪽 창의 **[!UICONTROL 강의]**&#x200B;를 클릭합니다. 이 페이지에서 Learning Manager 계정의 모든 작성자가 생성한 모든 강의를 확인할 수 있습니다.
...
1. 학습자 포함 필드에서 학습자의 이름을 입력하고 학습자를 선택합니다. 여기에서 사용자 그룹을 추가할 수도 있습니다. Learning Manager 계정에 모든 학습자를 등록하려면 모두를 입력합니다. 학습자를 팀에 등록할 수도 있습니다.
...
1. 새로 생성된 카탈로그 옵션에서 추가할 수 있는 여러 학습 유형(예: 학습 프로그램, 인증 또는 강의)을 선택할 수 있습니다.
...
**사전 요구 사항:** 책임자는 책임자 앱의 **설정 > 일반**&#x200B;에서 **여러 번 시도** 옵션을 활성화해야 합니다.
