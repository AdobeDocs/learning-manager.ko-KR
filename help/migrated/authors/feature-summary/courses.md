...
1. **강의 설정 수정**-
   ...
   * **관리자 승인:** 관리자가 이 강의를 승인합니다. 학습자는 이러한 과정에 등록할 수 있지만 관리자의 승인 없이 이러한 유형의 과정에 직접 등록할 수는 없습니다. 학습자가 이 유형의 강의에 참여하면 관리자에게 알림 요청이 전송됩니다. 관리자가 승인하면 학습자에게 해당 강의가 등록됨으로 목록화됩니다.
   * **자가 등록:** 학습자는 해당 유형의 강의에 직접 등록할 수 있습니다.

1. 변경 사항을 저장하려면 **[!UICONTROL 저장]**&#x200B;을 클릭합니다. 과정을 게시하려면 게시&#x200B;]**를 클릭합니다**[!UICONTROL .
...
1. **강의 설정 수정**-
   ...
   1. 강의 설정 페이지에서 강의를 위한 스킬을 선택합니다. 스킬 드롭다운 목록에서 필요한 스킬을 선택합니다. 그런 다음 레벨 드롭다운 목록에서 필요한 레벨을 선택합니다.
   1. 강의 스킬, 레벨을 선택한 다음 스킬에 대한 점수를 설정합니다. 필요한 경우 스킬을 더 추가합니다.
   1. 필요한 경우 과정에 사용자 지정 규정 준수 레이블을 추가합니다. 과정/학습 경로/인증](/help/migrated/authors/feature-summary/courses.md#add-compliance-labels-to-courselearning-pathcertification)에 규정 준수 레이블 추가를 참조하세요[.
   1. **등록 유형** 드롭다운 목록에서 등록 유형을 선택합니다.
...
1. 강의의 가격을 설정할 것인지 무료로 제공할 것인지 선택합니다. 강의를 유료로 제공하려면 **[!UICONTROL 유료]** 옵션을 선택하고 가격을 지정합니다. 그러면 가격이 강의 카드와 학습자의 강의 개요 페이지에 표시됩니다.

   참고: 이 기능은 Adobe Commerce 커넥터가 구성된 경우에만 사용할 수 있습니다.

1. 학습자가 등록한 강의를 직접 취소할 수 있게 하려면 **학습자가 직접 등록을 취소할 수 있음** 확인란을 선택합니다.

1. **인스턴스 구성**

   이 옵션을 활성화하면 진행 중 상태에 있는 학습자는 다른 인스턴스를 방문하여 등록할 수 있습니다. 학습자는 이전 인스턴스의 진행률을 유지할 수 있습니다.
   ...
   * 혼합
   ...
   참고: 코스를 복제하는 동안 소스 코스에서 인스턴스 구성 옵션을 활성화한 경우 해당 옵션은 대상 코스에서 비활성화된 상태로 유지됩니다.

   **인스턴스 스위치는 다음에 대해 지원되지 않습니다.**
...
1. **다중 등록**
   ...
   이를 사용하여 하나 또는 다른 기간에 두 개 이상의 강의 인스턴스에 학습자를 등록할 수 있습니다.

   다중 등록&#x200B;**토글을**&#x200B;활성화하여 학습자의 다양한 과정 등록 간을 전환할 수 있습니다. 인스턴스 전환을 활성화한 경우 다중 등록을 사용할 수 없습니다.

1. 강의를 수강하기 전에 완료해야 하는 전제 조건 강의를 선택합니다. 강의 필드를 클릭한 다음 강의 목록에서 선택합니다.
1. ******필수 구성 요소 과정을 필수로 설정하려면 필수 구성 요소** 사용 확인란을 선택합니다.
...
1. 강의와 관련된 키워드를 태그로 추가합니다. 학습자는 태그를 통해 검색 중 귀하의 강의를 쉽게 찾을 수 있습니다. 이 모든 태그는 추가한 모듈에 따라 자동으로 추가됩니다. 이 강의에 추가하려는 다른 태그가 있으면 계속 입력할 수 있습니다.
1. 자동 사용 중지 필드에서 강좌가 사용 중지되는 날짜를 선택합니다. 책임자는 먼저 자동 중단 옵션을 활성화해야 합니다.
1. 변경 사항을 저장하려면 **[!UICONTROL 저장]**&#x200B;을 클릭합니다. 과정을 게시하려면 게시&#x200B;]**를 클릭합니다**[!UICONTROL .

### 과정/학습 경로/인증에 규정 준수 레이블 추가 {#add-custom-compliance-label}

코스에 규정 준수 레이블을 추가하려면 다음 단계를 따르십시오.

1. 작성자 앱에서 과정&#x200B;]**/학습 경로]**/**[!UICONTROL **[!UICONTROL &#x200B;인증&#x200B;]**으로**[!UICONTROL &#x200B;이동하여 추가를&#x200B;]**선택합니다**[!UICONTROL .
1. 이름과 설명, 기술과 같은 기타 세부 정보를 입력합니다.
1. **[!UICONTROL 사용자 지정 규정 준수]** 텍스트 상자에 규정 준수 레이블을 입력하고 선택합니다.
   ...
   >[!IMPORTANT]
   >
   >Custom Compliance를 추가할 때 과정의 기한을 설정해야 합니다.

1. 과정/학습 경로/인증을 저장하고 게시합니다.
이제 과정/학습 경로/인증은 규정 준수 유형으로 간주됩니다. 관리자는 이 과정을 규정 준수 대시보드에 추가하고 관리자와 공유하여 진행 상황을 추적할 수 있습니다

>[!NOTE]
>
>작성자는 규정 준수 레이블을 편집하여 기존 과정/학습 경로/인증에 추가할 수도 있습니다.

## 게임화 점수

강의 및 강의 인스턴스 레벨에서 게임화 점수를 할당할 수 있습니다. 이를 통해 다양한 강의 또는 인스턴스에 점수를 부여할 수 있습니다. 학습자는 특정 과정을 수강하거나 다른 과정보다 특정 과정을 선호하도록 인센티브를 받습니다.

1. 강의 인스턴스 레벨에서 **[!UICONTROL 게임화 점수]**&#x200B;를 선택합니다.

![게임화 포인트](assets/select-gamification-points-new.png)

*게임화를 위한 포인트 설정*

1. **[!UICONTROL 편집]**&#x200B;을 선택합니다.
1. 코스 수준 설정 사용을 선택하면 다음 옵션이 표시됩니다.
...
1. **[!UICONTROL 사용자 정의 설정 사용]**&#x200B;을 선택하는 경우 다음 옵션이 표시됩니다.
   ...
   * **추가 규칙**
   ...
   ![게임화 포인트](assets/gamification-custom-settings.png)
...
작성자는 학습 경로&#x200B;]**>**[!UICONTROL &#x200B;설정을&#x200B;]**선택합니다**[!UICONTROL . [편집]]**을 클릭합니다**[!UICONTROL .

**[!UICONTROL 리소스]** 섹션의 학습 경로 수준에서 집계된 구성 과정 리소스 표시 확인란은 활성화된 경우 과정 수준에 있는 리소스가 학습 경로 수준에 표시되는지 여부를 표시합니다.

>[!NOTE]
>
>학습 경로의 설정 페이지에서 관리자는 학습 경로 수준에 표시되는 과정 수준에 있는 리소스를 표시하는 이 옵션을 활성화할 수도 있습니다.

## 스케줄링 도우미

강사 및 강의실 예약의 충돌을 관리합니다. 강사를 코스에 배정하기 전에 강사가 참석할 수 있는 시간과 날짜를 알고 싶다면 예약 도우미를 사용하십시오.

VC 또는 CR 과정에 대한 과정을 만드는 동안 Scheduling Assistant를 클릭합니다.

![Scheduling Assistant를 선택합니다](assets/scheduling-assistant.png)

*일정 도우미 시작*

일정 도우미 창이 시작됩니다.

![Scheduling Assistant 화면](assets/scheduling-assistant-window.png)
...
여러 명의 강사를 선택할 수 있지만, 한 번에 한 명의 강사만 할당할 수 있습니다. 선택한 시간이 시간 충돌 창에서 강조 표시됩니다. 강사 근처에 십자 아이콘이 나타나며, 이 아이콘을 클릭하면 강사를 제거할 수 있습니다.
...
또한 강사 검색 상자 오른쪽에 있는 필터 아이콘을 클릭하면 표시되는 스킬을 검색할 수 있습니다. 아래 스크린샷에 버튼이 표시됩니다.

![강사를 위한 기술 입력](assets/scheduling-assistant-instructor-skill.png)
...
이는 코스 또는 인스턴스 페이지의 Scheduling Assistant에 적용됩니다.
...
Scheduling Assistant는 인스턴스 페이지에서도 사용할 수 있으며 관리자 및 사용자 지정 관리자/작성자도 사용할 수 있습니다.

![인스턴스 페이지의 Scheduling Assistant](assets/instances-scheduling.png)

*세션 페이지에서 강사 예약*

### 위치 검색

모듈과 일정 도우미 페이지 모두에서 강의실 이름과 위치 영역 이름을 모두 지정하여 위치를 검색할 수 있습니다.

## 서식 있는 텍스트 서식

강의, 학습 프로그램, 인증 또는 작업 지원을 만드는 동안 작성자는 텍스트, 이미지와 같은 다양한 유형의 콘텐츠를 입력하거나 다양한 텍스트 서식 옵션을 적용할 수 있습니다.

코스를 생성할 때 코스 개요 필드에서 리치 텍스트 편집기를 볼 수 있습니다. 콘텐츠의 서식을 지정하고, 이미지를 추가하고, 하이퍼링크를 추가하는 등의 작업을 수행할 수 있습니다.
...
첨부된 CSS 스타일시트에는 Learning Manager에서 적용한 스타일이 포함되어 있습니다. 스타일은 대부분의 사용 사례를 고려하여 조정됩니다. 첨부된 CSS 파일을 다운로드하고 규칙 및 빌드 시스템에 따라 웹 앱으로 가져옵니다. 정의된 CSS 클래스는 ql-editor 클래스 아래에 네임스페이스가 지정되며 기존 스타일을 방해하지 않습니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>
    <p>모듈에 대한 시도 횟수를 무한으로 또는 지정된 횟수만큼 설정할 수 있습니다.<span style="font-size: 0.8125rem;">활성화되면 시도 정보가 학습자에게 표시됩니다. 학습자는 '다시 시도' 버튼을 클릭하여 모듈을 다시 시도하도록 선택할 수 있습니다.</span></p></td>
  </tr>
  <tr>
   <td>
    <p>모듈을 완료 또는 통과하면 새로운 시도 중지</p></td>
   <td>
    <p>학습자가 새로운 시도 옵션을 선택하지 못하게 할 시점을 구성하려면 "모듈이 완료되거나 통과되면 새로운 시도 중지" 체크 상자를 활성화합니다. '재시도' 옵션은 학습자 모듈을 성공적으로 완료하면 학습자 보기에서 제거됩니다.</p></td>
  </tr>
  <tr>
   <td>
    <p>시도 간 모듈 잠금 0:0:1 형식: 일/시간/분</p></td>
   <td>
    <p>"<b>시도 간 모듈 잠금 0:0:1 형식: 일/시간/분" 확인란을 활성화하여 시도 사이의 특정 시간 동안 모듈을 잠글 수 있습니다</b>. 모듈이 잠겨 있으면 학습자는 제공된 잠금 시간이 경과할 때까지 모듈을 방문할 수 없습니다. </p>
    <p>ou는 '<b>플레이어 닫기</b>' 또는 '<b>완료</b>' 확인란을 선택하여 시도의 종료 기준을 정의할 수 있습니다.</p></td>
  </tr>
  <tr>
   <td>
    <p>플레이어 닫기</p></td>
  ...
  <tr>
   ...
   <td>
    <p>작성자는 "<b>모듈 완료에 대한 시간 제한 설정" 확인란을 활성화하여 모듈 완료 시간 제한을 설정할 수 있습니다</b>.</p>
...
1. **[!UICONTROL 모듈 추가]**&#x200B;를 클릭합니다. 모듈을 추가하는 네 가지 옵션이 표시됩니다. 첫 번째 옵션은 자가 진행식 모듈을 추가하는 것입니다. 이 모듈을 통해 Adobe Learning Manager에서 모듈 라이브러리를 만들고 추가할 수 있습니다. 두 번째 옵션은 가상 강의실을 설정하는 것입니다. 세 번째 옵션은 강의실 모듈을 설정하는 것이며 네 번째는 활동 모듈입니다.
   ...
   *VC 모듈 추가*
   ...
   모임 이끌이에 대해 예를&#x200B;**선택하는**&#x200B;경우 이끌이의 이름을 입력해야 합니다. 이름을 입력하고 주최자를 선택합니다.
   ...
   *클래스룸 모듈 추가*
   ...
   과정을 만들 때 가상 교실 구성 대화 상자에서 회의 시스템을 만든 Microsoft Teams 연결로 설정합니다. 이벤트에 대한 회의 주최자를 원하는지 여부를 선택합니다.

   모임 이끌이에 대해 예를 선택하는 경우 이끌이의 이름을 입력해야 합니다. 주최자의 이름을 입력하고 주최자를 선택하십시오.

   **로비 우회**

   * 예를 선택하면 모든 학습자가 미팅에 참여할 수 있습니다.
   * 아니요를 선택하면 학습자가 회의에 참여하는 것을 허용하거나 금지하는 요청이 이끌이에게 전송됩니다.

   **참고:** 학습자가 게스트로 Microsoft Teams에 참가하려면 이메일을 입력해야 합니다. 이메일은 Learning Manager에 있어야 합니다.
   ...
   *활동 모듈 추가*
   ...
   활동 유형 파일 제출 및 xAPI 기반 모듈에 대한 과정에서 활동 모듈을 추가하는 동안 기간을 지정할 수 있습니다.
...
작성자는 활동 모듈에 HTML 링크를 추가하고 완료 기준을 설정할 수 있습니다. HTML 링크를 추가하고 완료 기준을 설정하려면 다음 단계를 수행합니다.

1. 작성자 앱의 홈페이지에서 [Create Courses ]**]를 선택합니다**[!UICONTROL .
1. 코스 카탈로그&#x200B;]**화면에서 추가를]** 선택합니다&#x200B;**[!UICONTROL .**[!UICONTROL 
1. 과정의 이름과 설명을 입력합니다.
1. 모듈 옵션에서 Add Module ]**>**[!UICONTROL  Activity Module ]**을 선택합니다**[!UICONTROL .]****[!UICONTROL 
1. 활동 모듈&#x200B;]**프롬프트에**[!UICONTROL &#x200B;이름과 설명을 입력합니다.
1. [유형&#x200B;]**]을 [외부 URL]**]로 **[!UICONTROL **[!UICONTROL &#x200B;선택합니다.
1. [완료 기준&#x200B;]**] 옵션에서**[!UICONTROL &#x200B;다음 옵션을 선택합니다.
   * **[!UICONTROL 학습자 표시]**: 학습자는 Fluidic Player에서 강의를 완료로 표시할 수 있습니다.
   * **[!UICONTROL 콘텐트]** 시작 시: 학습자가 강의를 시작하면 과정이 자동으로 완료로 표시됩니다.

   ![](assets/completion-criteria-activity-module.png)
   _완료 기준_

1. 과정 추가&#x200B;]**및 게시를 선택합니다**[!UICONTROL .

## 체크리스트 {#create-checklist}

평가는 LMS의 중요한 측면입니다. 온라인 평가는 주제에 대한 학습자의 이해도를 평가하는 가장 좋은 방법 중 하나입니다. 그러나 종종 직장에 있는 동안 필요한 작업을 수행하는 것을 관찰하여 이해도를 평가해야 합니다.
...
작성자만 체크리스트를 만들 수 있습니다. 체크리스트는 활동 모듈의 유형입니다. 활동 모듈을 설정하는 동안 작성자는 아래와 같이 활동을 체크리스트&#x200B;**로**&#x200B;선택할 수 있습니다.
...
또한 댓글이나 메모가 있는 경우 강사&#x200B;**에게 메모 텍스트 필드에 추가할**&#x200B;수 있습니다.
...
1. **모듈** 추가 섹션에서 교수자를 할당하려는 모듈을 클릭합니다.
...
1. 업데이트된 강의를 다시 게시하려면 [다시 게시&#x200B;]**]를 클릭합니다**[!UICONTROL .

## 관찰 체크리스트

이제 강사뿐만 아니라 관리자도 체크리스트 모듈을 검토할 수 있습니다. 인력 관리자 및 스토어 관리자 또는 위치 관리자와 같은 비계층 관리자도 체크리스트를 검토하고 완료할 수 있습니다.

강좌 작성자는 체크리스트 모듈을 설정하는 동안 &quot;검토자&quot; 섹션에서 이러한 역할 옵션을 선택하여 사용자 관리자와 비계층적 관리자(해당되는 경우)를 검토자로 추가할 수 있습니다. 강의 인스턴스 레벨에서 수행할 수 있습니다.

![관리자를 위한 체크리스트](assets/manager-checklist.png)

*활동 모듈에 검토자 추가*

&#39;**[!UICONTROL +관리자&#39; 옵션을 선택하면 조직 계층 구조에서 학습자의 관리자가 자동으로 체크리스트를 검토할 수 있습니다]**. 관리자 이름을 개별적으로 검색하고 추가할 필요가 없습니다.

계정 책임자가 활성 필드 옵션을 사용하여 비계층 관리자 역할(예: 위치 관리자 또는 사이트 관리자)을 설정한 경우, 해당 관리자 역할을 선택하여 체크리스트를 검토할 수 있습니다.

관리자 이름을 개별적으로 검색하고 추가할 필요가 없습니다. 학습자가 체크리스트 강의에 등록하면 선택한 강사와 함께 검토를 위해 자동으로 관리자/스토어 관리자에게 알림을 보냅니다. 이 워크플로를 통해 작성자는 개별 관리자의 이름을 언급하지 않아도 됩니다.

위에 제공된 샘플 스크린샷에서 &quot;**[!UICONTROL +Store Managers]**&quot; 옵션을 선택하면 학습자에 맞춰 계층이 아닌 관리자가 체크리스트를 검토할 수 있도록 자동으로 활성화됩니다. 여기서 &quot;store&quot;는 관리자가 정의한 활성 필드로 대체됩니다.
...
![학습자로 과정 미리 보기](assets/preview-as-a-learner.png)
...
* **초안** - 작성자가 코스 생성 및 저장을 완료한 경우. 이 상태인 경우 학습자는 강의를 사용할 수 없습니다.
* **게시됨** - 작성자가 강의 게시를 완료한 경우입니다. 이 상태인 경우 학습자는 강의에 등록할 수 있습니다. 이 상태의 강의는 편집할 수도 있습니다.
* **중단** 됨 - 작성자는 강의를 게시한 후 학습자를 위한 강의 카탈로그에 강의가 표시되는 것을 원하지 않는 경우 강의를 중단됨 상태로 이동할 수 있습니다.
* **삭제됨** - 삭제된 상태의 강의는 Adobe Learning Manager 응용 프로그램에서 완전히 제거된 경우입니다. 작성자만 초안 또는 중단됨 상태에 있는 과정을 삭제할 수 있습니다.
...
작성자는 강의를 정렬하여 더 쉽게 필요한 강의를 찾을 수 있습니다. 정렬 기준을&#x200B;]**클릭하고**[!UICONTROL &#x200B;알파벳순 오름차순, 알파벳순 내림차순, 코스 생성 날짜, 코스 업데이트 날짜 및 코스 효과를 선택합니다.
...
1. 학습자 등록 대화 상자에서 오른쪽 상단 모서리의 **기본 인스턴스** 옵션이 선택되어 있는 것을 볼 수 있습니다. 작성자가 과정을 생성하자마자 해당 과정의 기본 인스턴스가 생성됩니다.

   ![](assets/default-instance.png)

   *과정의 기본 인스턴스 보기*

1. 학습자 포함 필드에서 학습자의 이름을 입력하고 학습자를 선택합니다. 여기에서 사용자 그룹을 추가할 수도 있습니다. Learning Manager 계정에 모든 학습자를 등록하려면 all을 입력하기 시작합니다. 학습자를 팀에 등록할 수도 있습니다.
...
1. Captivate에서 게시&#x200B;]**를 클릭합니다**[!UICONTROL > **[!UICONTROL Learning Manager]**&#x200B;에 게시합니다.
...
인증 및 학습 프로그램을 만들고 관리하는 방법을 알아보려면 인증](/help/migrated/administrators/feature-summary/certifications.md) 및 [학습 프로그램을](/help/migrated/administrators/feature-summary/learning-programs.md) 참조하십시오[.
...
+++코스에 대한 &quot;관리자 지명 찾기&quot;를 제거하는 방법은 무엇입니까?

다음 단계를 수행하십시오.

1. Learning Manager에 작성자로 로그인합니다.
1. 강의를 엽니다.
1. 왼쪽 창에서 [설정] > **[!UICONTROL [편집]**]을 클릭합니다&#x200B;**[!UICONTROL .]**
...
+++모든 학습자가 모든 강의를 볼 수 없도록 하는 방법은 무엇입니까?

카탈로그를 통해 설정할 수 있습니다. 기본 카탈로그에는 기본적으로 Learning Manager에 추가된 모든 강의가 포함되어 있습니다.
...
1. 새로 생성된 카탈로그 옵션에서 추가할 수 있는 다양한 학습 유형(예: 학습 프로그램, 인증 또는 과정)을 선택할 수 있습니다.
...
1. 카탈로그 페이지에서 D[!UICONTROL efault 카탈로그&#x200B;]**를**&#x200B;비활성화하고 작성한 카탈로그를 활성화하십시오.

![](assets/enable-custom-catalog.png)

+++

+++완료한 과정에 다시 등록하려면 어떻게 해야 합니까?

강의 완료는 되돌릴 수 없습니다. 학습자는 완료한 강의를 **다시 등록할 수 없습니다**.

+++

+++학습자가 과정을 완료한 후에도 과정을 보려면 어떻게 해야 합니까?
...
+++모듈에서 여러 번 시도를 설정하는 방법은 무엇입니까?

**전제 조건:** 관리자는 관리 앱의 [설정] > [일반&#x200B;**]에서**[다중 시도&#x200B;**] 옵션을**&#x200B;활성화해야 합니다.
...
+++Adobe Learning Manager에 업로드된 콘텐츠를 다운로드하여 수정할 수 있습니까?
