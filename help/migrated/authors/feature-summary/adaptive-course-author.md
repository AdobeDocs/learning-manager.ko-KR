...
* 필수 모듈이 지정된 그룹이 있는 경우 해당 학습자는 필수 모듈입니다.
...
적응 과정이 포함된 카탈로그가 피어 계정 외부로 공유되면 다음 동작이 적용됩니다.

* **직접 공유된 적응형 과정:** 적응형 과정이 공유 카탈로그에서 제외됩니다. 수신 계정에는 표시되지 않습니다.
* **학습 경로 또는 인증 내 적응 과정:** 적응 과정이 포함된 LP나 인증이 공유되면 LP 또는 인증 자체가 수신 계정으로 복사됩니다. 해당 응용 프로그램 내의 적응 과정이 **정규 과정**(모든 표시 및 완료 규칙을 포함한 적응 구성)으로 복사되지 않습니다. 수신 계정의 작성자는 강의를 모든 학습자가 볼 수 있는 모든 모듈이 있는 정규 강의로 봅니다.
* **필수 구성 요소로 설정된 적응형 과정:** 적응형 과정이 공유되는 일반 과정, 학습 경로 또는 인증의 필수 구성 요소로 구성된 경우 필수 구성 요소 관계가 수신 계정으로 전파되지 않습니다. 상위 과정 또는 LO는 전제 조건 없이 수신 계정에 도착합니다.

>[!NOTE]
>
>적응형 구성은 카탈로그 공유 중에 복사되지 않으므로 카탈로그를 외부에서 공유하기 전에 모든 필수 구성 요소 관계 및 LP/인증 구조를 검토하십시오. 수신 계정의 학습자는 소스 계정의 학습자와 동일한 적응형 동작을 경험하지 않습니다.
...
>[!NOTE]
>
>학습자는 전환 인스턴스에 대기자 명단에 등록될 수 없으며 등록은 차단됩니다.
...
>[!NOTE]
>
>**순서가 지정된 학습 경로:** 순서가 지정된 학습 경로에 적응형 과정이 포함되면 적응형 과정에 표시되는 모듈이 없는 학습자는 강의를 완료할 수 없습니다. 이렇게 하면 순서가 지정된 학습 경로의 모든 후속 항목에 액세스할 수 없습니다. 학습 경로에 등록된 모든 학습자가 경로의 모든 적응형 강의에서 하나 이상의 모듈을 볼 수 있는 하나 이상의 사용자 그룹에 속해 있는지 확인합니다.

>[!NOTE]
>
>**정규 학습 경로 — 자동 등록 취소:** 사용자 그룹 변경으로 보이는 모든 모듈이 제거되었으므로 학습자가 정규 학습 경로 내의 적응 과정에서 자동 등록 취소되면 상위 학습 경로는 등록된 상태로 유지됩니다. 학습 경로는 자동 등록 취소되지 않습니다. 학습자는 적응형 강의에 더 이상 액세스할 수 없어도 학습 경로가 성적 증명서에 등록된 것으로 확인합니다. 응용 프로그램을 사용할 때 적응 과정이 등록 취소되는 학습 경로가 필요한 경우 일반 학습 경로 대신 **적응 학습 경로**&#x200B;를 사용하십시오.
...
* 인스턴스에 사용할 수 있는 시트가 없으면 대기자 명단에 올릴 수 없습니다. 등록이 차단됩니다.
...
정규 강의에서 강사는 세션 대기자 명단에 넣을 수 있는 학습자 수의 한도인 **대기자 명단 제한**&#x200B;을 구성할 수 있습니다.

적응형 강의에서는 강사 앱에서 **대기자 명단 제한** 설정을 사용할 수 없으므로 구성할 수 없습니다. 적응형 강의의 세션 대기자 명단에 등록될 수 있는 학습자 수에는 제한이 없습니다. 세션이 가득 찼을 때 등록하려는 모든 학습자는 제한 없이 대기자 명단에 등록됩니다.
...
>[!NOTE]
>
>**Flex 학습 과정 동작:** 적응형 과정이 Flex 학습 과정의 일부인 경우 대기 중 동작은 직접 등록과 다릅니다. 학습자가 Flex LP 내에서 적응 강좌 인스턴스를 선택하고 해당 인스턴스에 대해 사용 가능한 시트가 없는 경우 학습자는 특정 인스턴스에 대해 대기자 명단에 나열됩니다. 이 시나리오의 대기자 명단에 등록된 학습자 정보는 **관리자 > [적응형 과정] > 대기자 명단**&#x200B;에서만 볼 수 있습니다. **관리자 > 학습 경로**&#x200B;에는 표시되지 않습니다. 적응 과정의 자체 대기자 명단 탭을 확인하여 Flex LP를 통해 대기자 명단에 등록된 학습자를 관리합니다.

Flex 학습 경로의 일부인 적응형 과정에서 세션에 대한 **출석 보고서 PDF**&#x200B;을 다운로드하면 대기자 명단에 등록된 학습자가 PDF의 **활성** 섹션 아래에 표시됩니다. 학습 경로 인터페이스에는 대기자 명단 섹션이 별도로 존재하지 않기 때문이다. 대기자 명단에 등록된 학습자를 식별하고 출석을 표시하기 전에 확인된 참석자와 구분하려면 **관리자 > [적응 과정] > 대기자 명단**&#x200B;을 사용합니다.
