...
조직 지원 책임자 전자 메일 주소를 추가하거나 변경하려면 을 클릭하여 구성할 수 있습니다. **[!UICONTROL 일반]** 왼쪽 창에. 다음을 수행합니다. **[!UICONTROL 변경]** 에 인접하여 **[!UICONTROL 지원 이메일 ID]** 이메일 id를 추가합니다. 학습자가 을 클릭하면 책임자에게 전자 메일이 전송됩니다 **[!UICONTROL 관리자에게 문의]** 를 클릭합니다.
...
>[!NOTE]
>
>외부 프로필이 여러 개 설정되어 있으면 모든 프로필은 하나의 로그인 모드를 가져야 합니다. 예를 들어 로그인 유형이 Adobe ID인 경우 모든 프로필은 Adobe ID을 통해서만 로그인해야 합니다. 각 프로필은 개별 로그인 유형을 가질 수 없습니다.
...
다음을 수행합니다. **[!UICONTROL 피드백]** 강의를 완료한 후에 왼쪽 창에서 학습자에게 피드백을 받을 수 있는 설문지를 설정합니다. 자세한 내용은 [강의 기능 도움말 콘텐츠](courses.md) (L1 및 L3 피드백 생성)
...
다음을 수행합니다. **[!UICONTROL 일반]** 왼쪽 창에서 &#39;강의 조정&#39; 옵션을 선택하여 강의 조정 기능을 활성화합니다. 이 기능에 대한 자세한 내용은 [강의 조정](courses.md#main-pars_header_1879001177).

**토론 게시판**

토론 게시판 확인란을 활성화하면 학습자 앱 강의 페이지에서 토론 탭을 사용하여 학습자와 강사가 강의에 대한 의견을 게시할 수 있습니다. 그러나 강의 레벨 설정에 이 기능을 선택하지 않았다고 표시된 경우 강의 레벨 설정이 관리자 설정에 우선합니다.
...
다음을 수행합니다. **[!UICONTROL Adobe Connect]** 왼쪽 창에서 가상 강의실 세션을 호스팅하도록 Adobe Connect 계정을 구성합니다. 자세한 내용은 다음을 참조하십시오.  [Adobe Connect](adobeconnect-integration.md) 기능 도움말.

## 일반 설정 {#general}

다음 설정을 활성화하거나 비활성화하십시오.
...
<table>
 <tbody>
  ...
  <tr>
   <td>토론 게시판</td>
   <td>토론 게시판 확인란을 활성화하면 학습자 앱 강의 페이지에서 토론 탭을 사용하여 학습자와 강사가 강의에 대한 의견을 게시할 수 있습니다. 그러나 강의 레벨 설정에 이 기능을 선택하지 않았다고 표시된 경우 강의 레벨 설정이 관리자 설정에 우선합니다.</td>
  </tr>
  <tr>
   <td>다중 시도</td>
   <td>활성화되면 작성자는 강의 모듈에 대해 여러 번 시도를 구성할 수 있습니다.</td>
  </tr>
  <tr>
   <td>스킬 옵션 탐색</td>
  ...
  <tr>
   <td>스킬/태그 가시성</td>
   <td>학습자에게 모든 스킬 및 태그를 표시합니다. 모든 스킬 및 태그, 할당된 스킬 및 태그, 또는 학습자에게 표시되는 카탈로그의 일부만 표시할 수 있습니다.</td>
  </tr>
  <tr>
   <td>고유 학습 개체 ID</td>
  ...
  <tr>
   ...
   <td>
    ...
    <p><b>참고: </b>필터 <b>형식 </b>및 <b>기간 </b>기본적으로 꺼져 있으며 릴리스 이후 즉시 학습자에게 표시되지 않습니다. 책임자는 이를 활성화해야 합니다. <br></p></td>
  </tr>
  <tr>
   <td>카탈로그 목록 표시</td>
  ...
  <tr>
   <td>모듈 버전 업데이트</td>
   <td>콘텐츠를 업데이트하도록 기본 설정을 구성합니다. 강의 페이지에서 각 콘텐츠에 대한 설정을 수정할 수 있습니다.</td>
  </tr>
  <tr>
   <td>사용자 자동 등록</td>
   <td>활성화되면 새롭게 가져온 사용자가 자동 등록됩니다. 기본적으로 사용자는 수동으로 등록해야 Learning Manager 사용을 시작할 수 있습니다.</td>
  </tr>
  <tr>
   <td><a id="autodelete"></a>내부 사용자 자동 삭제</td>
   <td>활성화되면 내부 사용자가 지정된 기간 동안 시스템에 액세스하지 않으면 자동으로 삭제됩니다. 이 기능은 역할만 맡은 사용자에게 적용됩니다 <b>학습자</b>. 액세스를 복원하려면 관리자에게 문의해야 합니다.<br></td>
  </tr>
  <tr>
   <td>카탈로그 레이블 표시</td>
  ...
  <tr>
   ...
   <td>
    ...
    <ul>
     <li>대상 <b>활성 계정</b>, 다이제스트 전자 메일은 기본적으로 비활성화되며 책임자는 수동으로 이를 활성화할 수 있습니다.</li>
     <li>대상 <b>평가판 계정</b>, 다이제스트 전자 메일의 옵션은 비활성화된 상태로 유지되며 책임자는 옵션을 활성화할 수 없습니다.</li>
    </ul>
    <p>기능이 비활성화되면:</p>
    <ul>
     <li>옵션 <b>다이제스트 이메일</b> 이(가) 비활성화됩니다.</li>
     <li>학습자는 다이제스트 전자 메일 구독에 대한 사용자 설정을 볼 수 없습니다.</li>
    </ul>
    <p> 기능이 활성화되면:</p>
    <ul>
     <li>책임자는 다이제스트 전자 메일 옵션을 활성화하고 수정할 수 있습니다.</li>
     <li>(으)로부터 <b>프로필 설정 </b>학습자 앱에서 학습자(DND 목록에는 없음)는 다이제스트 전자 메일을 구독/구독 취소하도록 선택할 수 있습니다.</li>
    </ul></td>
  </tr>
  <tr>
  ...
  <tr>
   <td>바닥글 링크</td>
   <td>
    <p>바닥글로 표시되는 링크 또는 전자 메일 ID를 추가합니다. 최대 3개의 바닥글 링크를 추가할 수 있습니다.</p>
    <p>바닥글의 링크를 사용자 정의하려면 다음 단계를 수행하십시오.</p>
    <ol>
     <li>다음을 수행합니다. <b>더 추가하기</b>지정된 필드에 이름과 URL 또는 이메일 ID를 입력합니다. URL 앞에 http:// 또는 https://을 붙입니다.</li>
     <li>모든 로케일에 변경 내용을 계단식으로 배열하려면 <b>복제</b>. 이렇게 하면 모든 언어에 이름 및 URL이 지정됩니다.</li>
     <li>변경 사항을 저장하려면 를 클릭합니다. <b>저장</b>. 변경 사항을 확인하는 팝업 메시지를 볼 수 있습니다. [확인]을 클릭하면 바닥글이 새로 추가된 링크로 채워집니다.</li>
    </ol>
    <p>또한 다음을 수행할 수 있습니다.</p>
    <ul>
     <li>다음을 클릭합니다. <b>재설정</b> 아이콘을 클릭하여 기본값 재설정 <b>도움말</b> 및 <b>관리자에게 문의</b> 필드.</li>
     <li>모든 언어에 대해 바닥글의 링크를 사용자 정의할 수 있습니다. 다음을 클릭합니다. <b>언어</b> 드롭다운 목록에서 언어를 선택하고 <b>이름</b> 및 <b>URL</b> 을 입력합니다. 변경 사항을 저장하면 업데이트된 링크가 바닥글에 표시됩니다.<br></li>
    </ul></td>
  </tr>
  <tr>
   <td>시간대 보고<br></td>
...
<table border="0" cellpadding="0" cellspacing="0" width="1709">
 <tbody>
  ...
  <tr>
   ...
   <td>활성화되면 내부 사용자가 지정된 기간 동안 시스템에 액세스하지 않으면 자동으로 삭제됩니다. 이 기능은 학습자 역할만 맡은 사용자에게 적용됩니다. 액세스를 복원하려면 관리자에게 문의해야 합니다.</td>
  </tr>
  <tr>
  ...
  <tr>
   <td height="20">학습자는 자신의 점수를 볼 수 있습니다</td>
   <td>활성화되면 학습자는 학습자 성적 증명서에서 자신의 점수를 볼 수 있습니다.</td>
  </tr>
  <tr>
   <td rowspan="9" height="172">다이제스트 이메일</td>
  ...
  <tr>
   ...
   <td>바닥글로 표시되는 링크 또는 전자 메일 ID를 추가합니다. 최대 3개의 바닥글 링크를 추가할 수 있습니다.</td>
  </tr>
  <tr>
   <td height="19">바닥글의 링크를 사용자 정의하려면 다음 단계를 수행하십시오.</td>
  </tr>
  <tr>
   <td height="19">1. 더 추가를 클릭하고 지정된 필드에 이름과 URL 또는 전자 메일 ID를 입력합니다. URL 앞에 http:// 또는 https://을 붙입니다.</td>
  </tr>
  <tr>
   <td height="19">2. 모든 로케일에 변경 사항을 계단식으로 배열하려면 복제를 클릭합니다. 이렇게 하면 모든 언어에 이름 및 URL이 지정됩니다.</td>
  </tr>
  <tr>
   <td height="19">3. 변경 사항을 저장하려면 저장을 클릭합니다. 변경 사항을 확인하는 팝업 메시지를 볼 수 있습니다. [확인]을 클릭하면 바닥글이 새로 추가된 링크로 채워집니다.</td>
  </tr>
  <tr>
  ...
  <tr>
   <td height="19">계정 수준 시간대 환경 설정</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>활성화되면 학습자는 Badgr 웹 사이트에 자신의 배지를 업로드할 수 있습니다. 고객 교육 시나리오에서 조직은 고객을 "인증"하고 소셜 미디어를 통해 해당 자격 증명을 표시할 수 있는 기회를 제공하고자 합니다. 이는 학습자가 교육을 받고 각자의 성과를 다른 사람들과 공유할 수 있도록 동기를 부여합니다. </td>
  </tr>
  <tr>
   <td height="135">
    <p>등급 표시</p></td>
   <td>
    <ul>
     <li>필요한 경우 <b>강의 효과</b> 이(가) 활성화되면 학습자는 강의 효과 값만 확인할 수 있습니다.</li>
     <li>필요한 경우 <b>별점 매기기</b> 이 옵션을 활성화하면 학습자는 평균 평가 등급과 함께 강의를 평가한 학습자 수만 확인할 수 있습니다.<br></li>
    </ul>
    <p>이 기능은 강의에서만 사용할 수 있습니다. 학습 프로그램이나 인증에서 평가 등급은 지원되지 않습니다.<br><br><b>참고: </b>해당 변경은 학습자 앱에만 영향을 줍니다. </p>
    <p>다른 모든 앱(책임자, 작성자, 관리자, 사용자 정의 책임자, 사용자 정의 작성자)에서는 설정 변경(평가 등급/강의 효과/등급 표시 비활성화)이 영향을 미치지 않습니다. </p>
 ...
 <tbody>
  <tr>
   <td>
    <p>학습 경로</p></td>
   <td>
    <p>필요한 경우 <b>학습 경로의 확장 기능 활성화</b> 활성화하면 책임자가 학습 경로에 학습 프로그램을 삽입하고 이러한 학습 경로를 강의에 반영할 수 있습니다. 이 옵션은 되돌릴 수 없습니다.<br></p></td>
  </tr>
  <tr>
   <td>
    <p>강사 관리<br></p></td>
...
추천 알고리즘은 Adobe이 파트너로부터 소스를 받은 작업 역할, 제목 및 설명에 대한 산업 데이터를 포함하여 여러 입력 소스에 포함되도록 고안되었습니다. 그런 다음 이 데이터는 Learning Manager가 산업 정렬 스킬을 작업 제목 및/또는 직함에 연결하는 지도와 함께 제공될 수 있도록 Adobe의 AI 알고리즘을 교육하는 데 사용됩니다. 그러면 이는 추천 알고리즘에 대한 하나의 입력이 됩니다.

그런 다음 Learning Manager는 주제 모델링 알고리즘을 사용하여 계정 내의 교육 콘텐츠를 분석하고 스킬에 매핑합니다.

Learning Manager는 피어 활동 데이터를 다른 신호로 사용하여 개인화된 방식으로 추천 알고리즘을 추진합니다. 학습자가 제공한 등록, 완료 및 명시적 피드백과 같은 활동이 여기에서 사용됩니다.

또한 Learning Manager는 추가로 추천을 개인화하기 위해 개별 학습자로부터 수집한 명시적 및 암시적 정보를 사용합니다. 학습자는 등록을 통해 명시적으로 관심 영역을 나타낼 수 있으며 Learning Manager는 학습자가 교육 참여를 종료하는 방식을 기반으로 이 정보를 암시적으로 받게 됩니다.
...
1. 다운로드된 CSV 파일에는 A 열에 있는 객체의 이름이 포함됩니다. 열 B에서 해당 개체에 지정할 이름을 선택합니다. (|)로 구분된 이름의 형식은 단수이고 복수입니다.
...
콘텐츠 폴더는 관리자가 구성할 수 있습니다. 한 번만 구성되는 콘텐츠 폴더는 작성자에게 표시되며 작성자는 하나 또는 여러 폴더에 콘텐츠를 배치할 수 있습니다.
...
폴더는 콘텐츠의 저장소로, 다음 속성을 가진 계정에서 사용할 수 있는 전체 콘텐츠 라이브러리의 하위 집합입니다.

* 책임자만 폴더를 만들거나 편집하거나 삭제할 수 있습니다.
* 책임자는 사용자 정의 책임자에 대한 역할만 정의하는 일환으로 폴더에 대한 액세스 권한을 제어할 수 있습니다.
* 내용 **항상 최소 하나의 폴더와 연결되어야 합니다.**. 시작하려면 모든 콘텐츠는 공개 폴더와 연결되며 이는 나중에 변경할 수 있습니다.
* 콘텐츠는 생성 시 여러 폴더와 연결될 수 있으며, 복사 작업으로 이렇게 할 수도 있습니다
...
공개 폴더는 계정에 항상 있으며 초기에 모든 콘텐츠는 이 폴더의 일부입니다. 나중에 작성자는 이 폴더에서 콘텐츠를 다른 폴더로 이동할 수 있습니다. 공용 폴더에는 다음과 같은 속성이 있습니다.

* 이 폴더와 연결된 모든 콘텐츠는 기본적으로 모든 유형의 작성자가 액세스할 수 있습니다.
* 공개 폴더의 일부인 모든 콘텐츠는 다른 폴더의 일부가 될 수 없습니다. 반대로도 적용됩니다.

이 폴더는 구성 가능한 역할 정의의 일부가 될 수 없습니다. 결과적으로 구성 가능한 역할 정의에 공개 폴더가 없으면 공개 폴더에 대한 액세스가 제한되지 않습니다.
...
책임자는 이 설정을 사용하여 강의실 위치 라이브러리를 만들고 구성할 수 있습니다. 작성자는 사전 구성된 위치를 선택하여 강의실 이벤트를 설정할 수 있습니다. 라이브러리에서 위치를 선택하여 위치 정보, URL 및 인원 제한을 자동으로 채웁니다.

관리자는 다음 중 하나를 수행할 수 있습니다.

### 위치 CSV 가져오기

위치의 CSV 파일을 가져와서 계정에 위치를 추가합니다. CSV 파일에는 구/군/시 열이 포함되어야 합니다.

### 위치 추가

다음을 추가합니다.

1. 위치 이름: 강의실 이름을 입력합니다.
2. 위치 정보: 위치에 대한 정보를 입력합니다.
3. 위치 영역: 입력한 값은 학습자용 교육 위치 필터로 나타납니다.
4. 위치 URL: 위치의 URL을 입력합니다.
5. 인원 제한: 회의실의 인원 수를 입력합니다.

![강의실 위치](assets/location-alm.gif)

*강의실 위치 추가*

CSV를 사용하여 위치를 추가할 수도 있습니다. CSV에는 다음 필드가 포함되어야 합니다.
...
자세한 내용은 [콘텐츠 폴더](settings.md#content-folder).
+++
