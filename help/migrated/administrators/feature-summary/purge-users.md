...
>[!NOTE]
>
>삭제된 사용자가 시스템에 다시 추가되면 이전 역할(예: 관리자, 관리자, 작성자, 강사 등)이 유지되지 않습니다.학습자 역할로 추가됩니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td><strong>기타 사용자 제거 - 기업(내부 또는 외부 Learning Manager 사용자가 아닌 개인)</strong></td>
   <td> </td>
  </tr>
  <tr>
...
일괄 삭제 플로우는 일괄 처리당 최대 10,000명의 사용자를 처리할 수 있습니다. 이 기능은 대규모 삭제를 위한 배경 신뢰성 중심 서비스로 포지셔닝됩니다.

관리자는 삭제된 사용자를 삭제할 수 있는 기간을 지정할 수 있습니다. 자세한 내용은 [관리자 설정](/help/migrated/administrators/feature-summary/settings.md)을 참조하세요.

#### 기능:

* 계정 수준에서 삭제된 사용자에 대한 구성 가능한 자동 제거 제공
* 사용자가 제거 기준을 충족하면 24시간 이내에 제거되도록 합니다.
* 시스템 성능 저하 없이 하루에 최대 10,000명의 사용자** 대량 삭제 지원
* 이러한 작업이 실행되는 동안 전반적인 시스템 응답성 및 데이터베이스 상태 유지
* GDPR 의무를 준수하기 위해 자동화된 데이터 보존 관리 적용

#### What it does:

* 대량 삭제 플로우는 예약된 cron 작업으로만 실행됩니다(요청당 온디맨드가 아님).
...
3. 페이지를 아래로 스크롤하여 **삭제된 사용자 자동 제거**합니다.
   ...
   *자동 제거 옵션*
   >[!NOTE]
   >
   >**삭제된 사용자 자동 제거**&#x200B;가 활성화되지 않은 경우, 옵션 섹션 맨 아래에 **구성되지 않음**&#x200B;이라는 메시지가 표시됩니다.
4. **편집**&#x200B;을 선택합니다.
5. **사용 확인란**&#x200B;을 선택합니다.
6. 삭제가 적용되는 기간을 입력합니다.
   >[!NOTE]
   >
