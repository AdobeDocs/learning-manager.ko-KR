...
[![단추](assets/launch-training-button.png)](https://content.adobelearningmanageracademy.com/app/learner?accountId=98632#/course/8318922)
...
1. 책임자는 왼쪽 창에서 카탈로그를 클릭합니다. 이 페이지에는 기본 카탈로그가 있으며 이는 Learning Manager 계정을 만들 때 자동으로 생성됩니다. 기본적으로 이 카탈로그의 모든 학습 객체는 모든 학습자와 공유됩니다.
...
또는 카탈로그 근처의 확인란을 선택할 수 있습니다. 오른쪽 상단 모서리에 있는 작업을 클릭하고 **[!UICONTROL 비활성화]**&#x200B;를 클릭합니다.

또한, 목록에서 여러 카탈로그를 비활성화할 수도 있습니다. 카탈로그를 선택하고 작업 메뉴에서 **[!UICONTROL 비활성화]** 옵션을 선택합니다.
...
[![단추](assets/launch-training-button.png)](https://content.adobelearningmanageracademy.com/app/learner?accountId=98632#/course/8318924)
...
귀하의 Learning Manager 계정과 연결된 서브 도메인을 입력합니다. **[!UICONTROL 하위 도메인을 가져오는 방법]**&#x200B;을 클릭하세요. 을 눌러 하위 도메인 사용에 대해 알아보십시오. 관리자 홈페이지의 왼쪽 창에 있는 **[!UICONTROL 브랜딩]**&#x200B;을 클릭하면 조직 계정과 연결된 서브 도메인을 볼 수 있습니다.
...
**[!UICONTROL 만료]** 필드에서 만료 날짜를 설정하여 공유 카탈로그의 유효성을 설정할 수 있습니다. 공유한 카탈로그의 기한이 지나면 강의에 등록할 수 없습니다. 기존 학습자는 계속 콘텐트에 액세스할 수 있습니다. 학습 객체가 중단되어도 배지와 스킬은 활성화 상태입니다. 이러한 상태에서는 공유한 카탈로그 수신자는 중단된 학습 객체를 중단 취소할 수 없습니다.

모든 필수 정보를 추가한 다음 **[!UICONTROL 저장]**&#x200B;을 클릭합니다. 페이지의 오른쪽 상단 모서리에 있는 **[!UICONTROL 동작]** 드롭다운에서 **[!UICONTROL 공유]**&#x200B;를 클릭하여 외부 계정에 카탈로그를 공유합니다.
...
카탈로그 공유 취소는 응용 프로그램에서 외부 계정을 비활성화하는 것입니다. 카탈로그 공유를 해제하려면 페이지의 오른쪽 상단 모서리에 있는 **[!UICONTROL 작업]**&#x200B;을 클릭하고 **[!UICONTROL 공유 해제]**&#x200B;를 클릭합니다. 언제든지 카탈로그 공유를 취소할 수 있습니다. 카탈로그 공유가 취소되면, 해당 공유 카탈로그에 있는 모든 학습 객체는 중단된 상태로 변경됩니다. 공유가 취소된 카탈로그는 수신자의 응용 프로그램에 중단됨으로 표시됩니다.

수신자가 요청을 수락하지 않은 경우 공유 목록에서 외부 계정을 제거할 수 있습니다.

**카탈로그 삭제**

공유 카탈로그를 삭제하려면 **[!UICONTROL 동작]** 드롭다운을 클릭한 다음 **[!UICONTROL 삭제]**&#x200B;를 클릭합니다.

* 기본 카탈로그는 외부 계정과 공유할 수 없습니다.
* 외부에서 귀하에게 공유한 카탈로그는 삭제할 수 없습니다.
* 다른 계정이 귀하에게 공유한 카탈로그는 삭제할 수 없습니다.

## 획득한 과정/학습 경로 변경 제한 {#restrictchangesonacquiredcourseslp}

계정 공유를 사용하는 조직에서는 학습 콘텐츠가 기본 계정에서 작성 및 유지된 다음 공유 카탈로그를 통해 피어 계정과 공유되는 경우가 많습니다. 이 모델은 여러 계정에서 일관된 콘텐츠를 유지하는 데 도움이 되지만 수신 계정에서 의도하지 않은 수정을 초래할 수도 있습니다.

따라서 계정을 받는 작성자가 원본 계정과 공유 콘텐츠 간의 관계를 끊을 수 있는 변경을 수행하지 못하도록 하여 공유 학습 개체를 보호할 수 있습니다. 이를 통해 컨텐츠 소유자는 중앙 집중식 거버넌스를 유지하면서 소스 계정의 업데이트가 수신 계정으로 계속 전달되도록 할 수 있습니다.

이 기능은 조직에 도움이 됩니다.

* 원본 계정에서 공유된 콘텐츠의 무결성을 유지합니다.
* 소스 계정과의 동기화를 중단시킬 수 있는 우발적인 수정을 방지합니다.
* 여러 계정에 걸쳐 컨텐츠 관리 부담 경감
* 원본 계정에서 수행한 업데이트가 수신 계정으로 계속 전파되는지 확인합니다.
* 작성자가 공유 콘텐츠를 수정하려고 할 때 경고를 명확하게 표시합니다.

카탈로그를 소스 계정에서 수신 계정으로 공유하면 강의, 학습 경로 및 인증과 같은 학습 객체를 수신 계정에서 사용할 수 있습니다.

수신 계정에서 획득한 강의/학습 경로 변경 제한 을 활성화하면 Adobe Learning Manager은 공유 카탈로그에서 생성된 상속된 콘텐츠의 편집을 제한합니다. 작성자는 공유 콘텐츠를 계속 사용하고 사용할 수 있지만 원본 계정과의 관계를 끊을 수 있는 변경 작업을 수행할 수 없습니다.

### 공유 카탈로그 콘텐츠에 대한 편집 제한 사용

**필수 구성 요소**

계정은 이미 공유 카탈로그에서 콘텐츠를 수신하고 있어야 합니다.

1. 관리자 권한으로 ALM에 로그인합니다.
2. **카탈로그**&#x200B;를 선택하십시오.
3. 공유 카탈로그를 선택합니다.
4. 공유 카탈로그의 **개요** 섹션에서 **획득한 과정/학습 경로에 대한 변경 제한** 토글을 선택합니다. 활성화되면, 획득한 카탈로그의 강의/학습 경로 부분에 대한 변경 사항이 메인 계정에서 이 계정으로 업데이트를 진행할 수 있도록 제한됩니다.

   ![](assets/shared-catalog-enabled.png)

5. **업데이트**&#x200B;를 선택하여 구성을 저장합니다.

### 작성자 경험

작성자가 공유 카탈로그를 통해 획득한 강의를 열면 시스템은 다음을 나타내는 정보 메시지를 표시합니다.
...
작성자가 보호된 콘텐츠의 수정을 시도하는 경우:

* 작성자에게 원본 계정에서 강의를 획득했음을 나타내는 경고가 표시될 수 있습니다.
* 이 경고는 콘텐츠를 수정하면 원본 계정과의 관계가 손상될 수 있음을 설명합니다.
* 설정에 따라 영향을 받는 내용이 제한되거나 변경되지 않습니다.

이를 통해 작성자는 변경하기 전에 자신의 동작이 미치는 영향을 이해할 수 있습니다.

**제한된 변경 내용은 무엇입니까?**

편집 제한을 활성화하면 해당 구성 요소가 원본 계정에 의해 유지되므로 상속된 강의 콘텐츠에 대한 수정이 제한됩니다. 이러한 제한은 계정 간의 동기화를 유지하기 위해 고안되었습니다.

소스 계정에서 일반적으로 관리하는 콘텐츠의 예는 다음과 같습니다.
...
목표는 입고 계정의 사본이 출처 버전과 다른 것으로 변할 수 있는 변경을 방지하는 것입니다.

**허용되는 변경 내용은 무엇입니까?**

일부 설정은 수신 계정 내에서 독립적으로 계속 관리되며 편집 가능한 상태로 유지됩니다. 그 기능은 다음과 같습니다.

* 전자 메일 템플릿 사용자 지정
* 인스턴스 관리
* 원본 계정에서 비롯되지 않는 기타 계정 수준 구성

### 예제 시나리오

#### 제한 전

기업 학습 팀은 소스 계정에서 규정 준수 과정을 유지 관리하고 여러 지역 계정과 공유합니다.

지역 계정의 작성자가 획득한 강의를 직접 편집합니다. 따라서 원래 강의와의 연결이 중단되어 향후 원본 계정의 업데이트가 올바르게 적용되지 않을 수 있습니다.

#### 제한 이후

수신 계정에서 **획득한 과정/학습 경로 변경 제한**&#x200B;을 사용할 수 있습니다.

지역 작성자가 공유 강의에 액세스할 때 내용을 보고 할당할 수 있지만, 보호된 필드는 수정할 수 없습니다. 원본 계정에서 수행한 업데이트는 중단 없이 수신 계정으로 계속 전달됩니다.
...
* 공유 카탈로그의 학습 개체는 작성자 및 책임자 응용 프로그램에 표시되지만, 모듈은 숨겨져 나열되지 않습니다.
* 공유 학습 개체는 수신자 계정의 모든 워크플로우에 사용할 수 있습니다. 예를 들어 인증(영구 및 반복), 학습 경로 및 카탈로그와 같은 학습 객체에 공유 강의를 추가하는 경우를 예로 들 수 있습니다. 그러나 이미 다른 계정의 공유 학습 객체가 있으므로 이러한 학습 객체는 다시 공유할 수 없습니다.
...
전달 기능의 일부로 지원되는 학습 객체는 다음과 같습니다.
...
1. 오른쪽 상단 모서리의 **[!UICONTROL 편집]**&#x200B;을 클릭합니다. 이 페이지에 사용 가능한 카탈로그 레이블 목록이 표시됩니다.
1. 카탈로그에 레이블을 추가하려면 **[!UICONTROL &#39;카탈로그 추가&#39;]**&#x200B;를 클릭합니다.
1. 카탈로그에 추가된 기존 레이블을 제거하려면 **[!UICONTROL 제거]**&#x200B;를 클릭합니다.
