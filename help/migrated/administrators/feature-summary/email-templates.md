...
**대체 경로를 통해 강의/경로가 완료된 것으로 표시되었습니다.** 이 템플릿은 학습자가 대체 경로를 통해 강의를 완료하면 알림을 보냅니다. 이는 관리자가 해당 대상에 대한 대체자로 구성한 소스 교육을 직접 완료했음을 의미합니다.

**강의/경로가 불완전으로 표시되었습니다.** 이 템플릿은 이전에 대안을 통해 완료하도록 되어 있던 강의 또는 학습 경로가 이제 불완전으로 표시되었음을 학습자에게 알립니다. 이는 관리자가 대체 관계를 제거하거나 수정하고 계정 수준에서 소급 미완료가 활성화된 경우 발생합니다.

**체크리스트 통과/체크리스트 실패:** 이러한 템플릿의 목표는 학습자에게 검토자가 평가했음을 알리는 것입니다. 학습자는 각 평가에 대해 이 전자 메일을 수신하므로 플랫폼으로 이동하여 조치를 취해야 하는지 확인할 수 있습니다.

1. 템플렛에 액세스하려면 왼쪽 창의 **[!UICONTROL 전자 메일 템플릿]**&#x200B;을 클릭합니다. 이 페이지에서 **[!UICONTROL 일반]**, **[!UICONTROL 학습 활동]** 및 **[!UICONTROL 알림 메시지 및 업데이트]**&#x200B;의 세 가지 범주 템플릿을 확인할 수 있습니다. 네 번째 탭인 **[!UICONTROL 설정]**&#x200B;에서 템플릿을 구성할 수 있습니다.

   ![](assets/categories-of-emailtemplates.png)

   *일반 탭 선택*

2. 특정 역할에 대한 전자 메일 템플릿을 대량으로 활성화 또는 비활성화하려면 줄임표(...)를 선택합니다. 열 머리글에서 **[!UICONTROL 전자 메일 받는 사람]**&#x200B;의 모든 해당 역할을 사용하도록 설정합니다.

   ![](assets/email-template-role.png)
   _역할 수준에서 모든 전자 메일 템플릿 보내기_

3. 역할에 대한 특정 전자 메일 템플릿을 활성화 또는 비활성화하려면 줄임표(...)를 선택합니다. 해당 템플릿의 경우 역할을 선택하거나 선택 해제합니다.
...
1. **[!UICONTROL 전자 메일 배너]** 옵션에서 전자 메일 배너를 사용자 정의합니다. **[!UICONTROL 배너 배경]**&#x200B;을 선택하여 배너의 색상을 변경합니다.
   ...
   **[!UICONTROL 사용자 정의 이미지]** 옵션을 선택하여 배너의 이미지를 사용자 정의할 수 있습니다. 변경 후 **[!UICONTROL 저장]**&#x200B;을 클릭합니다.
...
이는 **[!UICONTROL 방해 금지]** 옵션을 통해 실행할 수 있습니다. 방해 금지 옵션은 **[!UICONTROL 설정]** 탭에 있습니다. 이름, 전자 메일 ID 또는 고유 사용자 ID를 사용하여 이 목록에 사용자를 추가할 수 있습니다.
...
각 사용자에 대해 **차단된 전자 메일** 드롭다운 목록에서 사용자를 위해 차단할 전자 메일 유형을 선택합니다.
...
선택한 옵션에 대해 사용자가 전자 메일을 받는 것을 차단하려면 값 필드에 참을 입력합니다. 값에 false를 입력하면 차단된 사용자의 차단이 해제됩니다. 열이 비어 있으면 이전 선택 내용이 적용됩니다.
...
* 전자 메일 바닥글 편집. 바닥글 영역에는 계정 URL, 서명 및 사용자가 추가하는 모든 자유 형식 텍스트에 대한 사전 정의된 변수가 포함됩니다.
...
+++전자 메일 템플릿에서 도움말을 비활성화하려면 어떻게 해야 합니까?
