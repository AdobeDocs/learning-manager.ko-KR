...
1. 다음을 수행합니다. **[!UICONTROL 추가]** > **[!UICONTROL 새 위치]**.

1. **[!UICONTROL 강의실 위치]** 대화 상자에서 다음 세부 정보를 입력합니다.

   * 다음을 입력합니다. **[!UICONTROL 위치 이름]**. 고유한 이름을 사용합니다. 그렇지 않으면 Learning Manager에 오류 메시지가 표시됩니다.
   ...
   * 을 입력하고 **[!UICONTROL 위치 영역]**. 이 필드는 선택 사항입니다.
...
1. 선택 **[!UICONTROL 강의실 위치 편집]** 아이콘을 클릭합니다.

1. 강의실 위치를 수정하고 선택합니다. **[!UICONTROL 저장]**.

## CSV를 사용하여 강의실 추가

또는 강의실 정보가 포함된 CSV를 가져오기하여 하나 이상의 강의실 위치를 추가할 수 있습니다.

위치 **[!UICONTROL 관리 앱]** > **[!UICONTROL 설정]** > **[!UICONTROL 강의실 위치]** > **[!UICONTROL 추가]**&#x200B;을 클릭하고 **[!UICONTROL 대량 가져오기 위치]** 버튼을 클릭합니다. CSV 파일이 포함된 위치를 찾아 파일을 선택합니다.
...
강의실을 검색하려면 가상 강의실 강의를 선택한 다음 **[!UICONTROL 인스턴스]** > **[!UICONTROL 세션]**. 작성자 또는 책임자는 위치 이름을 입력하여 표시되기 시작하는 관련 결과를 확인할 수 있습니다. 그런 다음 표시된 결과에서 위치를 선택할 수 있습니다. 앞에 입력 결과에 위치가 표시되지 않더라도 사용자는 새 강의실 위치 이름을 추가할 수 있습니다. 참고: 세션 생성 작업 과정을 사용하여 생성된 위치 이름은 책임자가 생성한 위치 라이브러리에 추가되지 않습니다.
...
관리자 앱의 **[!UICONTROL 설정]** > **[!UICONTROL 일반]**, 관리자는 **[!UICONTROL 강사 관리]** 선택 사항입니다. 이 기능을 사용하면 강사로 할당된 사전 승인된 사용자만 세션 수행에 추가할 수 있습니다.

강사를 할당하려면 다음 단계를 따르십시오.

1. 다음으로 이동 **[!UICONTROL 시작하기]** 페이지 및 선택 **[!UICONTROL 사용자]** 왼쪽 창에.

1. 원하는 사용자를 선택합니다.

1. 다음을 선택하여 사용자에게 강사 역할을 할당합니다. **[!UICONTROL 액션]** > **[!UICONTROL 역할 할당]**.

### 세션 취소:

에 **[!UICONTROL 강의 인스턴스]** 페이지에서 관리자는 하나 이상의 세션을 취소할 수 있습니다. 세션이 취소되면 시스템에서 모든 세션 세부 정보를 제거하지만 인원 제한은 유지합니다.

또한 관리자는 다음을 수행할 수 있습니다.

* **[!UICONTROL 등록 보기]**: 각 세션의 등록 학습자 및 대기자 명단에 등록된 학습자에 대한 정보를 가져옵니다.
* **[!UICONTROL 학습자 등록 취소]**: 등록 상태를 변경하지 않고 취소된 세션이 있는 강의에서 학습자를 제거합니다.
* **[!UICONTROL 출석 관리]**: 세션이 취소되더라도 세션의 출석 표시를 합니다.
* **[!UICONTROL 강의 완료]**: 관리자는 세션이 취소된 경우에도 강의를 완료로 표시할 수 있습니다.
* **[!UICONTROL 스케줄 조정]**: 나중에 취소된 세션을 예약하고 예약 조정 중에 강사를 추가합니다.

참고: 취소한 후에도 학습자는 교육 인스턴스에 등록된 상태를 유지합니다. 등록 확인, 대기자 명단, 관리자 승인 대기 등의 등록 상태는 변경되지 않습니다. 책임자가 나중에 취소된 세션을 설정하고 다시 예약할 때 유용합니다.

## 작성자

관리자가 **[!UICONTROL 강사 관리]** 옵션, 작성자는 강의실 세션, 가상 강의실 세션, 체크리스트 및 파일 제출 모듈에서 강사 역할이 있는 사용자만 검색해서 추가할 수 있습니다.
...
따라서 책임자가 을 활성화하면 **[!UICONTROL 강사 관리]** 옵션을 선택하면 강사 역할이 있는 사용자만 강사로 추가할 수 있습니다.

>[!NOTE]
>
>세션 CSV 파일을 사용하여 세션을 마이그레이션하는 경우에는 해당되지 않습니다. 이 경우에는 강사 역할이 없는 사용자를 강사로 추가할 수 있습니다.

에 **[!UICONTROL 강의 인스턴스]** 페이지에서 작성자는 하나 이상의 세션을 취소할 수 있습니다. 세션이 취소되면 시스템에서 모든 세션 세부 정보를 제거하지만 인원 제한은 유지합니다.
