...
2. 강의 목록에서 필수 강의를 선택하고 **[!UICONTROL 강의 보기]**&#x200B;를 클릭합니다.

   ![](assets/view-course.png)

   *강의 보기*

3. 인스턴스를 생성하려면 왼쪽 창의 **[!UICONTROL 인스턴스]**&#x200B;를 클릭합니다. 모든 강의에는 기본적으로 인스턴스가 있습니다. 기본 인스턴스를 수정하거나 인스턴스를 추가할 수 있습니다. 이 강의 인스턴스는 삭제할 수 없습니다.
...
5. 인스턴스의 속성을 입력합니다.
   ...
   * **시작 날짜:** 인스턴스의 시작 날짜입니다. 두 개의 미리 알림 전자 메일은 선택된 두 개의 날짜에 이 시작 날짜 이전에 학습자에게 전송될 수 있습니다. 시작 날짜는 완료 날짜보다 이전 날짜여야 합니다. 시작 날짜를 선택하면 이 날짜는 인스턴스의 **알림 경고** 섹션에 있는 달력에서 파란색으로 강조 표시됩니다.
...
4. **전자 메일 서식 파일:** 드롭다운 목록에서 서식 파일을 선택합니다.
...
>[!NOTE]
>
>강의에 여러 인스턴스가 있는 경우, 엑셀에 있는 학습자 목록은 각 탭에 개별적으로 제공됩니다. 학습자 목록은 학습자 이름, 상태, 선택 조건으로 구성됩니다. 학습자 상태는 **시작되지 않음**, **진행 중** 또는 **완료** 중 하나입니다.

### 승인 보류 상태의 학습자 내보내기

책임자, 관리자 및 사용자 정의 책임자는 등록 승인 보류 상태의 학습자 데이터를 내보낼 수 있습니다. **강의 > 학습자** 탭에서 드롭다운 목록의 동작을 클릭하여 데이터를 내보낼 수 있습니다.

관리자 승인 강의에 등록됨/승인 보류 중인 학습자가 없는 경우 옵션이 표시되고 빈 보고서가 생성됩니다. 또한 학습자가 승인 보류 상태, 등록됨 상태, 보류 중 상태, 등록 취소됨 상태일 때도 내보낼 수 있습니다.

보고서에는 승인 보류 중인 활성, 삭제됨, 일시 중단됨 상태의 사용자 데이터가 포함됩니다. 또한 보고서에는 승인 보류 상태의 내/외부 사용자 데이터가 포함됩니다.
...
## 학습자 등록, 출석 및 완료 일괄 관리 {#bulk-enrollment}
...
등록, 출석 및 완료에 동일한 CSV 파일 형식을 사용할 수 있습니다. &quot;이메일&quot; 열 아래에 학습자 이메일 ID를 입력하고 파일을 작업에 따른 이름(예: bulk_enrollment.csv, bulk_attendance.csv 또는 bulk_completion.csv)으로 저장하십시오. CSV 형식만 지원됩니다. UTF-8 형식은 지원되지 않습니다. [샘플 csv](/help/migrated/administrators/feature-summary/assets/Sample-Bulk-Action-CSV.xlsx)를 다운로드합니다.

### CSV를 사용하여 학습자 일괄 등록

책임자는 한 번에 하나씩 학습자를 추가하는 대신 CSV 파일을 업로드하여 최대 100,000명의 사용자를 동시에 등록할 수 있습니다. 파일에는 등록할 학습자의 전자 메일 주소가 있는 **userEmail** 레이블이 지정된 열이 포함되어야 합니다.

CSV를 사용하여 대량 학습자를 등록하려면:

1. 책임자로 로그인합니다.
2. **[!UICONTROL 강의]** 섹션에서 강의를 선택합니다.
3. **[!UICONTROL 강의 개요]** 페이지에서 **[!UICONTROL 학습자]**&#x200B;를 선택합니다.
4. **[!UICONTROL 등록]**&#x200B;을 선택한 다음 **[!UICONTROL CSV 업로드]**&#x200B;를 선택합니다.\
   ![](assets/upload-a-csv-learners.png)
   _CSV 업로드를 사용하여 학습자 등록_
5. CSV를 업로드하고 **[!UICONTROL 진행]**&#x200B;을 선택합니다.

CSV 파일에는 &quot;사용자 이메일&quot;이라는 레이블이 지정된 열이 포함됩니다. 이 열에 사용자의 이메일 주소를 입력합니다.

### 대량으로 강의 완료 표시

책임자는 전자 메일 주소와 함께 CSV 파일을 업로드하여 한 번에 여러 학습자의 강의 완료를 빠르게 표시할 수 있습니다. 이렇게 하면 각 학습자를 개별적으로 업데이트할 때보다 시간을 절약할 수 있습니다. CSV의 사용자 이메일 열에는 업데이트할 학습자가 표시됩니다. 한 번에 최대 10,000명의 학습자를 완료로 표시할 수 있습니다.

대량 완료를 표시하려면

1. **[!UICONTROL 강의]** 섹션에서 강의를 선택합니다.
2. **[!UICONTROL 강의 개요]** 페이지에서 **[!UICONTROL 학습자]**&#x200B;를 선택합니다.
3. **[!UICONTROL 작업]**&#x200B;을 선택한 다음 **[!UICONTROL 완료 표시]**&#x200B;를 선택합니다.
4. **[!UICONTROL 대량]**&#x200B;을 선택합니다.
5. 강의를 완료한 학습자를 나열하는 사용자 이메일 열이 있는 CSV 파일을 업로드합니다.

   ![](assets/bulk-completion.png)
   _CSV를 사용하여 대량 완료 표시_

### 대량으로 출석 표시

책임자는 일괄 출석 기능을 사용하여 한 번에 여러 학습자의 출석을 표시할 수 있습니다. 책임자는 각 학습자의 출석을 개별적으로 업데이트하는 대신 학습자의 전자 메일 주소가 포함된 CSV 파일을 업로드할 수 있습니다. CSV의 사용자 이메일 열에는 기록할 학습자의 출석이 표시됩니다. 이 프로세스는 한 번의 업로드로 최대 10,000명의 학습자를 처리할 수 있어 출석 표시의 속도가 빨라지고 효율성이 높아집니다.

대량 출석을 표시하려면

1. **[!UICONTROL 강의]** 섹션에서 강의를 선택합니다.
2. **[!UICONTROL 강의 개요]** 페이지에서 **[!UICONTROL 출석 및 점수]**&#x200B;를 선택합니다.
3. **[!UICONTROL 작업]**&#x200B;을 선택한 다음 **[!UICONTROL 대량 출석 표시]**&#x200B;를 선택합니다.
4. 출석을 업데이트하려는 학습자의 이메일 주소가 포함된 userEmail 열이 포함된 CSV 파일을 업로드합니다.

   ![](assets/mark-bulk-attendance.png)
   _CSV를 사용하여 대량 출석 표시_

>[!NOTE]
>
>CSV를 사용하여 최대 10,000명의 사용자에 대한 출석을 일괄 표시할 수 있습니다.

### 일반적인 CSV 업로드 오류

* CSV의 학습자 전자 메일은 Adobe Learning Manager 사용자 디렉터리에 없습니다.
* 파일 형식이 잘못되었습니다.
* 파일에 추가 열 또는 잘못된 데이터가 있습니다.

![](assets/error-bulk.png)
_오류 알림_

쉽게 식별할 수 있도록 행 수준에서 실패한 사용자 관련 오류를 나열하는 CSV 파일을 다운로드하고 볼 수 있습니다.
...
Adobe Learning Manager을 사용하면 책임자가 강의의 모든 인스턴스에 대해 대기자 명단에 등록된 학습자 목록을 다운로드할 수 있습니다. 관리자는 **[!UICONTROL 강의 개요]** 페이지의 대기자 명단 섹션에서 이 보고서에 액세스할 수 있습니다.
...
>[!NOTE]
>
>학습 프로그램 인스턴스에서 이러한 설정을 활성화하는 것으로는 충분하지 않습니다. 학습 프로그램에서 각 과정의 과정 인스턴스 레벨에서도 이 설정을 활성화해야 합니다.

인스턴스 기본 페이지에서 **[!UICONTROL 모든 질문을 필수로 설정]**&#x200B;을 활성화하면 그후에 생성된 모든 새로운 인스턴스는 이러한 설정을 상속합니다.
...
Learning Manager의 이 릴리스에서 책임자는 학습 프로그램에 포함된 모든 과정에 관한 L1 피드백을 보낼 수 있습니다. 책임자는 L1 피드백이 과정 인스턴스 레벨에서 모든 과정에 대해 활성화되어 있는지 확인해야 합니다.
...
1. **[!UICONTROL 각 과정에 대해 활성화]** 옵션을 활성화합니다.
   ...
   *강의 피드백 활성화*
   ...
   학습 프로그램 레벨에서 이 전환만 활성화하면 이 프로그램 내의 과정에 대해 L1 피드백이 시작되지 않습니다. L1 피드백을 활성화하려면 학습 프로그램에서 각 과정으로 이동하여 L1 피드백 전환을 활성화합니다.

   ![](assets/l1-reaction-feedback.png)

   *각 과정에 대해 L1 피드백 사용*

   L1 피드백이 모든 과정에 대해 활성화되었지만 학습 프로그램 인스턴스에서 비활성화되었다면 L1 피드백이 과정에 대해 시작되지 않습니다.

### 언어별 퀴즈 보고서

퀴즈 보고서로 학습 프로그램 또는 강의를 끝낸 후 학습자의 성과를 평가할 수 있습니다.

Learning Manager에서는 현재 13개의 인터페이스 언어 및 32개의 콘텐츠 언어를 제공하여 학습을 용이하게 합니다. 이 옵션은 학습자 친화적이며 글로벌 학습자 지원에 편리함을 제공하지만 관리자가 다양한 로케일에서 시도한 보고서를 가져오기는 어렵습니다.

강의가 다중 언어로 제공되는 경우 퀴즈 보고서에는 다른 언어로 된 데이터가 포함됩니다. 지금까지 책임자가 생성한 보고서의 경우 퀴즈를 진행한 언어와 상관없이 아래에 하나씩 표시되었습니다. **예를 들어** 사용자가 네덜란드어로 퀴즈를 진행한 경우 책임자는 한 번에 네덜란드어를 사용하는 사용자가 진행한 퀴즈 보고서만 볼 수 있습니다. 인터페이스 언어로 영어를 선택한 책임자의 경우 진행한 로케일에 상관없이 모든 사용자의 보고서를 한 번에 확인할 수 없었습니다.
...
책임자는 계정 레벨에서 이 설정을 사용하도록 지정하여 새로 생성된 과정 및 학습 프로그램에 대해 L1 피드백을 활성화할 수 있습니다. 하지만 이 설정을 지정하면 기존의 과정 및 학습 프로그램에는 영향을 주지 않습니다.

활성화되면 모든 새 교육 및 새 인스턴스에 피드백이 기본적으로 활성화됩니다. 작성자/책임자가 인스턴스를 방문하는 경우 인스턴스는 기본값이 되고 수동으로 끈 다음 인식됩니다.
...
오른쪽 위 모서리에서 **[!UICONTROL 편집]**&#x200B;을 클릭하고 옵션을 전환하여 L1 피드백을 활성화합니다.
...
L3 피드백을 활성화하면 아래의 스냅샷과 같이 옵션을 볼 수 있습니다.
...
1. 책임자로 로그인한 뒤 왼쪽 창의 **[!UICONTROL 설정]**&#x200B;을 클릭한 다음 **[!UICONTROL 피드백]**&#x200B;을 클릭합니다.\
   피드백 설정 페이지에는 두 개의 탭이 있습니다. **[!UICONTROL L1 피드백]** 및 **[!UICONTROL L3 피드백]**.\
   **[!UICONTROL L1 피드백]** 탭은 알림 메시지 설정이 있는 자가 진행식 및 강의실 강의에 대한 기본 **[!UICONTROL L1 피드백]** 질문 목록으로 구성되어 있습니다. **[!UICONTROL L3 피드백]** 탭에서는 L3 피드백 기본 명령문과 알림 메시지 설정을 볼 수 있습니다.
...
1. 페이지 오른쪽 상단 모서리에 있는 **[!UICONTROL 저장]**&#x200B;을 클릭합니다.
...
책임자는 사용 가능한 모든 강의의 목록을 볼 수 있습니다.   검색 및 필터 옵션이 있는 강의 목록을 보려면 왼쪽 창에서 **[!UICONTROL 강의]**&#x200B;를 클릭하십시오. 강의 축소판에서 각 강의의 강의 효과 진행률 또한 볼 수 있습니다.

>[!NOTE]
>
>학습자가 강의를 수료한 후 또는 게시 후 특정 강의를 지연하고 싶은 경우 강의를 중단할 수 있습니다. 게시됨 상태의 강의만 중단할 수 있습니다. 중단된 강의의 전체 목록은 **[!UICONTROL &#39;중단됨&#39;]** 탭을 클릭하면 볼 수 있습니다.
...
* **[!UICONTROL 게임화]**: 완료, 조기 완료 및 적시 완료를 위한 점수를 포함하는 게임화 설정을 구성합니다. 관리자는 계정 수준 설정을 선택하거나 이 인스턴스에 대한 게임화 지점을 사용자 정의할 수 있습니다.
...
책임자는 학습자가 강의, 학습 경로 또는 인증을 완료로 표시하면 댓글을 남길 수 있습니다. 이러한 주석은 규정 준수 및 감사 목적에 도움이 됩니다. 책임자는 한 번에 한 명의 학습자 또는 여러 명의 학습자에 대한 댓글을 손쉽게 추가할 수 있습니다.

### 완료 주석 추가

완료 주석을 추가하려면 다음 단계를 따르십시오.
...
책임자는 강의 모듈을 보면서 **[!UICONTROL 학습자로 미리 보기]** 옵션을 클릭하여 강의를 미리 보기할 수 있습니다.
...
1. 검색 필드를 사용할 수 있습니다. 오른쪽 상단 모서리에 표시된 검색 아이콘을 클릭합니다. 검색창이 나타납니다. 강의 이름 또는 강의/학습 프로그램과 관련된 키워드를 입력하여 강의를 찾을 수 있습니다. Captivate, C, Java 및 HTML 등 미리 정의된 태그로 검색할 수도 있습니다. 검색 필드 내에서 태그를 검색할 수 있습니다. 즉, 검색 필드에 입력을 하면 태그가 표시됩니다.
1. 필터를 사용하여 강의/학습 프로그램 목록을 필터링할 수 있습니다. 전체, 게시됨, 초안, 중단됨 등의 상태로 강의에 필터를 적용할 수 있습니다. 책임자 모드에서는 초안 필터가 표시되지 않습니다.

&#39;역량&#39;을 클릭하여 선택하면 역량 기준으로 검색할 수도 있습니다. 책임자는 4가지 방법으로 강의를 정렬하여 더욱더 쉽게 필요한 강의를 찾을 수 있습니다. 정렬 기준 을 클릭하고 알파벳 오름차순, 알파벳 내림차순, 강의 갱신일 또는 강의 효과를 선택합니다.
...
1. 게시되어 있는 강의 타일 중 하나에 마우스를 가져간 다음 &#39;학습자 등록&#39;을 클릭합니다.\
...
**[!UICONTROL 학습자 등록]** 대화 상자에서 **[!UICONTROL 고급 옵션]**&#x200B;을 확장합니다. **[!UICONTROL 추가 댓글]** 필드에 필요한 설명을 입력합니다.
...
학습자가 관심을 등록하는 방법을 알아보려면 [강의에 관심 등록](/help/migrated/learners/feature-summary/courses.md#register-interest-for-the-courses)을 확인하십시오.

책임자는 강의 개요 페이지에서 학습자의 관심 내용을 보고 관심 있는 학습자 보고서를 다운로드할 수 있습니다.

관심 있는 학습자 보고서를 다운로드하려면:

1. 관리자 권한으로 Adobe Learning Manager에 로그인합니다.
2. **[!UICONTROL 과정]**&#x200B;으로 이동하여 과정을 선택합니다.
3. **[!UICONTROL 관심 있는 학습자]**&#x200B;를 선택합니다.

   ![](assets/select-interested-learner.png)
   관리자 인터페이스의 _강의 개요 페이지에는 보고서를 보고 다운로드할 수 있는 관심 있는 학습자 섹션이 표시됩니다_
4. 작업 을 선택한 다음 보고서 내보내기 를 선택합니다.
관심 있는 학습자 목록이 포함된 보고서가 다운로드됩니다. 보고서에는 다음 열이 포함됩니다.
...
>[!NOTE]
>
>계정에 활성화된 경우 보고서에는 학습자의 UUID가 포함됩니다.

## 강의 사용 주기 {#courselifecycle}

일반적인 강의 사용 주기는 다음과 같습니다.

**초안** - 작성자가 강의 생성 및 저장을 완료하면 이 상태인 경우 학습자는 강의를 사용할 수 없습니다. 이 상태의 강의는 삭제할 수 있습니다.
...
**삭제됨** - Adobe Learning Manager 응용 프로그램에서 완전히 제거된 상태의 강의입니다. 강의는 오직 초안 또는 중단 상태일 때만 작성자가 삭제할 수 있습니다. 중단 상태인 강의도 삭제할 수 있습니다.
