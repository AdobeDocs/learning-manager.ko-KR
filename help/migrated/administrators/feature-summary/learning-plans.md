...
옵션으로 학습자가 강의를 수강하는 시기를 결정합니다. 이벤트 유형을 선택한 후 적절한 교육, 강의, 학습 프로그램 또는 인증을 선택합니다.
...
**2 - 새 사용자나 직원이 그룹에 가입하면 학습자가** 그룹에 추가됩니다.  드롭다운 목록에서 이 이벤트를 적용할 사용자 그룹을 입력하고 선택합니다. 다수의 그룹을 선택할 수 있습니다. 또한, 옵션을 선택하면 해당 그룹의 모든 기존 구성원에게 이 이벤트를 할당할 수도 있습니다.

![](assets/learner-gets-addedtoagroup.png)

이 학습 계획은 ***사용자 정의- 그룹*** 사용자에 맞게 설계되었습니다. 필드에 그룹 이름을 입력하고 타이프 어헤드 검색을 사용하여 그룹을 선택합니다.

**3 - 학습자가 그룹에서 제거됨:** 사용자 또는 학습자가 그룹에서 제거되면 이벤트가 트리거됩니다. 이 이벤트를 적용할 수 있는 드롭다운 목록의 사용자 그룹을 입력하고 선택합니다. 다수의 그룹을 선택할 수 있습니다.

![](assets/learner-removed-from-group.png)


**4 - 학습자가 강의/학습 경로/인증을 완료합니다.** 학습자가 강의, 학습 프로그램 등과 같은 학습 개체를 완료하면 이벤트가 트리거됩니다. 이 이벤트가 적용될 학습 객체를 선택합니다. 이벤트 완료 상태를 선택합니다. 또한, 학습자가 속한 사용자 그룹을 선택할 수도 있습니다. 학습 객체를 완료한 다음에 이 이벤트가 시작하는 날짜 수를 입력합니다. 학습 객체를 이미 완료한 기존 사용자에게 이벤트를 부여하려면 이 옵션을 선택합니다.

![](assets/learner-completealearningobject.png)

**5 - 학습자가 강의 모듈에 실패했습니다.** 학습자가 강의, 학습 프로그램 등의 학습 개체에 실패하면 이벤트가 트리거됩니다. 이 이벤트가 적용될 학습 객체를 선택합니다. 또한, 학습자가 속한 사용자 그룹을 선택할 수도 있습니다.
...
1. **[!UICONTROL 교육에서 등록 취소]**&#x200B;로 동작을 선택합니다.
...
1. 하나 이상의 사용자 그룹을 추가합니다. 여러 그룹이 선택된 경우 학습자가 지정한 교육을 완료하면 계획이 트리거됩니다.
1. **[!UICONTROL 교육에서 등록 취소]**&#x200B;로 동작을 선택합니다.
...
기본적으로 생성한 모든 새 학습 계획은 비활성화 상태입니다. 학습자가 할당되도록 계획을 활성화해야 합니다. **[!UICONTROL 현재 학습자]** 확인란을 사용하면 이벤트가 자체적으로 활성화됩니다.
...
1. 페이지의 오른쪽 상단 모서리에서 **[!UICONTROL 작업]** > **[!UICONTROL 활성화]**&#x200B;를 클릭합니다. 이렇게 하면 학습 계획이 활성화됩니다.
...
1. 신입 사원의 온보딩을 위해 자동 등록을 구성하도록 Learning Manager를 어떻게 설정합니까?
   ...
   **[!UICONTROL 다음과 같은 경우 발생]** 드롭다운 목록에서 **[!UICONTROL 새 학습자 추가]** 옵션을 선택합니다. 그런 다음 학습자의 학습 목표, 인스턴스 및 완료 날짜를 할당합니다. 책임자 및 작성자는 자동 등록 이벤트를 만들 수 있습니다. 이벤트를 작성한 후 활성화합니다.

1. 강의실 및 가상 강의실 강의에 대한 학습 계획/자동 등록은 어떻게 설정합니까?

   필수 세션 세부 사항으로 강의 인스턴스를 설정하는 것이 좋습니다. 그런 다음 학습 계획을 설정하고 이미 작성된 강의 인스턴스에 매핑합니다.

1. 특정 학습 계획에 등록한 학습자 목록을 보려면 어떻게 합니까?
