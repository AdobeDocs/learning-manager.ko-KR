...
* 모든 학습자 대상 모듈(SCORM, PDF, HTML 등) 다중 콘텐츠 언어로 제공될 수 있어 학습자가 원하는 언어를 선택할 수 있습니다.
...
이 변경 후 동일한 열에 - 제출 모듈에 대한 제출 의견 이 포함됩니다.
...
* 이 열에는 위에서 설명한 비헤이비어에 맞게 정렬된 검토자의 주석이 포함됩니다.
   * 제출 모듈에 대한 제출 의견
   * 체크리스트 모듈에 대한 평가 설명
...
| 시나리오 | 영향 | 조치 사항 |
|----------|--------|----------------|
| 열 이름(예: &quot;제출 의견&quot;)으로 Admin LT의 필드를 식별합니다. | 열 헤더가 검토자의 주석으로 변경됩니다. | 예. 검토자의 주석을 사용하기 위해 제출 주석을 참조하는 매핑 또는 논리를 업데이트합니다. |
