...
Adobe Learning Manager 사용자 인터페이스가 보다 깔끔하고 최신 환경을 제공하기 위해 몇 가지 업데이트되었습니다. 책임자 및 작성자 역할의 랜딩 페이지가 개선되었으며 모든 역할에 대한 UI 테마 업데이트가 진행되었습니다. 그러나 메뉴, 버튼 또는 링크의 위치는 변경되지 않았으며 이전과 동일한 위치에서 찾으실 수 있습니다.
...
자세한 내용은 [퀴즈 만들기](/help/migrated/authors/feature-summary/content-library.md)를 참조하세요.
...
이번 릴리스에서는 지정된 기간 내에 알림을 다운로드할 수 있는 날짜 필터를 추가했습니다.  단, 지난 6개월 동안의 보고서만 다운로드할 수 있습니다.
...
**[!UICONTROL 보고서]** > **[!UICONTROL 사용자 지정 보고서]** > **[!UICONTROL 기록 보고서]** > **[!UICONTROL 강의 액세스 보고서]**&#x200B;에서 보고서를 다운로드할 수 있습니다.
...
보고서에 대한 자세한 내용은 Adobe Learning Manager의 [보고서](/help/migrated/administrators/feature-summary/reports.md)를 참조하세요.
...
이번 릴리스에서는 삭제된 게시물에 더 이상 액세스할 수 없도록 합니다. /posts/{id} API에서 ID를 지정했는데 게시물의 ID를 더 이상 사용할 수 없는 경우 API에 &quot;지정된 리소스에 대해 게시물을 찾을 수 없습니다&quot;라는 메시지가 표시됩니다.
...
`instances.loResources.resources` 끝점이 퀴즈와 함께 `ResourceContentType`을(를) 반환합니다.
...
현재 및 이전 릴리스의 Learning Manager 웹 앱 및 장치 앱과 관련된 정보는 [출시 안내](release-note/release-notes.md)를 참조하십시오.
...
* Adobe Learning Manager의 이전 릴리스에서는 강의, 인증 또는 학습 계획이 삭제되고 알림이 표시되면 알림을 방문하여 강의, 인증 또는 학습 계획에 계속 액세스할 수 있었습니다. 이번 릴리스에서는 삭제된 게시물에 더 이상 액세스할 수 없도록 합니다. /posts/{id} API에서 ID를 지정했는데 게시물의 ID를 더 이상 사용할 수 없는 경우 API에 &quot;지정된 리소스에 대해 게시물을 찾을 수 없습니다&quot;라는 메시지가 표시됩니다.
