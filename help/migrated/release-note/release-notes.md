...
보기 [Adobe Learning Manager의 새로운 기능](/help/migrated/whats-new.md) 를 참조하십시오.
+++
...
* 책임자에게 문의하는 방법에 대한 경고 팝업에는 다른 이메일 주소도 표시됩니다.
...
* Adobe Connect 커넥터에서 CSRF 토큰을 보내려면 API가 필요합니다. 자세한 내용은 Adobe Connect 계정 보안 향상 을 참조하십시오.
...
* 강사 앱에서 세션에서 대기자 명단 제한을 0으로 설정하고 세션을 저장하면 사용자 인터페이스에서 0이 아닌 &quot;해당되지 않음&quot;이 잘못 표시됩니다.
* Power BI 커넥터에 대한 학습자 성적 증명서를 생성할 때 교육 또는 모듈 지속 시간(분) 열에 특정 강의실 또는 VC 모듈에 대한 null 값이 표시됩니다.
* 인스턴스 또는 여러 인스턴스에서 학습자를 대상으로 강의를 완료로 표시하면 현재 인스턴스나 인스턴스의 학습자뿐만 아니라 강의의 모든 학습자가 완료로 표시됩니다.
+++
...
작성자는 이제 &quot;관리자&quot; 및 &quot;스토어 관리자&quot;를 체크리스트의 관찰자로 선택할 수 있습니다. 관리자는 강사로 역할을 전환할 필요 없이 관리자 인터페이스 내에서 체크리스트를 보고 완료할 수 있습니다. 관리자에게 체크리스트가 할당되면 관리자에게 알림이 전송됩니다.
...
Adobe Learning Manager는 이제 &quot;하이브리드&quot; 강사 Led 교육(ILT) 세션을 생성하는 기능을 지원합니다. 가상 ILT 세션은 학습자가 해당 위치에서 사용할 수 있는 경우에도 직접 세션에 참석할 수 있도록 위치 정보 옵션을 사용하여 생성할 수 있습니다.

**강의실 및 가상 ILT에 대한 진행 상황 추적 개선**

강의실 및 가상 ILT 모듈은 이제 학습자의 출석 상태와 함께 퀴즈 상태(합격 또는 불합격)를 보고할 수 있는 기능을 제공합니다. 따라서, 출석과 퀴즈 성공 모두 학습자 진도를 결정하는 데 고려될 수 있습니다.
...
* 강의가 &quot;UNDER_CONSTRUCTION&quot; 상태일 때 사용자 정의 작성자는 모듈을 미리 볼 수 없습니다. 응답에는 오류 404가 표시됩니다.
* 강의 제목이 특정 문자 제한을 초과할 경우 작성자 앱의 강의/추가 페이지에 있는 강의 제목이 오버플로됩니다.

**작성자**

* 작성자 앱에서 강의의 제목(긴 경우)이 강의를 만드는 동안 페이지 경계를 초과합니다.
* 선택한 작성자가 없더라도 강의가 추가되는 경우가 있습니다.
...
* 특정 유형의 게시물의 경우, 게시 후 소셜 게시판이 예상대로 열리지 않습니다.
...
* 학습자가 강의실/VC 강의가 포함된 학습 프로그램에서 등록 취소할 때 취소 전자 메일이 수신되지 않습니다.

**작업 지원**

* 작업 지원 위젯에 강의 이름이 표시되지 않습니다.

**게시 중**

* Adobe Captivate에 추가된 모듈 설명은 모듈이 ALM에 게시될 때 Learning Manager에 표시되지 않습니다.
...
* 사용자 csv 파일의 열이 외부 사용자 활성 필드와 이름이 같으면 csv 업로드가 실패합니다.

**API 관련 수정 사항**

* learningObjects 응답에서 bookmark 특성이 없습니다.
...
iPhone 14에서 앱의 활동 탭에서 알림을 클릭하면 아무 작업도 수행되지 않습니다.
* MS Teams 앱에서 Learning Manager 알림(완료됨, 등록됨, 기한, 만료)에 활동 탭에 상태 및 강의 이름이 표시되지 않습니다.
* 통합 책임자가 MS Teams 앱을 승인하지 않으면 XML 콘텐츠가 포함된 팝업이 표시됩니다.
* 언어가 변경될 때 MS Teams의 Adobe Learning Manager 앱의 사용자 인터페이스 언어가 예상대로 변경되지 않는 경우가 있습니다.
* 포커스가 Iframe(홈 및 카탈로그 탭) 내에 있으면 첫 번째 알림과 상호 작용할 수 없습니다.

**Adobe Learning Manager 모바일 앱의 제한 사항**

* 오프라인 콘텐츠 보기.
* 카탈로그/내 학습 페이지의 표/목록 보기.
* 강의를 여러 번 수강하려는 경우.
* 강의 카드에 대한 등록 기한.
* iOS 디바이스에서 앱이 전경에 있을 때 푸시 알림이 표시되지 않습니다.
* 푸시 알림의 딥 링크는 의도한 랜딩 페이지로 리디렉션되지 않습니다.
* 관심 항목 등록 버튼을 클릭하면 웹으로 리디렉션됩니다.
* 소셜 학습에서 댓글을 달거나 답변하는 동안에는 파일을 첨부할 수 없습니다.
* linkedIn Learning에 로그인할 수 없습니다.
+++
...
대규모 계정에 대한 학습 계획 처리를 최적화했습니다. 이렇게 하면 검색 문제나 지연이 발생하지 않습니다.
+++
...
로케일 설정 관련 일부 문제로 인해 학습자 앱에서 사용자 및 사용자 그룹 검색이 실패합니다.
+++
...
학습자가 모든 교육 재시도 및 재방문 보고서를 볼 수 있습니다.

**모듈 재설정 API**

이제 관리자는 모듈 재설정 API를 사용하여 모듈을 재설정할 수 있습니다. 자세한 내용은 다음을 참조하세요. [Adobe Learning Manager API 참조](https://captivateprime.adobe.com/docs/primeapi/v2/).

**전자 메일 템플릿**

몇 가지 전자 메일 템플릿에 대해 이제 템플릿에 전제 조건을 추가할 수 있습니다.

**기타 변경 사항**

* 관리자가 승인한 과정을 전제 조건으로 추가할 수 있습니다.
* 학습 요약 대시보드를 새로 고칠 때의 성능이 개선되었습니다.
* 반송된 보고서를 보내기 전에 이메일 ID와 계정 ID가 확인됩니다.

### 이번 업데이트에서 수정된 버그

* 강의 개요 페이지에 중복된 작성자 이름이 표시됩니다.
* 계정 생성 페이지의 하이퍼링크가 오류 404로 이어졌습니다.
* 체코 로케일이 플레이어 설정에 예상대로 반영되지 않았습니다.
* 진행 중 및 시작되지 않은 학습자에 대해 스킬이 정의되지 않음으로 표시되는 경우가 있습니다.
...
Learning Connector에 연결된 경우 학습자가 필터 페이지에서 등록을 취소할 수 있다는 확인란이 새로 추가되었습니다. 자세한 내용은 다음을 참조하세요. [LinkedIn Learning 커넥터](/help/migrated/integration-admin/feature-summary/connectors.md).
...
* 다중 시도가 설정된 SCORM 콘텐츠가 있는 강의에서 학습자가 강의를 시도하면 재방문 버튼이 비활성화됩니다.
* 경우에 따라 학습자를 강의에 등록하고 전자 메일 감사 로그를 다운로드한 후 전자 메일이 전달되지만 로그에 나타나지 않습니다.
* 강사 일정 초대는 주제에 텍스트 강사를 포함해야 합니다.
* 교육 카드 아이콘은 강의 개요 페이지에 있는 관련 강의 추천 및 학습 경로 카드에 반영되지 않습니다.
* 학습자 홈페이지 설정에서 내가 저장한 섹션을 추가합니다.
* 특정 계정의 경우 Adobe ID가 필요한 계정에 대해 SSO 로그인을 요청하는 메시지가 사용자에게 표시됩니다.
* 일광 절약 시간제가 있는 시간대에서 &#39;start_time&#39; 필드는 실제 시작 날짜 및 시간의 시간 차이를 기반으로 계산되지 않고 현재 시간 차이를 기반으로 계산됩니다. 이로 인해 잘못된 시간대의 초대가 발생했습니다.
* 인증이 되풀이될 때마다 기본 강의의 사본이 데이터베이스에 내부적으로 생성됩니다. 그러면 예상 동작과 달리 검색 창에 해당 강의가 표시됩니다.
* CSV 업로드가 실패하면 전자 메일 알림을 받지 못합니다.
* 활성 필드의 이름이 길면 끌어서 놓을 때 이름이 사라집니다. 그런 다음 저장 버튼도 예상대로 작동하지 않습니다.
* 보고서의 첫 번째 사용자가 활동 등급 테이블에 주석이 null인 레코드가 있는 경우 강의의 출석 및 점수 페이지를 통해 세션 보고서를 내보내지 않습니다.
* 관리자 계정을 사용하여 배지를 검색하면 목록을 예상대로 정렬할 수 있습니다. 그러나 학습자에게 동일한 작업을 수행하면 결과가 정렬되지 않습니다.
* 검색 결과에서 강의를 선택한 다음 뒤로 버튼을 사용하여 검색 결과로 돌아가려고 하면 검색 결과가 사라집니다.
* 일부 사용자가 사용자 그룹에 세션의 강사로 추가되지 않았습니다.
...
* 이제 관리자는 학습자가 다음 모듈/강의를 시작하기 전에 완료 및 합격하는 것을 요청할 수 있습니다. 필수 구성 요소, 주문 강의 및 LP에 적용됩니다.

**버그 수정**

* 인도네시아 언어 문제(사이드바 및 바닥글에서 몰입형 모바일 경험)를 해결했습니다.
* 모듈 미리 보기와 관련된 몰입형 보기 수정 사항.
* 관리자 및 작성자의 강의 검색은 입력한 것과 다른 로케일의 결과를 반환합니다.
* 환영 이메일 템플릿에 대한 변경 사항이 편집 후에 저장되지 않았습니다.
* 다른 이메일 ID 및 Adobe ID를 가진 사용자는 모바일 앱에 로그인할 수 없습니다.
* Zoom/BJ VC 세션에 참여하는 동안 사용자를 잘못 식별했습니다.
+++
...
자세한 내용은 다음을 참조하세요. [Adobe Learning Manager 2022년 11월 릴리스의 새로운 기능](/help/migrated/whats-new-2022-november.md).

**참고:** Adobe Learning Manager의 2022년 11월 릴리스부터 Zoom이 중단됩니다 [2023년 6월까지 JWT 인증](https://marketplace.zoom.us/docs/guides/auth/jwt/). 따라서 JWT가 지원되는 Zoom 커넥터는 언급한 날짜까지 작동하지만 사용자가 서버 간 OAuth 앱을 생성하여 계정에서 해당 기능을 대체하는 것이 좋습니다. 모든 새 연결은 기본적으로 Zoom Oauth 인증을 거치게 됩니다.

### 이번 업데이트에서 수정된 버그

* 학습자가 모바일에서 10개 이상의 강의로 구성된 학습 프로그램에 액세스하려고 하면 오류 메시지가 나타납니다.
...
* 퀴즈 점수 그래프가 예상대로 표시되지 않는 경우가 있습니다. 그래프 크기를 조정하면 시작 부분에 공백이 표시됩니다. 또한 일부 질문이 표시되지 않거나 잘못된 데이터가 간헐적으로 표시됩니다.
+++
...
* 이제 iOS 모바일 앱의 로그인 문제가 수정되었습니다.
* 반송된 이메일의 문제를 수정했습니다.
* 강사는 학습자가 과제물을 제출하기 전에 잘못된 알림을 받았습니다.
* 강사는 학습자가 활동을 제출하지 않아도 전자 메일 알림을 받습니다.
* MS Teams 또는 Adobe Connect에서 VC 세션을 만든 후에 강사가 세션 초대를 받지 못했습니다.
* 학습 경로의 상태가 잘못되었습니다.
* 앱의 성능이 개선되었습니다.
+++
...
* 대형 보고서를 다운로드할 때의 성능이 향상되었습니다.
* 사용자의 이메일이 반송되면 관리자는 이메일 알림을 받습니다. 전자 메일에는 클릭하면 전자 메일이 반송된 사용자 목록의 CSV를 다운로드할 수 있는 링크가 포함되어 있습니다. 해당 관리자는 필요한 조치를 취할 수 있습니다.
   * 전자 메일이 반송되거나 삭제되면 전자 메일이 트리거됩니다.
   * 전자 메일은 목록에 추가된 모든 관리자에게 하루에 한 번 트리거됩니다.
   * 링크는 7일 후에 만료됩니다.
* 이미 통합된 Adobe Connect 계정을 다른 Learning Manager 계정과 통합하려고 할 때 오류 메시지가 표시됩니다.
+++
...
**2022년 7월에 Adobe Learning Manager 제품이 Adobe Learning Manager로 리브랜딩됩니다**. 이는 특정 비즈니스 우선순위에 따라 제품의 정렬을 보다 정확하게 반영하기 위해 실행된 전략적인 노력입니다.

제품 팀은 플랫폼 사용에 영향을 주지 않도록 모든 조치를 취하고 있습니다. 제품을 평소와 같이 계속 사용할 수 있습니다. 해당 플랫폼의 책임자는 7월의 특정 화면에서 새로운 브랜드 이름을 발견할 수도 있습니다.
...
기존의 모든 URL은 계속 작동합니다.

이 작업을 완료하려면 귀사의 IT 부서와 협력하십시오. 자세한 내용은 다음 주소로 문의하십시오. `learningmanagersupport@adobe.com`.
+++
...
Adobe Learning Manager 인도 인스턴스는 기능 성능에 있어 다른 인스턴스(EU와 US 지역)와 동일합니다. 몇 가지 기능은 인도 인스턴스에서 지원되지 않습니다. 그 기능은 다음과 같습니다.
...
차이점이 없습니다. 뭄바이의 사례는 다음과 같습니다 [AWS 미국](http://learningmanager.adobe.com/) 또는 [AWS](http://learningmanagereu.adobe.com/) 인스턴스. 인도에 호스팅된 인스턴스, 모든 학습 기록 및 사용자 데이터는 인도에 보관됩니다. 다음 기능은 인도 인스턴스에서 지원되지 않습니다.
...
* 이제 콘텐츠 언어(헝가리어 및 핀란드어)에서 전자 메일 템플릿을 사용할 수 있습니다.
+++
...
* 이전에 학습 계획과 연관성이 있었던 강의나 학습 경로를 삭제할 수 없었습니다. 이 문제가 수정되었습니다. 사용자는 이제 현재 학습 계획과 연관된 것이 아니라면 강의나 학습 경로를 삭제할 수 있습니다.
...
* 책임자 및 강사 앱의 출석 및 점수 탭에서 학습자에게 이메일을 보내는 옵션이 모든 작업 선택을 수행한 다음 선택 취소된 학습자를 제외하지 않았습니다. 그래서 Learning Manager에서 모든 학습자에게 전자 메일을 발송했습니다. 이 문제는 수정되었습니다.
...
* 학습자가 학습 계획을 통해 교육에 등록되면 학습 계획의 완료 날짜를 편집한 횟수만큼 완료 알림 전자 메일이 여러 번 전송되었습니다. 이 문제는 수정되었습니다.
...
* 소셜 학습 페이지에서 게시물을 생성할 때 마이크 버튼을 눌러도 오디오를 녹음하거나 오디오를 업로드할 수 없습니다. 이는 브라우저의 제한 사항입니다.
* iOS에서 H264 및 WMA 오디오 파일은 모바일 브라우저에서 지원되지 않습니다.
* 전자 메일 주소에 +가 있는 학습자는 진행 상황이 표시되지 않습니다. Microsoft Teams에서 VC 강의를 수강한 후입니다.
...
* 사용자 정의 FTP는 이제 sftp:// 및 http:// 외에도 https://을 지원합니다
* Exavault 커넥터는 이제 V2 API를 사용합니다.
* 경우에 따라 비디오 품질이 최적이 아니었습니다. 이 문제는 수정되었습니다.
...
* 스킬을 기준으로 학습 경로를 필터링할 수 없습니다.
* 책임자가 인스턴스를 생성하고 새 세션을 추가할 때 강사가 세션 초대 전자 메일을 받지 못하는 경우가 있습니다. 이 문제는 Zoom VC 강의에서 발생합니다.
+++
...
* 텍스트 내부 **완료 표시** 작업을 되돌릴 수 없음을 반영하도록 대화 상자가 업데이트됩니다.
...
자세한 내용은 다음을 참조하세요. [***학습 경로***](../administrators/feature-summary/learning-paths.md).
...
자세한 내용은 다음을 참조하세요. [***Learning Manager 2021년 10월 릴리스의 새로운 기능***](../whats-new.md).
...
* 삭제된 교육이 카탈로그에 계속 표시되는 경우가 있었습니다. 이 문제는 책임자 앱에서만 발생하였으며 지금은 수정되었습니다.
* 관리자 역할을 한 사용자에서 다른 사용자로 변경할 때 이전 사용자의 관리자 역할이 여전히 UI에 반영되는 경우가 있었습니다. 이 문제가 수정되었습니다. 이 문제는 내부 사용자에게만 나타나고, 외부 사용자에게는 나타나지 않았습니다.
* 사용자 csv를 통해 대규모 사용자 그룹을 가져올 때 가져오기에 실패하는 경우가 있었습니다. 이 문제는 수정되었습니다.
* 외부 인증서 생성 후 필수 강의를 추가하고 사용자를 등록한 경우 학습 성적 증명서에 외부 인증서의 완료 날짜가 표시되지 않습니다. 이 문제가 수정되었습니다.
* 인증서에 학습자의 현지화된 이름이 정상적으로 표시되지 않는 경우가 있었습니다. 이 문제가 수정되었습니다.
* 강사가 Zoom VC 세션 초대를 항상 받지 못하는 경우가 있었습니다. 이 문제가 수정되었습니다. 이제 강사가 필요한 메시지를 수신합니다.
* 강의 레벨 템플릿이 활성화되어 있지만 계정 레벨 템플릿은 비활성화된 경우 학습자가 세션 초대를 받지 못하는 경우가 있었습니다. 이 문제가 수정되었습니다.
* 특정 시간대에 전자 메일 알림이 예상보다 하루 늦게 전달되는 경우가 있었습니다. 이 문제가 수정되었습니다.
* 특정 전자 메일 템플릿을 비활성화한 경우 학습자에게 세션 알림 메일이 발송되지 않았습니다.
* BlueJeans 회의가 작성자, 관리자, BJ 회의 URL에 의해 업데이트되면, 사용할 수 없게 됩니다. 이 문제가 수정되었습니다.
* GET /LO API를 실행했을 때 학습 프로그램에 포함된 강의가 반환되지 않는 경우가 있습니다.
* 학습자가 이름에 공백이 있는 콘텐츠를 업로드하려고 하면 내부 서버 오류가 발생합니다.
* 영어 이외의 로케일로 학습자용 배지 PDF을 생성할 때 배지 템플릿에 형식 문제가 발생했습니다. 이러한 문제는 수정되었습니다.
+++
...
* 모바일 브라우저에서 다음 항목으로 이동할 수 있는 딥 링크가 구현되었습니다.
   ...
   * 모든 게시판
   * 공개 게시판 및 게시물
   * 액세스 권한이 있는 비공개 게시판 및 게시물
   * 액세스 권한이 없는 비공개 게시판 및 게시물
   * 제한된 게시판 및 게시물
   * 게시물 댓글
   * 댓글에 답글 달기
   * 소셜 사용자 프로필

* 사용자 정의 도메인을 사용하는 계정의 경우 학습자 앱에 파비콘이 표시되지 않습니다.
* AEM에서 Learning Manager 구성 요소가 다른 구성 요소의 구성을 삭제합니다.
* AEM 구성 요소에 대한 도움말 페이지가 잘못된 위치로 리디렉션합니다.
...
* 내부 사용자가 받는 환영 전자 메일도 외부 사용자가 받습니다.
* AEM의 페이지 템플릿에 Learning Manager 위젯을 추가합니다.
* 강의를 제거한 경우 인증서를 다시 게시할 수 없습니다.
* 학습자에게 세션 세부 정보가 포함된 메일이 발송되지 않습니다.
+++

+++업데이트 67 - Azure로 업데이트

이 업데이트에는 Azure의 새 인스턴스가 도입됩니다.
...
자세한 내용은 다음을 참조하세요. [***Learning Manager 2021년 8월 릴리스의 새로운 기능 및 변경된 기능***](../whats-new.md).
...
* 학습자를 두 개의 사용자 그룹에 추가하고 학습 계획을 추가하면 학습자가 동일한 강의의 다른 인스턴스에 등록됩니다.
* 등록을 마치고 강의를 시작해도 강의가 정상적으로 재생되지 않는 경우가 있습니다.
* 강의 설명에 HTML 태그가 표시되며, 이제 수정되었습니다.
...
* 사용자가 이전에 대문자로 값을 추가한 경우에도 활성 필드 값이 사용자 편집 대화 상자에서 소문자로 표시됩니다.
* 책임자와 관리자가 승인 보류 상태의 강의를 볼 수 있습니다. 이를 통해 관리자는 직원의 학습 및 교육을 추적할 수 있고, Learning Manager 책임자는 필요에 따라 강의 등록을 승인할 수 있습니다.
...
* Android 및 iPhone 장치에서 학습자가 강의 모듈을 무작위로 실행할 수 없습니다. 승인되지 않은 오류 401이 생성됩니다.
* 학습자는 QR코드를 두 개까지 스캔할 수 있으나 세 번째 QR 코드를 스캔하면 오류 메시지가 표시됩니다.
* 일부 Android 및 iOS 장치에서 다운로드한 일부 강의에서 파일이 예상대로 열리지 않습니다.
* 작업 지원을 열려고 하면 오류 메시지가 표시됩니다.
* 학습 프로그램이 오프라인으로 사용되는 경우 장치 앱이 예기치 않게 작동합니다.
* 학습자가 다시 온라인으로 돌아가 앱을 열면 앱이 시작 화면에서 멈춥니다.
...
* 활성 필드에 &#39;/&#39; 문자가 있는 경우 user.csv를 다운로드하는 작업을 생성한 후에 작업의 상태가 &quot;제출됨&quot;에서 &quot;완료됨&quot;으로 변경되지 않습니다.
* 주문한 모듈은 순서를 따르지 않습니다.
* 외부 작성자가 삭제되면 작성자가 만든 강의는 더 이상 사용할 수 없습니다.
* 두 개 이상의 스킬로 학습 객체를 검색하면 예기치 않은 결과가 발생합니다.
+++
...
* 계정이 SP 로그인을 시작하면 앱에 로그인할 수 없습니다.
...
* 파일 제출에 대한 API가 EU 클러스터에서 예상대로 작동하지 않습니다.
+++
...
* 보고서를 만든 후에는 중단된 강의를 검색할 수 없습니다.
...
* 인증 개요 페이지에서 작성자가 추가한 기간은 외부 인증에 표시되지 않습니다.
* 옵션 추가 **모두** 스킬 필터로 이동합니다.
* 학습자가 여러 개의 다이제스트 전자 메일을 수신하고 있습니다.
* 선택한 행 수가 페이지에 예상한대로 반영되지 않습니다.

**AEM 구성 요소**

* 페이지를 새로 고친 후 위젯이 예상대로 업데이트되지 않습니다.

**로컬라이제이션**

* 일부 독일어 문자열은 예상대로 현지화되지 않습니다.
* 학습자가 인터페이스 및 콘텐츠 언어를 선택하지 않은 경우 번역은 영어로 기본 설정됩니다.

**인증**

* 모듈의 주문은 필수 구성 요소가 적용되지 않으면 우회될 수 있습니다.

**브라우저**

* 작성자, 관리자 또는 학습자 앱은 IE 11에서 예상대로 표시되지 않습니다.
...
* 콘텐츠 평가판 앱 강의가 예상대로 작동하지 않습니다.

**플레이어**

* 플레이어는 위젯이 있는 공간에서만 로드됩니다.
* Captivate 모듈 내의 비디오가 예상대로 재생되지 않습니다.

**커넥터**

* 경우에 따라 파일이 FTP/Box 커넥터에서 삭제될 수 있습니다.
* 파일이 동일한 이름으로 업데이트되면 ftp에서 삭제됩니다.
* BlueJeans 이벤트는 이벤트 수가 100보다 큰 페이지네이션을 지원합니다.
...
Captivate Learning Manager 모바일 앱 업데이트 3.3에서는 발행인 및 AI 기반 교육 권장 사항을 지원하는 새로운 홈페이지를 소개합니다. 이 홈페이지는 새로운 몰입형 레이아웃 옵션으로 구성된 모든 계정에서 사용할 수 있습니다. 클래식 레이아웃으로 구성된 계정의 경우 계속해서 클래식/기존 홈페이지가 표시됩니다. 홈 페이지에서 어떠한 변경 사항도 관찰해서는 안 됩니다.
...
이번 업데이트를 통해 학습자는 PDF 및 이미지 형식으로 배지를 다운로드할 수 있습니다.
...
자세한 내용은 의 새로운 기능을 참조하십시오. [Learning Manager 2021년 2월 업데이트](../whats-new.md).

### 이번 업데이트에서 수정된 버그 {#bug-fixes}
...
* 의 도움말 URL이 잘못되었습니다. **새 콘텐츠 추가** 페이지. 올바른 URL이 업데이트되었습니다.
...
* SCORM-2004의 특정 유형 콘텐츠에 대해 다시 시작 시나리오가 작동하지 않았습니다. 따라서 학습자는 중단한 지점으로 이동해야 했습니다. 이 문제가 수정되었습니다. 콘텐츠는 이전에 사용했던 지점에서부터 다시 시작됩니다.
* 강의에 등록한 후, 콘텐츠가 재생되지 않는 경우가 있습니다. 이 문제는 수정되었습니다.

**등록 취소**

* 등록 보고서에는 강의/인증 등록을 취소한 학습자가 더 많더라도 그 중 20명만 나열됩니다. 이 문제는 수정되었습니다.
* 때에 따라 등록 보고서에서 등록이 취소된 학습자 목록을 내보내는 데 문제가 발생했습니다. 이 문제가 수정되었습니다.
...
* 책임자가 전자 메일 템플릿에서 전자 메일 서명을 편집하고 여러 줄을 추가할 때, 관리자 인터페이스에서만 html 태그를 확인했었습니다. 이 문제는 수정되었습니다.
...
자세한 내용은 다음을 참조하세요. [***BlueJeans 이벤트 커넥터***](../integration-admin/feature-summary/connectors.md#bj-events).
...
* 대시보드에서 다운로드한 보고서가 대시보드 보고서에 대해 받은 구독 이메일과 다릅니다.
* 학습자 성적 증명서에 반복 인증에 대해 업데이트된 데이터가 없습니다.
* 강의를 시작한 후 강의 시간이 초과되면 시도 횟수가 예상대로 표시되지 않습니다. 또한 강의를 여러 번 시도할 때 비어 있는 화면이 나타나는 경우가 있습니다.
...
### linkedIn Learning 과정 수강 {#takecoursesfromlinkedinlearning}
...
자세한 내용은 다음 위치에서 강의 수강 을 참조하십시오. [***LinkedIn Learning***](../learners/feature-summary/ipad-android-tablet-users.md#linkedin).

### 관리자 등록을 위한 푸시 알림 {#pushnotificationforadminenrollments}

책임자가 학습자를 교육에 등록하면 학습자는 등록에 대한 알림을 받게 됩니다.
...
### 사용자 인터페이스 개선 사항 {#userinterfaceenhancements}
...
* 이전 버전의 Learning Manager에서 학습자 성적 증명서 열에는 25/100 형식으로 점수 및 최대 점수를 제공하는 데 사용되는 퀴즈 점수 및 최고 퀴즈 점수가 나열되어 있습니다. 더 나은 가독성과 분석을 지원하기 위해 퀴즈 점수도 이제 별도의 열로 내보냅니다. **Quiz_score, Quiz_score_max, Highest_Quiz_score 및 Highest_Quiz_score_max**. 이를 통해 책임자는 빠른 계산과 분석을 만들 수 있습니다.

### 이번 업데이트에서 수정된 버그 {#bug-fixes-2}

**커넥터**

* 학습자는 두 명의 다른 작성자가 만든 두 개의 다른 회의에 동시에 참여할 수 없습니다.
* Adobe Connect 카드에서 [연결 관리] 옵션을 클릭하면 FTP 연결 페이지로 이동합니다.
...
* 학습자 카탈로그 페이지에서 메모별로 강의를 검색하려고 하면 검색 결과가 나타나지 않습니다.

**SCORM**

* 일부 콘텐츠의 경우 SCORM 플레이어는 빈 화면을 표시합니다.
* 게시된 Storyline 프로젝트에 게시된 Captivate 출력을 가리키는 웹 개체가 포함된 경우 Storyline 콘텐츠가 Captivate 콘텐츠로 식별됩니다.
...
* 사용자 정의 작성자 또는 사용자 정의 관리자는 학습 프로그램이나 강의 또는 인증을 복제할 수 없습니다.
...
* 많은 사용자가 기밀로 설정되면 Edge 및 Internet Example에서 기밀 학습자 탭을 클릭하면 브라우저가 예기치 않게 작동합니다.
...
* 경우에 따라 계정에 유효한 PO가 있더라도 계정은 비활성화됩니다.

**API**

* Adobe Connect 카드에서 [연결 관리]를 클릭하면 FTP 연결 페이지로 리디렉션됩니다.
* 일부 시나리오의 경우 연결 관리자가 잘못된 경보를 받습니다.
...
최신 기능 및 업데이트를 사용하고 더 나은 경험을 얻으려면 디바이스 앱을 최신 버전으로 업데이트하는 것이 좋습니다. 업데이트는 다음과 같습니다 **필수**.

### 새로운 기능 및 향상된 기능 {#newandenhancedfeatures}

책임자는 앱의 모든 사용자에게 중요한 정보를 전달할 수 있습니다. 공지는 비디오 또는 이미지 유형으로 또는 단순 텍스트 메시지로 보낼 수 있습니다. 이번 디바이스 앱 릴리스를 통해 이제 디바이스 앱에서 공지를 지원합니다. 앱을 시작하면 새로운 공지가 표시되므로 학습자는 책임자가 보낸 중요한 전달사항을 놓치지 않을 것입니다. 학습자는 다음을 방문하여 즉시 읽거나 나중에 읽을 수 있습니다. **공지** 탭.

공지가 있는 경우 다음 링크에서 공지를 볼 수 있습니다. **공지** 섹션으로 이동하십시오.

공지를 보려면 다음 아이콘을 누릅니다. **공지**. 가장 최근의 공지가 화면에 표시됩니다.

다음 공지를 보려면 다음 아이콘을 누릅니다. **왼쪽으로 쓸어 넘기면 다음**.

### 공지 {#announcements}

![](assets/read-later.png)

공지를 바로 읽지 않으려면 공지를 나중에 읽도록 선택할 수 있습니다. 다음 아이콘 누르기: **나중에 읽기** 공지와 공지는 읽지 않음 상태로 전환됩니다.

### 이번 업데이트에서 수정된 버그 {#bugsfixedinthisupdate}

* iOS에서 화면이 잠겨 있으면 팟캐스트 재생이 중지되는 문제가 있었습니다. 이 문제는 수정되었으며 이제 화면이 잠겨 있어도 오디오가 재생됩니다.
* 장치 앱에서 강의를 수강하는 경우 결과 슬라이드가 공백으로 표시될 수 있습니다. 이 문제는 이번 업데이트에서 수정되었습니다.
...
Learning Manager 2020년 4월 릴리스는 다음 사항에 초점을 맞췄습니다.
...
자세한 내용은 다음을 참조하세요. [***Learning Manager 2020년 4월 릴리스의 새로운 기능***](../whats-new.md).
...
이제 앱에서 책임자가 설정한 설정에 따라 디바이스 앱 내에 브랜드 이름, 브랜드 로고 또는 둘 다를 표시할 수 있습니다.
...
* 전자 메일로 받은 교육 URL을 클릭합니다.
* 전자 메일 템플릿에 표시되는 기본 교육 URL
* 전자 메일 템플릿에 표시되는 계정 URL.
* 내 학습 및 카탈로그 이동 URL

또한 도메인이 포함된 모든 URL *learningmanager.adobe.com* 장치 앱에서 을 엽니다.

#### 수료를 증명하는 외부의 인증서 에셋 업로드 {#uploadassetsinexternalcertificateasproofofcompletion}

이번 업데이트에서 학습자는 수료를 증명하는 외부 인증서 에셋을 업로드할 수 있습니다.
...
### 이 릴리스에서 수정된 문제 {#issuesfixedinthisrelease}

* 전자 메일에 특수 문자가 포함되어 있으면 사용자가 장치 앱에 로그인할 수 없습니다.
* 목록 보기에서 스크롤하면 학습 객체 아이콘이 깜박입니다.
* 이제 아래쪽 화살표 버튼을 누르지 않고 푸시 알림을 모두 볼 수 있으며 한 번에 메시지를 확인할 수 있습니다.
...
자세한 내용은 다음을 참조하세요. [***구성 가능한 역할별로 범위가 지정된 학습 계획***](../administrators/feature-summary/custom-role.md#scopeconfigure).

### 보고서에서 활성 필드 제한 {#restrictactivefieldsinreports}

활성 필드의 경우 두 가지 새로운 옵션이 추가되었습니다. **보고 가능** 및 **내보내기 가능**.

CSV 필드 및 수동으로 추가된 필드의 경우 활성 필드가 다음으로 표시됨 **보고 가능**&#x200B;활성 필드 는 대시보드 보고서 내의 필터에서 검색할 수 있습니다.
...
모듈을 만드는 동안 설명을 추가합니다. 강의 생성에 대한 자세한 내용은 다음을 참조하십시오 [***강의 생성***](../authors/feature-summary/courses.md).

### 강의 및 세션 이름 표시 {#displaycourseandsessionnames}

강사는 출석 보기에서 세션 및 강의 이름을 볼 수 있습니다. 수정 중인 세션을 계속 추적할 수도 있습니다.

### 학습자용 공지 {#announcementforlearners}

학습자는 이제 목록 보기 대신 전체 보기로 공지를 볼 수 있습니다. 이는 학습자에게 읽지 않은 공지가 하나 있는 경우 적용됩니다. 이 기능을 통해 학습자가 편리하게 공지를 확인할 수 있습니다.
...
자세한 내용은 다음을 참조하세요. [***계정 사용자 정의***](../administrators/feature-summary/themes.md#customize).

### 배너 이미지 업로드 {#uploadbannerimages}
...
이번 업데이트에는 배지 PDF 다운로드를 허용하는 학습자 API가 포함됩니다.
...
이번 업데이트에는 퀴즈 보고서를 다운로드할 수 있는 관리자 API가 포함됩니다.
...
현재 책임자만 UI를 통해 사용자를 방해 금지 목록에 추가할 수 있습니다. 이번 릴리스 이후 학습자는 API가 관리자에 의해 활성화된 경우 제공된 API를 통해 이러한 권한을 스스로 설정할 수 있습니다. 계정에 대해 이 API를 활성화하려면 백엔드 설정이 필요합니다. 이 API를 사용하여 학습자는 전자 메일과 관련된 아래 권한을 편집할 수 있습니다.
...
### 이 릴리스에서 수정된 문제 {#Issuesfixedinthisrelease-1}

* 특정 사용자 그룹에 속한 사용자만 해당 그룹에 전달된 공지를 받아야 합니다. 다른 사용자는 해당 공지를 받아서는 안 됩니다.
* 플레이어는 콘텐츠가 표시되기 전에 로딩 회전자를 표시합니다.
* 보고서의 사용자 메타데이터로 인해 Null 포인터 예외가 발생합니다.
* Connect VC 강의의 기본 인스턴스에 대한 강사 추가 시 다음과 같은 메시지가 표시됩니다. *&quot;이 모듈에 대한 세션 없음&quot;*&#x200B;책임자 앱의 강의 인스턴스 페이지에 표시됩니다.

* FTP 전송 중에 학습자 성적 증명서를 내보내면 예상치 못한 동작이 발생합니다.
* 학습 프로그램의 강의에 작성자의 이름이 잘못 표시됩니다.
* 작업 지원 제품 용어의 변경 사항이 예상대로 반영되지 않습니다.
* 모듈의 이름이 길고 모바일 세로 모드에서 보는 경우 잘립니다.
* 이전 Connect 구현을 통해 기본 인스턴스를 업데이트한 후 이전 Connect 강의의 인스턴스를 생성할 수 없습니다.
* 강사는 강의를 게시하기 전에도 일정 초대를 받습니다.
...
이번 Learning Manager 업데이트에서는 각 카탈로그에 서로 다른 권한 집합이 부여된 여러 카탈로그에 대해 범위를 지정할 사용자 정의 역할을 만들 수 있습니다. 자세한 내용은 다음을 참조하세요. [***여러 카탈로그에 대한 사용자 정의 역할***](../administrators/feature-summary/custom-role.md#multi-scope).

### 검색 개선 사항 {#enhancementstosearch}

**학습 계획**

책임자 및 작성자를 위한 학습 계획 페이지에는 이제 학습 계획 검색에 사용할 수 있는 검색 창이 있습니다.
...
에 **카탈로그** 및 **내 학습** 페이지에서 학습자는 왼쪽 패널에서 다음과 같은 필터를 적용할 수 있습니다. **강의** 또는 **학습 프로그램**&#x200B;강의 또는 카탈로그 항목을 클릭합니다.

![](assets/choose-learning-objects.png)

학습자가 **카탈로그** 또는 **내 학습** [브라우저 뒤로] 단추를 사용하는 페이지에는 필터가 유지됩니다. 이전에 학습자가 적용한 필터는 더 이상 재설정되지 않습니다.
...
(으)로 **설정** [필터 패널 표시] 범주에 해당하는 페이지(를 클릭한 경우) **[!UICONTROL 편집]**, 다음 옵션을 볼 수 있습니다. 이 옵션을 사용하면 학습자에게 표시되는 필터 패널을 결정할 수 있으므로 학습자가 검색 결과를 미세하게 조정할 수 있습니다.

자세한 내용은 다음을 참조하세요. [***필터 패널 표시***](../administrators/feature-summary/settings.md#filter-panels).

### 책임자 앱에서 QR 코드 다운로드 {#downloadqrcodefromadministratorapp}

Learning Manager의 이전 업데이트에서는 사용자 정의 책임자가 QR 코드를 다운로드할 때 문제가 발생했습니다. 이번 업데이트에서는 액세스 권한이 있는 사용자 정의 책임자가 **모든 학습자** 및 권한: **강의 등록** qr 코드를 다운로드할 수 있습니다.

제한된 범위의 사용자에 대한 권한이 있는 경우 사용자 정의 역할 사용자는 여전히 QR 코드를 사용할 수 없습니다.
...
자세한 내용은 다음을 참조하세요. [***학습자를 등록하는 동안 댓글 추가***](../administrators/feature-summary/courses.md#enroll-comments).
...
이번 Learning Manager 릴리스에서는 이제 Adobe Connect과의 통합이 개선되어 지속 회의실을 지원합니다. 이제 Adobe Connect에서 이미 만든 회의실 중 하나를 사용하여 가상 강의실 세션을 만들 수 있습니다.

또한 Learning Manager를 통해 학습자는 SSO 인증을 사용하여 가상 세션을 위한 Connect 회의실에 입장할 수 있습니다.

자세한 내용은 다음을 참조하세요. [***Adobe Connect에서 지속 회의실 지원***](../integration-admin/feature-summary/connectors.md#persistent).
...
이번 업데이트에서는 **세션 지속 기간이 0임을 나타내는 경고 메시지**&#x200B;책임자, 관리자 또는 강사에게 표시됩니다.
...
이번 업데이트에서는 **세션 데이터가 불완전하다는 경고 메시지**&#x200B;을(를) 작성자에게 표시합니다.

### 이 릴리스에서 수정된 문제 {#Issuesfixedinthisrelease-2}
...
* 전자 메일 DND 목록에 사용자가 많을 경우 **설정** 페이지가 매우 느리게 로드됩니다. 이번 업데이트에서는 페이지네이션이 전자 메일 DND 목록에 추가됩니다.
...
* 모바일 앱에서 학습자는 의도된 동작이 아닌 학습 계획 인스턴스를 보고 등록할 수 있습니다. 이번 업데이트에서 이 문제는 수정되었습니다.
...
* 일부 시나리오에서는 학습자에게 몇 가지 강의 모듈이 표시되지 않습니다.
...
* 이전 릴리스보다 더 빠르게 시스템에서 사용자를 가져옵니다. 대용량 사용자 데이터를 가져오는 경우 크게 개선되었습니다.
* 관리자 및 책임자의 경우 필요에 따라 데이터를 로드하도록 보고서 구성 드롭다운 목록의 옵션이 수정되었습니다.
* API 성능이 향상되었습니다. 많은 API에서 응답 시간이 개선되었습니다.
* 학습자 성적 증명서를 생성하는 데 걸리는 시간이 개선되었습니다.
* 내부 및 외부 학습자, 특히 많은 수의 사용자가 나열된 페이지에는 지연이 발생하지 않습니다.

**특별 사용자 권한**

책임자는 모든 보드에 참여할 수 있는 그룹의 구성원을 사용하여 사용자 그룹에 특별한 권한을 부여할 수 있습니다. 특별 사용자 그룹은 범위 설정 섹션에서 설정한 모든 제한의 영향을 받지 않습니다. 자세한 내용은 다음을 참조하세요. [***특별 사용자 권한***](../administrators/feature-summary/social-learning-configurations-as-an-admin.md#privilege).

**사용자 인터페이스 변경 사항**

* (으)로 **보고서 추가** 대화 상자, **시간 범위** 및 **필터** 선택기는 별도 섹션으로 나타나며 기본적으로 접힌 상태로 표시됩니다. 자세한 내용은 다음을 참조하세요. [***보고서 만들기***](../administrators/feature-summary/reports.md#report).

* (으)로 **보고서 추가** 대화 상자에서 사용자 그룹의 경우 타이프 어헤드 검색을 사용하여 단일 또는 여러 사용자 그룹을 선택할 수 있습니다. 자세한 내용은 다음을 참조하세요. [***사용자 그룹 보고서***](../administrators/feature-summary/reports.md#user-group-reporting).

**시간 열 값 변경 사항**

학습자 성적 증명서의 시간 열에서 분은 가장 가까운 분으로 반올림되며 초 값은 00입니다. 자세한 내용은 다음을 참조하세요. [***시간 열***](../administrators/feature-summary/learner-transcripts.md#datetime).

### 이 릴리스에서 수정된 문제 {#Issuesfixedinthisrelease-3}
...
* 분류 변경 사항이 적용된 계정의 경우 변경 사항이 알림에 예상대로 반영되지 않았습니다.
...
또한 이 업데이트는 푸시 알림을 구현하기 위한 Google의 수정된 권장 사항을 준수하는 데 필요한 변경 사항을 지원합니다. 따라서 더 이상 수신하지 않습니다. **알림** 버전 2.7.4 이상을 사용하는 경우.
...
비공식 환경에서 아이디어와 의미 있는 인사이트를 공유합니다. 게시물에 좋아요, 싫어요를 표시하고, 콘텐츠를 업로드하고, 게시물에 댓글을 달아보십시오. 자세한 내용은 다음을 참조하세요. [***모바일 앱의 소셜 학습***](../learners/feature-summary/ipad-android-tablet-users.md#socialmobile).
...
강의 수료를 증명하는 파일을 강사에게 제출하십시오. 강사는 파일 콘텐츠를 기반으로 제출을 승인하거나 거부할 수 있습니다. 자세한 내용은 다음을 참조하세요. [***승인을 위해 파일 제출***](../learners/feature-summary/ipad-android-tablet-users.md#submitfile).

**업데이트된 플랫폼 지원**

Learning Manager 모바일 앱은 이제 Android 7 이상, iOS 10 이상이 설치된 디바이스에서 지원됩니다. 자세한 내용은 다음을 참조하십시오. [***시스템 요구 사항***](../system-requirements.md).

### 이 릴리스의 알려진 문제 {#Knownissuesinthisrelease-3}

1. Android에서는 GIF 파일을 게시물, 댓글 또는 댓글 회신에 업로드할 수 없습니다.
1. 게시판의 중재자는 사용자의 게시물, 댓글 또는 답변을 삭제할 수 없습니다. 그러나 웹 앱에서는 동일한 작업을 수행할 수 있습니다.
1. 앱에서는 질문 유형을 표시할 수 없습니다.
1. 앱에서 소셜 학습을 활성화한 후, 앱을 다시 실행하면 탭이 표시됩니다 **소셜 학습**. 소셜 학습이 표시되지 않으면 앱을 종료한 다음 다시 시작하십시오. 소셜 학습 탭이 표시될 것입니다.
...
소셜 학습을 사용하면 학습자가 게시한 콘텐츠를 두 가지 방식, 즉 **큐레이션 없음** 및 **수동 큐레이션**. 이번 릴리스에서 Adobe Learning Manager는 AI 지원 자동 큐레이션 기능을 제공하여 소셜 학습을 개선합니다. 콘텐츠가 게시되면 해당 콘텐츠를 분석하여 게시된 목적의 스킬에 속하는지 여부를 파악합니다. 신뢰 점수를 기반으로 콘텐츠가 실시간으로 게시되거나 수동 큐레이션을 위해 전송됩니다. 자세한 내용은 다음을 참조하세요. *[**&#x200B;자동 지원 큐레이션&#x200B;**](../administrators/feature-summary/social-learning-configurations-as-an-admin.md#autocuration)**.***

**스킬 도메인으로 스킬 매핑**

Learning Manager LMS에 있는 스킬 도메인으로 계정의 스킬을 매핑하십시오. 이는 Learning Manager가 자동 지원 큐레이션을 지원하는 스킬 도메인과 계정 스킬을 연결하는 데 도움이 됩니다. 자세한 내용은 다음을 참조하세요. [***도메인으로 스킬 매핑***](../administrators/feature-summary/curation-skills.md).

**CSV 사양 및 샘플 CSV**

기존 LMS 마이그레이션 데이터를 매핑하는 데 사용할 수 있는 CSV 사양이 업데이트되었습니다. 최신 다운로드 가능한 csv-specifications 및 sample-csvs zip 파일을 사용하여 입력할 데이터의 사전 정의된 형식을 이해합니다. 자세한 내용은 다음을 참조하세요.  [***마이그레이션 설명서***.](../integration-admin/feature-summary/migration-manual.md)

### 이 릴리스에서 수정된 문제 {#Issuesfixedinthisrelease-4}

**Learning Manager API**

* API의 POST 방법을 사용하여 외부 프로필을 추가하는 경우 *externalProfile*&#x200B;님, 환영 메일이 표시되지 않습니다.
...
* 작성자가 설명과 콘텐츠 개요를 포함하는 강의 및 인증을 만든 후 학습자가 강의 페이지에 액세스하면 콘텐츠 개요가 예상대로 표시되지 않았습니다. 그러나 책임자 및 작성자 앱의 학습자 미리 보기 페이지에는 콘텐츠가 표시되었습니다. 이번 릴리스에서는 개요 콘텐츠가 학습자 앱에 예상대로 표시됩니다.
...
* 학습 개체의 학습자 섹션에 있는 등록된 학습자에 대해 모든 인스턴스를 대상으로 통합 검색을 수행할 수 있습니다. 학습 개체의 학습자 섹션에 있는 등록된 사용자를 타이프 어헤드 검색을 통해 검색합니다. 자세한 내용은 다음을 참조하세요. [***등록된 사용자 검색***](../administrators/feature-summary/courses.md#searchforusers).
* 공유 카탈로그를 통해 획득한 학습 개체의 편집 기능을 완성합니다. 자세한 내용은 다음을 참조하세요. [***공유 카탈로그 제어***](../administrators/feature-summary/shared-catalog-full-control.md). 이 기능을 활성화하려면 Learning Manager 지원 팀에 문의하십시오.
...
* 사용자 정의 역할 생성을 자동화합니다. 자세한 내용은 다음을 참조하세요. [***CSV 파일을 통해 역할 구성***](../integration-admin/feature-summary/configure-role-csv-files.md).
...
* 삭제된 사용자의 학습자 성적 증명서 다운로드 자세한 내용은 다음을 참조하세요. [***학습자 성적 증명서***](../administrators/feature-summary/learner-transcripts.md).
* 다음 언어 지원:
...
* 스킬 점수에서 소수점 지원은 영어로만 지원됩니다.
* 한국어, 일본어, 러시아어의 경우 세션 시간 자오선 값이 예상대로 표시되지 않습니다.

**이 릴리스에서 수정된 문제**

* 출석 및 점수 탭의 학습 프로그램 또는 인증에 대해 퀴즈 점수가 저장되지 않습니다.
* 책임자 또는 관리자가 외부 인증 완료를 표시할 수 없습니다.
* 언어가 포르투갈어 또는 스페인어로 설정되면 학습자 성적 증명서 대화 상자에서 책임자가 사용자 정의 날짜 범위를 선택할 수 없습니다.
* 프로필 언어가 프랑스어인 경우 책임자가 외부 프로필을 생성할 수 없습니다.
* 로케일이 영어 이외의 다른 언어인 경우 활성 필드가 사용자 편집 대화 상자에 표시되지 않습니다.
...
* **QR 코드 스캔으로 등록:** QR 코드 기능이 향상되었습니다. QR 코드를 이용한 출결 표시 지원은 물론 이제 QR 코드를 이용한 강의 등록, 완료를 지원합니다.
...
* 강사 앱에서 세션 세부 정보가 변경되면 학습자와 강사가 전자 메일을 받지 못하였습니다. 이 문제는 수정되었습니다.
* url 끝에 &#39;/&#39;가 붙으면 Adobe Connect URL이 제대로 작동하지 않을 수 있습니다. 이 문제는 수정되었습니다.
...
* 중복 강의로 선택한 모듈의 값이 즉시 나타나지 않을 수 있습니다. 페이지를 새로 고침한 경우에만 중복 강의로 표시됩니다. 이 문제는 수정되었습니다.
* 강의 게시 후에는 모든 모듈이 편집 모드에서 선택 해제된 상태로 표시됩니다. 변경 사항을 보려면 페이지를 새로 고쳐야 합니다. 이 문제는 수정되었습니다.
* 필수 모듈을 선택할 수 없는 경우에는 지정된 강의에서 필수 모듈을 선택할 수 있었습니다. 이 문제는 수정되었습니다.
* 강의를 삭제한 후에도 필수 모듈이 드롭다운 확인란에 계속 표시됩니다. 이 문제는 수정되었습니다.
* 사전 작업 및 테스트 종료 모듈이 기본적으로 필수 항목으로 표시됩니다. 이 문제는 수정되었습니다.
* 이메일에서 L3 피드백 링크를 클릭하면 피드백 양식이 열리지 않을 수 있습니다. 이 문제는 수정되었습니다.
...
* 학습자 성적 증명서에는 선택한 날짜 범위에 따라 다운로드한 데이터가 표시되지 않았습니다. 이 문제는 수정되었습니다.
...
* 사용자 그룹 페이지에서 총 사용자 수와 사용자 목록이 일치하지 않을 수 있습니다. 이 문제는 수정되었습니다.
* 활성화되어도 학습 프로그램 팝업이 나타나지 않는 경우가 있었으며 사용자는 LO 페이지로 리디렉션되었습니다. 이 문제는 수정되었습니다.
* 대기자 목록이 삭제되고 학습자가 강의에 등록되면 관리자는 학습자의 이름이 나오는 대신 자신의 이름이 나오는 전자 메일 알림을 받습니다. 이 문제는 수정되었습니다.
...
* 대용량 학습자 성적 증명서를 다운로드하면 시간 초과 문제가 발생할 수 있습니다. 이 문제는 수정되었습니다.
* 학습자의 퀴즈 점수가 기록되는 대용량 등록 보고서를 다운로드하면 시간 지연 문제가 발생하였습니다. 이 문제는 수정되었습니다.
* 대용량 퀴즈 점수 보고서를 다운로드하면 평소보다 시간이 오래 소요될 수 있습니다. 이 문제는 수정되었습니다.
...
* Learning Manager는 24시간 형식으로 전자 메일 시간을 표시합니다. 이 문제는 수정되었습니다. 이제 시간이 12시간 형식으로 표시됩니다.
...
* 검색한 그룹에 사용자가 없으면 검색 결과가 빈 창으로 표시됩니다. 이 문제가 수정되었습니다.
* 직접 보고서가 없더라도 관리자를 삭제할 수 없었습니다. 이 문제는 수정되었습니다. 이제 관리자를 삭제할 수 있습니다.
* 모듈 진행 기능 재설정이 작동하지 않을 수 있습니다. 이 문제가 수정되었습니다.
* 삭제된 인증은 학습자용 위젯에 표시될 수 있습니다. 이 문제는 수정되었습니다.
* 강의 효과 계산 문제가 해결되었습니다.

* 새 Connect 계정 통합 문제가 수정되었습니다.
...
2018년 3월 버전 Adobe Learning Manager는 멋지고 새로운 기능과 개선 사항을 발표합니다. 사용자 감사 추적 보고서와 로그인/액세스 보고서를 생성하고, 학습자가 강의 인스턴스를 선택할 수 있는 기능을 제공하며, 강의실과 가상 강의실을 개선하는 등의 작업을 수행할 수 있습니다. 또한 이 릴리스에서는 버그가 수정되었으며 성능이 개선되었습니다.
...
* 학습자가 개인 작업 지원을 추가/실행할 수 없습니다. 이 문제는 수정되었습니다.
...
* 등록된 사용자가 강의에서 삭제되면 해당 특정 강의에 대한 강의 보고서와 퀴즈 보고서가 생성되지 않을 수 있습니다. 이 문제는 수정되었습니다.
* 책임자가 지정 시트의 수를 변경하면 다른 위치에서 여는 경우에 할당 요청 숫자가 잘못 표시될 수 있습니다. 이 문제는 수정되었습니다.
* 외부 사용자를 내부 사용자로 변환하면 해당 사용자가 모든 내부 학습자 그룹에 추가되지 않을 수 있습니다. 이 문제는 수정되었습니다.
* 개별 학습자를 검색하고 선택한 후에 등록 취소 작업을 수행하면 선택한 학습자 대신 해당 그룹의 모든 학습자가 등록 취소될 수 있습니다. 이 문제는 수정되었습니다.
* 책임자는 인증 안에서 학습자를 선택하기 위해 확인란을 사용할 수 없었습니다. 이 문제는 수정되었습니다.
* 개별 사용자가 600명을 초과하는 사용자 그룹은 생성 및 업데이트되지 않을 수 있습니다. 이 문제는 수정되었습니다. 이제 개별 사용자가 600명을 초과하는 사용자 그룹을 만들 수 있습니다.
* 다른 사용자 정의 사용자 그룹의 일부인 사용자 정의 사용자 그룹을 삭제하면 교차 규칙이 사용자 번호를 상위 그룹으로 넘길 수 있습니다. 이 문제는 수정되었습니다.
...
* 모든 응용 프로그램에서 강의 및 사용자를 검색하면 뛰어난 검색을 경험할 수 있습니다. 여기에는 강의, 사용자 및 사용자 그룹 검색이 포함됩니다.
* Learning Manager와 외부 시스템을 통합하는 데 Box 커넥터를 사용하여 데이터를 자동으로 동기화할 수 있습니다. 자세한 내용은 다음을 참조하세요. [Box 커넥터](../integration-admin/feature-summary/connectors.md#main-pars_header_302653946).
* 기존 LMS 마이그레이션 데이터를 매핑하는 데 사용할 수 있는 CSV 사양이 업데이트되었습니다. 최신 다운로드 가능한 csv-specifications 및 sample-csvs zip 파일을 사용하여 입력할 데이터의 사전 정의된 형식을 이해합니다. 자세한 내용은 다음을 참조하세요. [마이그레이션 설명서.](../integration-admin/feature-summary/migration-manual.md)
...
* 지원되지 않는 Internet Explorer(IE 7, IE 8, IE 9 및 IE 10) 및 Safari(버전 7, 8, 9)의 버전에서 지원되지 않는 브라우저 버전 오류 메시지가 표시되지 않습니다. 이 문제는 수정되었습니다.
...
* 알림 데이터가 없는 전자 메일 등 Learning Manager의 특정 전자 메일이 사용자에게 전달되지 않습니다. 예- 외부 프로필 생성 및 활성화 시 받은 전자 메일과 연결 계정 구성 시 받은 전자 메일. 이 문제는 수정되었습니다.
...
* 장치 브라우저에서 Learning Manager를 열면 장치 앱에서 Learning Manager를 경험할 수 있는 옵션이 표시됩니다. 예를 클릭하면 앱이 설치되어 있지 않은 경우 Play Store(Android)를 시작하고, 설치되어 있는 경우 앱(Android 및 iOS)을 실행해야 합니다. 이 작업 과정은 문제가 발생하여 수정되었습니다.
...
### 해결된 문제 {#IssuesFixed-3}

**강의 중 삭제하면 이전 버전으로 롤백되는 모듈이 여러 개 있는 강의가 있습니다.**

강의에 특정 모듈에 대한 모듈 버전 항목이 여러 개 있는 경우 모듈을 삭제하면 이전 버전으로 롤백되고 완전히 삭제되지는 않습니다. 이 문제는 수정되었습니다.

**공유 강의 변경은 탭을 통해 자동으로 저장하면 반영되지 않습니다.**

상위 계정의 작성자가 다음 탭으로 이동하여 강의를 자동 저장하여도 공유 강의 변경 내용이 테넌트 계정으로 전달되지 않습니다. 이 문제는 수정되었습니다.

**사용자가 활동 전용 강의 인스턴스의 마감일을 변경할 수 없었습니다.**

활동 강의의 인스턴스의 마감일을 이전 마감일로 되돌릴 수 있으므로 사용자는 인스턴스의 마감일을 변경할 수 없었습니다. 이 문제는 수정되었습니다.

**학습 객체에서 분리된 고유 ID를 사용할 수 없습니다.**

강의에 고유 ID를 부여한 다음 제거하면 해당 ID를 다시 사용할 수 없습니다. 이 문제는 수정되었습니다.
...
* 새 노트를 추가하려면 노트 패널을 닫고 플레이어에서 재생을 재개해야 합니다. 특정 경우에는 이러한 현상이 발생하지 않을 수도 있습니다. 이 문제는 수정되었습니다.
* 모듈에 추가된 노트를 열면 닫기 버튼이 나타나지 않을 수 있습니다. 이 문제는 수정되었습니다.
...
* 여러 언어를 사용하는 모듈이 있는 강의에서는 스크롤 막대가 필요한 만큼 확장되지 않아 사용 가능한 모든 언어가 표시되지 않습니다. 이 문제는 수정되었습니다.
...
* 장치 플레이어에서 가로 모드에서 모듈이 잘리는 TOC 섹션 관련 문제가 수정되었습니다. 경우에 따라 장치 플레이어에서 콘텐츠에 TOC가 표시되지 않습니다. 이 문제는 수정되었습니다.
...
### 개선 사항 {#Enhancements-1}

**게임화**

이 릴리스에서는 게임화 범위를 개선합니다. 이제 외부 사용자가 게임화에 참여할 수 있습니다. 책임자는 범위 설정을 변경하여 게임화 범위를 정할 수 있습니다. 유사한 프로필 사용자, 그룹 또는 위치 사이에서 선택하여 게임화 기능을 활성화할 수 있습니다.
...
작업 지원은 강의 또는 학습 프로그램과 같은 특정 학습 대상에 등록하지 않고도 학습자가 접속할 수 있는 교육 콘텐츠입니다. 이러한 향상된 기능을 통해 관리자는 작업 지원 보고서를 추출하고 다운로드할 수 있습니다. 책임자는 발송한 모든 공지사항에 대한 보고서를 생성할 수도 있습니다. 책임자와 관리자는 등록되지 않은 학습자의 보고서도 추출할 수 있습니다.

**Learning Manager 커넥터**

이제 데이터 내보내기 옵션을 사용하여 사용자 스킬을 FTP 위치로 내보내 타사 시스템과 통합할 수 있습니다. 통합에 대한 연결 이름을 지정하고 내부 사용자를 가져올지 또는 필요에 따라 사용자 스킬을 구성하거나 가져와서 내보낼지를 선택할 수 있습니다.

**강의 인스턴스 복사**

이제 인스턴스 오른쪽 상단 모서리에 있는 드롭다운 화살표를 클릭하여 인스턴스 URL을 복사할 수 있습니다.

### 해결된 문제 {#IssuesFixed-4}

**학습 프로그램 등록 시 일정관리 초대를 받지 못한 사용자**

사용자가 학습 프로그램, 강의실이 있는 인증서 및 연결 세션에 등록하여도 일정 초대 파일(.ics)을 받지 못하는 경우가 있었습니다. 이 문제는 수정되었습니다. 이제 사용자는 등록 전자 메일과 함께 세션 세부 사항을 안내하는 .ics 파일을 받습니다.

**다국어 제공을 받아도 활동 설명서가 영어로 나타나는 문제**

콘텐츠 언어 선호를 영어로 설정하지 않은 사용자여도 여전히 영어로 된 활동 모듈 설명서를 받습니다. 이 문제는 수정되었습니다.
...
**학습 계획에서 지원하는 여러 학습 객체**

이제 책임자나 작성자는 학습 계획에 학습 객체를 여러 개 부여할 수 있습니다. 학습 객체의 인스턴스 중 하나가 만료되면 전체 학습 계획이 자동으로 비활성화됩니다. 향상된 기능을 통해 다음을 검색할 수도 있습니다 **[!UICONTROL 완료한 학습]** 및 **[!UICONTROL 학습 할당]** 학습 객체 고유 ID를 사용하는 필드

**액세스 가능성**

이 업데이트를 통해 Learning Manager 학습자 경험은 이제 섹션 508 접근성 표준을 지원합니다. Learning Manager는 의 최신 버전과도 호환됩니다 **[!UICONTROL JAWS]**. 이 기능은 학습자 응용 프로그램에서만 지원됩니다. 이 기능에 액세스하려면 Internet Explorer 11, Windows Chrome 또는 macOS Safari를 사용하십시오.

### 해결된 문제 {#IssuesFixed-5}
...
경우에 따라 학습자가 새 스킬을 구독할 수 없었습니다. 이 문제는 수정되었습니다.
...
홈페이지 위젯을 사용하여 강의, 스킬 및 성과를 관리합니다. 모든 학습 객체, 카탈로그, 스킬, 노트와 토론을 포함한 전체 LMS에서 검색 막대를 사용하여 검색할 수 있습니다

새 홈페이지에 대한 자세한 내용은 다음을 참조하십시오.  [Learning Manager 학습자 홈페이지](../learners/feature-summary/getting-started-learner.md).

**학습자 대시보드 관련 책임자 설정**

책임자는 다른 위젯을 사용하거나 사용하지 않도록 설정하여 학습자의 홈페이지를 관리할 수 있습니다.
...
관리자인 경우 구입한 좌석을 공유할 수 있는 피어 계정을 생성할 수 있습니다.
...
이제 작성자와 관리자는 학습 객체를 만들면서 강의, 학습 프로그램 또는 인증에 대한 고유 ID를 지정할 수 있습니다. 학습 객체를 만들 때 고유 ID를 사용하려면 &#39;설정&#39; > &#39;일반&#39;을 클릭합니다. 고유 학습 개체 ID 옵션 옆에 있는 활성화 확인란을 선택합니다.

**학습자를 위한 토론 게시판**

강의에서 토론 게시판을 사용하여 동료와 강사가 상호 작용할 수 있습니다. 학습자는 강의에 대한 모든 게시물을 볼 수 있습니다. 그러나 입력한 게시물만 삭제할 수도 있습니다. 토론 게시판에 대한 자세한 내용은 다음을 참조하십시오.  [토론 보기 및 참여](../learners/feature-summary/courses.md#main-pars_header_1772461149).
...
이제 책임자는 작성자가 모듈을 편집하거나 업데이트하고 강의를 다시 등록할 때마다 알림을 받습니다.
...
이제 관리자는 스티커 형태의 공지를 사용할 수 있습니다.
...
### 해결된 문제 {#IssuesFixed-6}

#### 계정 설정 {#accountsettings}

책임자가 &#39;외부 프로필&#39;을 클릭하고 &#39;동작&#39; > &#39;프로필 변경&#39;을 실행하여도 모든 프로필이 정렬되지 않습니다. 이 문제는 수정되었습니다.

#### 강의 사용 주기 {#courselifecycle}

* Biz 라이브러리 e-러닝 도구를 사용하여 만든 강의를 시작하면 &quot;다시 시작&quot; 작업이 작동하지 않습니다. 이 문제는 수정되었습니다.
* 일부 사용자는 공지에서 강의 링크를 사용하여 iPad에서 강의를 실행할 수 없습니다. 이 문제가 수정되었습니다.
* 학습자 프로그램에서 &#39;계속&#39; 버튼을 클릭하면 강의에 순서대로 접속할 수 없습니다. 이 문제가 수정되었습니다.
* 학습자 프로그램 강의의 &#39;강의 개요&#39; 레이블이 잘못 배치되었습니다. 이 문제는 수정되었습니다.

#### 학습자 앱 {#learnerapp}

* 장치에서 전체 화면 모드로 공지 이미지를 열면 응용 프로그램으로 돌아갈 수 없습니다. 이 문제는 수정되었습니다.
* Captivate에서 업로드한 틴칸 콘텐츠가 Android 및 iOS 운영 체제에서 온라인 모드로 재생되지 않았습니다. 이 문제는 수정되었습니다.
...
AP/courses/를 사용하여 모듈 버전 정보를 가져오려고 하면 오류가 발생합니다.{coursesid}. 이 문제는 수정되었습니다.
...
Adobe을 통해 기업은 조직의 교육 데이터 및 교육 콘텐츠를 기존 학습 관리 시스템(LMS)에서 Learning Manager LMS 응용 프로그램으로 마이그레이션할 수 있습니다.
...
이제 관리자가 외부 사용자를 추가하면서 특정 도메인 이름도 추가할 수 있습니다. 학습자가 계정에 등록할 때 이러한 도메인 이름에서만 전자 메일 주소를 입력할 수 있습니다.

사용자가 계정에 등록할 때 사용자의 전자 메일로 전자 메일 확인 링크를 보낼 수도 있습니다. 이러한 개선 사항에 대한 자세한 내용은 다음을 참조하십시오.  [사용자/사용자 그룹 추가](../administrators/feature-summary/add-users-user-groups.md#main-pars_header_1217981931).

**Fluidic Player**

이제 Fluidic Player를 사용하여 재생 속도를 수정할 수 있습니다. 다섯 가지 속도 변형을 선택할 수 있습니다. 강의 수강 시 Fluidic Player를 사용하여 볼륨 설정을 제어할 수도 있습니다.
...
카탈로그 공유 기능을 통해 책임자는 계정 전반에서 학습 객체와 카탈로그를 공유하거나 획득할 수 있습니다. 카탈로그 공유 기능을 확장하면 배지, 스킬, 모듈, 강의, 학습 프로그램, 인증 및 작업 지원과 같은 학습 객체로 업데이트를 전송할 수 있습니다.
...
* 강의에 스킬을 추가하는 동안, 작성자가 이름 끝에 공백이 있는 스킬을 추가하면 오류가 발생하여 강의가 저장되지 않았습니다. 이 문제는 수정되었습니다.
...
* 경우에 따라 사용자가 숨겨진 프레임/슬라이드가 있는 모듈 콘텐츠를 업로드할 때 왼쪽 창의 목차에서 숨겨진 프레임/슬라이드가 표시되었습니다. 이 문제는 수정되었습니다.
...
* 복합 문자로 자체 등록된 사용자 정보를 수정한 후 CSV를 가져오면 자체 등록된 사용자 세부 정보가 관리자 사용자 인터페이스에 표시되지 않습니다. 이 문제는 수정되었습니다.
...
* 등록 또는 재설정 중에 암호를 설정하는 동안 입력한 암호가 암호 정책을 준수하지 않아도 오류 메시지가 나타나지 않습니다. 이 문제는 수정되었습니다.
...
* 책임자가 &#39;보고서&#39;>&#39;내 보고서&#39; 탭에서 강의 보고서 링크를 클릭하면 닫기 버튼이 없는 팝업 대화 상자가 나타납니다. 이 문제는 수정되었습니다.
...
이번 업데이트 릴리스에서는 제품 엔지니어링 백엔드 문제를 일부 해결하여 고객 경험을 개선했습니다.
...
경우에 따라 작성자가 수정된 모듈 정보를 업데이트하기 위해 공유 강의를 편집하면 이전 변경 내용이 처리 중이므로 편집할 수 없다는 경고 메시지가 나타납니다. 이 문제는 수정되었습니다.
...
학습자가 학습자 계정의 작업 지원 탭에 접근하면 콘텐츠를 로드하기 전에 &#39;이제 작업 지원이 목록에 존재하지 않습니다&#39;라는 오류 메시지가 나타납니다. 이 문제는 수정되었습니다.
...
* 사용자를 가져오는 동안 포함 관리자를 사용하면 사용자의 직속 관리자 대신 전체 등급의 관리자를 가져옵니다. 이 문제는 수정되었습니다.
* 학습자 범위 인증 권한이 있는 사용자가 사용자를 추가하면 일반적인 오류 메시지가 나타납니다. 이 문제는 수정되었으며 이제 비인증 접근 메시지가 학습자에게 나타납니다.
* 사용자가 사용자 그룹의 마지막 사용자를 삭제하려고 하면 오류 메시지 204가 나타납니다. 이 문제는 수정되었으며 이제는 그룹에 사용자가 한 명 이상 있어야 한다는 관련 오류 메시지가 사용자에게 나타납니다.
* 이름의 시작 부분에 공백이 있는 경우 GET/사용자 API에 사용자 이름을 표시하면 공백이 잘렸습니다. 이 문제는 수정되었습니다.
...
* 경우에 따라 Adobe Connect 기반 가상 강의실 모듈을 생성하는 동안 강사 이름이 강사 드롭다운 목록에 여러 번 나타납니다. 이 문제는 수정되었습니다.
...
이 업데이트의 일환으로 일부 고객의 요구 사항을 충족하기 위해 다음과 같은 새로운 API를 추가했습니다.

1. POST 사용자
1. DELETE 사용자
1. GET 사용자 그룹
1. GET 사용자 그룹 /{id}
1. DELETE 사용자 그룹 /{id}/Users
1. POST 사용자 그룹 /{id}/Users
1. GET /users/userId/userGroups

또한 다음과 같은 추가 기능으로 기존 사용자 모델을 개선했습니다.

1. 관리자 모델이 사용자 모델에 관계로 추가되었습니다.
1. userGroupId가 GetUsers에 대한 새 매개 변수로 추가되었습니다.
...
* 강의 모듈이 개방형 질문이나 설문 조사 질문으로만 구성된 경우 이를 내보내면 공백의 퀴즈 점수 보고서가 나타납니다. 이 문제는 수정되었습니다.
...
검색 필드가 특수 문자 입력을 지원하지 않아 검색 결과가 표시되지 않습니다. 이 문제는 수정되었습니다.
...
계정 책임자가 수신기와 강의 카탈로그를 공유하고 나중에 테스트 아웃 또는 사전 작업 모듈을 업데이트한 시나리오에서 수신기용 콘텐츠 모듈에서 재생하는 데 사용된 모듈 콘텐츠를 사전 작업하거나 테스트합니다. 이 문제는 수정되었습니다.
...
책임자가 응용 프로그램 로케일을 중국어 간체자 또는 스페인어로 변경하면 왼쪽 창의 일부 메뉴 내용, 온라인 지침 및 팝업 메시지가 제대로 된 단어나 문장으로 표시되지 않습니다. 이 문제는 수정되었습니다.

**Fluidic Player**

* 작성자가 AICC 또는 Tin Can 콘텐츠로 강의를 만들고 내용을 미리 보려 하면 콘텐츠가 재생되지 않습니다. 이 문제는 수정되었습니다.
...
* Salesforce 또는 FTP 연결을 설정하면 맵 속성 페이지의 필드 드롭다운 화살표가 IE, Edge 및 Safari 브라우저에서 표시되지 않습니다. 또한 일부 팝업 메시지는 워크플로우에 표시되지 않습니다. 이 문제는 수정되었습니다.
* 경우에 따라 책임자가 FTP 커넥터에 .csv로 가져온 데이터를 동기화하면 복사가 일어나며 동기화되지 않습니다. 이 문제는 수정되었습니다.
...
데이터 소스 페이지에서 자동 예약 시간을 설정하고 저장하면 이전 상태로 돌아가는 경우가 있습니다. 이 문제는 수정되었습니다.

**사용자 그룹 보고**

보고서 유형을 [사용자 정의]로 선택하면 필터의 사용자 그룹 값이 생성되지 않습니다. 이 문제는 수정되었습니다.
...
Learning Manager 7월 릴리스의 개선 사항에 대한 자세한 내용은 다음을 참조하십시오. [새로운 기능](../whats-new.md).

일부 개선된 기능이 참조할 수 있도록 아래에 나열되어 있습니다.
...
Learning Manager에서는 배지를 PDF 파일로 내보낼 수 있습니다. 자세한 내용은 다음을 참조하십시오.  [배지 기능 콘텐츠](../administrators/feature-summary/badges.md).

**모듈용 퀴즈 점수**

강의실, 가상 강의실 및 활동 모듈에 대해서 퀴즈 점수를 추가할 수 있습니다.
...
* 이제 학습자의 이름과 전자 메일 ID를 편집할 수 있습니다.

**자가 등록**

학습자는 강의, 학습 프로그램 또는 인증과 같은 학습 객체에 스스로 등록할 수도 있습니다. 단, 학습자는 학습 객체를 완료한 후에는 학습 객체 등록을 취소할 수 없습니다.

**학습 객체 이용**

이제 책임자는 학습자의 학습 활동을 완료로 표시할 수 있습니다.
...
외부 학습자가 외부 프로필 URL을 사용하여 등록 페이지를 방문하면 등록 페이지에 외부 프로필 이름이 표시되어 더 나은 인식이 가능합니다.

### 해결된 문제 {#Issuesfixed-9}
...
* 인증 **유효성** 월 단위로 사용자에게 표시됩니다. 이전에는 기준값이 년 단위로 나타났습니다.
...
학습자가 카탈로그에 접속할 때 기본 포커스는 다음과 같은 순서로 학습 객체의 가용성에 따라 탭 간을 이동합니다. 1. 강의 2. 프로그램 3. 인증 및 4. 작업 지원. 예를 들어, 해당 학습자의 강의 탭에서 해당 강의를 들을 수 없는 경우, 학습 프로그램이 있는 다음 학습 객체로 포커스가 이동합니다.

**계정 설정**

책임자가 계정을 비활성화하면 계정 비활성화 확인 대화 상자에서 피드백 옵션이 나타납니다.

### 해결된 문제 {#Issuesfixed-10}

**보고서 내보내기**

* 많은 사용자가 학습 프로그램에 등록되어 있으면 학습자 내보내기가 실패했습니다. 이 문제는 수정되었습니다.
* 강의에 이름이 같은 인스턴스가 두 개 있고 그 인스턴스 이름이 길면 내보낸 Excel 파일에 워크시트가 두 개 생성되지 않습니다. 이 문제는 수정되었습니다.
...
### 해결된 문제 {#Issuesfixed-11}

**모듈을 이용한 강의 생성**

파일 이름이 긴 모듈 콘텐츠가 업로드되면 버튼 모양에 문제가 발생합니다. 또한 모듈 업로드 및 삭제 옵션이 예상대로 작동하지 않습니다. 이 문제는 수정되었습니다.
...
URL을 사용하여 작업 지원을 생성합니다. 기존 온라인 콘텐츠를 작업 지원으로 사용하려면 작업 지원 생성 워크플로우에서 URL 이름만 언급하면 작업 지원 생성이 가능합니다.
...
의 값을 언급하면 몇 개월 단위로 인증을 갱신할 수 있습니다. **유효성** 필드. 이전에는 인증 갱신이 연 단위로만 허용되었습니다.

### 해결된 문제 {#Issuesfixed-12}
...
* 학습자가 작업 지원을 목록에서 제거하는 경우 **학습** 탭, **이름** 페이지를 새로 고칠 때까지 정렬이 예상대로 작동하지 않습니다. 이 문제는 수정되었습니다.

* 작업 지원이 여러 강의의 일부인 경우, 그 강의는 작업 지원 목록에 표시되지 않습니다. 이 문제는 수정되었습니다.
...
### 해결된 문제 {#Issuesfixed-13}

**모듈로 강의 생성**

* 책임자 로그인에서 강의 인스턴스를 **중단됨** tab 키를 누르면 오류가 발생합니다. 이 문제는 수정되었습니다.
* 지역 콘텐트의 책임자 로그인에서 학습자를 강의 인스턴스에 등록하는 동안 작업 및 등록 화면 레이아웃 왜곡 현상이 있습니다. 이 문제는 수정되었습니다.
* 작성자가 강의실 또는 가상 강의실 모듈을 생성할 때 날짜 캘린더의 기본 월은 2015년 1월로 표시됩니다. 이 문제는 현재 날짜를 기본적으로 반영하도록 수정되었습니다.
* 강의 인스턴스 이름이 슬래시 또는 백슬래시로 구성된 경우 학습자 목록 내보내기 작업이 실패합니다. 이 문제는 수정되었습니다.
...
CSV 업로드 기능의 도움말 URL이 영어 도움말 콘텐츠로 이동합니다. 이 문제는 수정되었습니다.

**강의 미리 보기 및 게시**

작성자 로그인에서는 발표자 10 및 11 TinCan API(SWF/HTML) 콘텐츠를 미리 볼 때 콘텐츠가 작동하지 않습니다. 이 문제는 수정되었습니다.

**사용자 정의 전자 메일**

전자 메일 템플릿의 제목이 적절하지 않습니다. 템플릿 제목을 업데이트하여 읽을 수 있도록 하였습니다.
...
책임자가 정의한 사용자 그룹에 멀티미디어 메시지(텍스트, 이미지 또는 비디오)로 공지를 보낼 수 있습니다. 공지를 사용하여 학습자가 교육에 참여하도록 동기를 부여하고 학습 문화를 구축할 수 있습니다.

이 기능에 대한 자세한 내용은 다음을 참조하십시오.  [공지 도움말](../learners/feature-summary/announcements.md).

### Tin Can API 지원

Adobe Learning Manager는 Tin Can API(Experience API 또는 xAPI라고도 함) 사양을 지원합니다. SCORM 및 AICC 콘텐츠를 추적하는 방법과 유사하게 Tin Can API 호환 콘텐츠를 업로드하고 추적할 수 있습니다.

자세한 내용은 Adobe 지원 팀에 문의하십시오.

### 강의 시퀸싱

후속 강의 또는 모든 학습 활동을 자동으로 할당하여 학습 경로를 생성할 수 있습니다.

학습 계획 이벤트가 업데이트되었습니다. 두 개의 새로운 이벤트가 추가되었습니다. 자세한 내용은  [학습 계획](../learners/feature-summary/learning-programs.md) 를 참조하십시오.

### 메모 미리 알림

강의를 이용하면서 메모하면 Learning Manager에서 메모 검토 알림을 15일 후에 전송합니다.

### 그룹 수준 게임화

책임자는 범위 설정을 변경하여 게임화 범위를 정할 수 있습니다. 유사한 프로필 사용자, 그룹 또는 위치 사이에서 선택하여 게임화 기능을 활성화할 수 있습니다. 자세한 내용은  [게임화](../learners/feature-summary/gamification.md) 기능을 사용하여 자세한 내용을 살펴보십시오.

### 프랑스어 및 독일어 지원

Learning Manager 응용 프로그램은 프랑스어와 독일어로 제공됩니다. 피드백, 강의 인스턴스 및 커뮤니케이션을 위해 언어를 사용자 정의할 수 있습니다.
...
사용자를 삭제하면 단일 사용자 추가를 사용하여 동일한 사용자를 다시 응용 프로그램에 추가할 수 없습니다. 그러나 CSV 업로드 프로세스를 사용하면 삭제된 사용자를 다시 추가할 수 있습니다. CSV 업로드 기능의 필수 필드 제한이 크게 변경되었습니다. 자세한 내용은  [CSV의 FAQ](../administrators/add-users-in-bulk.md) 를 참조하십시오.

### 강의 목록 보기

기본적으로 강의는 카드로 볼 수 있습니다. 이 릴리스에는 목록 보기가 추가되었습니다. 검색 필드 옆에 있는 트리플 바 아이콘을 클릭하여 보기를 변경할 수 있습니다.
...
학습자는 학습 프로그램에 자체 등록될 때마다 알림을 받습니다.

**강의실 모듈(ILT)**

과거 날짜에 대한 강의실 강의를 생성할 수 있습니다. 이 기능을 사용하면 회사 관리자가 이전 강의실 강의 정보도 Learning Manager에 제공할 수 있으며 보고서도 생성할 수 있습니다.
...
외부 학습자를 Learning Manager 계정에 등록한 후 필요한 경우 학습자 수를 줄일 수도 있습니다. 그러나 학습자 수를 사용된 시트 수 이상으로 줄일 수는 없습니다. 먼저 등록한 학습자를 삭제한 다음 필요한 시트 수로 다시 등록하여 문제를 해결할 수 있습니다.

### 해결된 문제 {#Issuesfixed-14}
...
기존의 자체 등록 사용자가 자체 등록 프로세스를 사용하여 다시 등록하려고 하면 적절한 메시지가 나타납니다. 메시지의 형식과 내용이 수정되었습니다.
...
기존 강의 모듈을 다른 모듈로 교체할 때마다 새로운 버전의 강의가 생성됩니다. 이 새 모듈의 퀴즈를 내보내면 예외가 발생하여 퀴즈 보고서가 생성되지 않습니다. 이 문제는 수정되었습니다.
...
* 사용자는 전자 메일 템플릿의 바닥글 섹션을 수정할 수 있습니다. 선택한 텍스트로 전자 메일 템플릿의 바닥글을 사용자 정의할 수 있습니다. 사용자 정의된 바닥글은 모든 유형의 전자 메일 템플릿에 자동으로 적용됩니다.

**강의 수강**

* 강의 미리 보기 모드에 있는 리소스 객체가 새 줄에 차례로 나열됩니다. 이전에는 강의 리소스가 한 줄로 옆에 나타나는 경우가 있었습니다.
...
* Adobe ID이 Learning Manager 전자 메일 ID와 일치하는 경우 추가 URL 확인 없이 사용자가 Learning Manager에 로그인할 수 있습니다. 계정에 사용자를 추가하면 조직의 책임자가 Learning Manager 전자 메일 ID를 제공합니다.
...
### 해결된 문제 {#Issuesfixed-15}

**강의 수강**

* 강의 콘텐츠에 접속하면 콘텐츠 재생 막대가 항상 화면에 표시됩니다. 이전에는 콘텐츠 재생 막대가 화면에서 간헐적으로 사라지는 문제가 있었습니다.
* 강의 콘텐츠에 접속하였을 때 사용자에게 콘텐츠를 정말로 종료할지 또는 페이지에 머무를지 묻는 팝업 대화 상자가 나타나고 머무르기를 누르면 사용자가 콘텐츠로 돌아갑니다. 일부 시나리오에서는 사용자가 머무르기 버튼을 클릭해도 강의 콘텐츠가 종료되는 경우가 있었습니다.

**기타 해결 방법**

* 콘텐츠 재생과 관련된 문제가 거의 수정되지 않았습니다.
...
### 해결된 문제 {#Issuesfixed-16}

**강의 수강**

* 학습자가 강의를 이용하는 동안 브라우저의 뒤로 버튼이나 키보드의 백스페이스 키를 사용하여 플레이어를 닫으면 강의에 소요되는 학습 시간이 보고서에 남습니다. 일부 시나리오에서는 이 시간이 보고서에 기록되지 않는 문제가 있었습니다.

**사용자 등록**

* 사용자가 활성화된 자체 등록 SSO를 사용하여 Learning Manager 계정을 등록하면 사용자 목록에 있는 사용자 이름이 기록에 따라 올바르게 표시됩니다. 이전에는 사용자 이름이 잘못 표시되는 경우가 있었습니다.

**모듈을 이용한 강의 생성**

* 작성자가 강의를 복사한 경우에도 복사한 강의에서 리소스 파일을 제거하거나 업데이트할 수 있습니다. 일부 시나리오에서는 사용자가 중복 강의에서 리소스를 업데이트하거나 제거할 때 문제가 있습니다.

**사용자 그룹을 위한 사용자 정의 카탈로그 만들기**
...
### 해결된 문제 {#Issuesfixed-17}

**사용자 로그인**

* 사용자가 활성화 링크를 사용하지 않고 Learning Manager 응용 프로그램에서 로그인을 시도하면 잘못된 형식의 오류 메시지가 나타납니다. 이 문제는 수정되었습니다.
...
### 해결된 문제 {#Issuesfixed-18}

**보고서**

* 책임자 또는 관리자가 학습 이용 시간을 보조 축으로 하여 보고서를 생성하고 관리자 필터를 적용하여 보고서를 저장하면 사용자는 이러한 보고서를 다운로드할 수 없습니다. 모든 종류의 보고서 다운로드가 가능합니다.
...
* 신규 사용자에게 로그인 성공을 위해서는 Adobe ID과 Learning Manager ID를 연동하는 것이 중요하다고 안내 전자 메일로 강조합니다.
...
* 전자 메일 템플릿의 URL 링크와 관련된 문제가 수정되었습니다.
* 다음에 대한 지원 제공:

   * Learning Manager에 게시
   * CP 8 버전에 대한 콘텐츠 업로드 지원 시간이 단축됩니다(CP803 패치 필요).
...
* 500MB가 넘는 Captivate 강의 콘텐츠를 업로드하면 Fluidic Player에서 콘텐츠가 재생되지 않습니다. 이 제한이 사라졌습니다. 현재는 크기 제한이 2GB로 변경되었습니다.

**결제**

* 책임자가 학습자 수를 입력하고 클릭하면 **주문하기,** 사용자당 월간 및 연간 구독 요금에 대한 세부 정보가 포함된 대화 상자가 나타납니다.

### 해결된 문제 {#Issuesfixed-19}
...
* 학습자가 등록 후 받는 환영 전자 메일에 Adobe ID 생성 URL 링크가 제공됩니다.

**사용자 추가**

* 책임자가 Exavault 계정에서 직접 CSV 업로드 옵션을 사용하여 사용자를 추가하는 작업이 일부 고객에게 적용되지 않습니다. 이 문제는 수정되었습니다.

### 해결된 문제 {#Issuesfixed-20}

**학습 프로그램 및 계획**

* 학습자는 여러 학습 계획의 일부로 동일한 학습 프로그램에 자동으로 등록될 수 있습니다. 이전에는 이 워크플로우에 몇 가지 예외가 있었습니다. 이 문제는 수정되었습니다.
...
* Akamai 넷 스토리지 지원으로 인한 사용자 계정 생성 문제가 해결되었습니다.
