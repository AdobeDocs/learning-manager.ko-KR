...
+++업데이트 111: Adobe Learning Manager 2026년 8월 31일 릴리스
...
* **관리자:** 관리자는 내가 저장한 위젯을 페이지에 배치하여 학습자가 책갈피에 추가한 콘텐츠를 쉽게 찾을 수 있도록 합니다. 그러면 학습자는 이전에 저장한 강의로 돌아갈 수 있습니다. 강의를 다시 검색하거나 탐색할 필요가 없습니다. [내가 저장한 위젯](/help/migrated/administrators/feature-summary/experience-builder/add-a-widget.md#bookmark-widget)에 대해 자세히 알아보십시오.

* **학습자:** **내가 저장함** 위젯에는 나중에 책갈피에 추가한 강의, 학습 경로, 인증 및 작업 지원이 표시됩니다. 이 위젯을 사용하면 카탈로그를 다시 검색하지 않고도 한 곳에서 저장된 콘텐츠에 빠르게 액세스할 수 있습니다. [내가 저장한 위젯](/help/migrated/learners/feature-summary/saved-by-me-widget.md)에 대해 자세히 알아보십시오.

**채널 테마 색상:** 채널을 만들 때 더 많은 채널 테마 색상을 선택할 수 있습니다. 테마 색상은 선택 후 밝은 영역 대비를 위해 상황에 맞게 적용됩니다. 채널에 대한 테마 색상을 선택하면 선택한 색상이 채널 인터페이스에 적용되며 채널 축소판 및 텍스트와의 대비를 유지하도록 디자인됩니다. 채널을 만들 때 기본 색상이 자동으로 할당되지만 언제든지 사용 가능한 옵션에서 다른 색상을 선택할 수 있습니다.

**비디오 축소판:** 이제 학습자는 타일로 비디오를 빠르게 식별할 수 있으며, 타일에는 콘텐츠를 한눈에 식별하는 데 도움이 되는 축소판이 표시됩니다. 소스 비디오에서 자체 썸네일 이미지를 제공하지 않는 경우 Adobe Learning Manager이 채널 테마 색상, 추상 패턴 및 비디오 제목을 사용하여 자동으로 썸네일 이미지를 생성합니다.

**채널:** 채널 탐색 옵션은 하나 이상의 채널이 만들어지고, 해당 소스의 비디오를 하나 이상 포함하며, 학습자에게 표시됨으로 표시된 경우에만 학습자에게 표시됩니다. [채널](/help/migrated/administrators/feature-summary/create-channels.md)에 대해 자세히 알아보세요.

## 수정된 버그

**Adobe Experience Manager 구성 요소:** Adobe Learning Manager 참조 사이트 패키지 v2.0.1로 업그레이드한 후 사용자가 알림 벨을 클릭할 때 알림 패널을 로드하지 못했습니다. 로더가 무기한 계속 회전하면서 알림이 표시되지 않았습니다. 이 문제로 인해 /userNotifications 끝점에 대한 호출이 반복되어 결국 HTTP 429 응답이 발생했습니다. 과도한 API 요청을 방지하고 로드 문제를 해결하기 위해 알림 로드 워크플로가 수정되었습니다. 이제 알림이 올바르게 로드되고 불필요한 API 제한 없이 패널이 예상대로 열립니다.

**Adobe Learning Manager 모바일 앱 > 학습 경로:** Adobe Learning Manager 모바일 앱에서 학습 경로 내에 있는 완료 강의의 상태가 완료됨으로 표시되었음에도 불구하고 노란색 표시기와 빨간색 점이 잘못 표시됩니다. 이 시각적 비일관성은 완료된 모듈이 올바르게 표시되는 동안 학습 경로 내 강의에만 발생했습니다. 모바일 앱의 학습 경로 과정에 대한 상태 표시기 로직이 수정되었습니다. 이제 완료한 강의에 올바른 녹색 완료 표시기가 표시됩니다. 이를 통해 모바일 앱과 데스크탑 브라우저 간에 일관된 완료 경험을 보장할 수 있습니다.

+++


+++업데이트 110: Adobe Learning Manager 2026년 8월 16일 릴리스
...
AI 프론트에서 학습자용 AI 어시스턴트는 강의 요약, 학습 객체 비교, Adobe Experience League 답변, Go1 및 LinkedIn 학습 카탈로그에서 서드 파티 콘텐츠 쿼리를 획득하고, 새 학습 경로 상담사가 학습자에게 대화를 안내하여 맞춤형 순차 학습 경로를 자동으로 생성하며, 인사이트 상담사는 자연어 쿼리를 책임자 보고로 가져옵니다. 모든 AI 기능은 청구 페이지의 새로운 Gen AI 크레딧 시스템이 적용되어 관리자에게 기능별 사용 제한 및 소비 경고를 제공합니다.
...
### 외부 학습 기능: 학습자 성적 증명서의 새 열

계정별 외부 학습 사용자 정의 필드를 LT 보고서에 추가합니다. 책임자 LT의 경우 끝에 새 사용자 정의 필드 열이 추가되고, 학습자 LT의 경우 관련 모듈 및 검토자 필드 근처에 외부 학습 이름 및 완료 댓글이 삽입되며, 끝에 나머지 사용자 정의 필드가 추가됩니다.

[Adobe Learning Manager 2026년 8월 릴리스의 변경 내용 보고](/help/migrated/reporting-changes-august-2026.md)에 대해 자세히 알아보십시오.

### 학습자 성적 증명서의 인증에 대한 루트 교육 ID

이렇게 하면 새 루트 교육 ID 열이 책임자 LT 및 학습자 LT 모두에 추가됩니다. 보고서 끝에 표시되는 열은 학습자 기록에 대한 상위 인증 컨텍스트를 식별하는 데 도움이 됩니다.

[Adobe Learning Manager 2026년 8월 릴리스의 변경 내용 보고](/help/migrated/reporting-changes-august-2026.md)에 대해 자세히 알아보십시오.

### 학습자 성적 증명서와 Webhook 날짜-시간 형식 맞춤

Webhook 데이터 개체 날짜-시간 값을 분 정밀도로 표준화합니다. 초는 항상 00으로 표시되며 Webhook 타임스탬프를 LT 보고서 형식에 맞춥니다.

[Adobe Learning Manager 2026년 8월 릴리스의 변경 내용 보고](/help/migrated/reporting-changes-august-2026.md)에 대해 자세히 알아보십시오.

### 학습자 성적 증명서의 가중치 열

Gradebook 지원 강의의 모듈에 대한 LT 보고서에 가중치 열이 추가됩니다. 그러면 보고서 출력에 모듈 가중치가 직접 노출됩니다.

[Adobe Learning Manager 2026년 8월 릴리스의 변경 내용 보고](/help/migrated/reporting-changes-august-2026.md)에 대해 자세히 알아보십시오.

### learningObjects API에서 강의 작성자 세부 정보 공유

세부 정보는 공유 강의에 대한 learningObjects API LO 응답을 업데이트하여 수신 계정에 작성자로 수락하는 관리자가 더 이상 표시되지 않도록 합니다. 원래 외부 작성자 세부 정보만 피어 계정에 노출되며 상위 계정 동작은 변경되지 않습니다.

### AI Orchestrator Agent에서 의도 감지

AI Orchestrator 에이전트가 단일 쿼리 요청에 대한 의도 검색을 오케스트레이터로 이동하고 수동 사용자 인터페이스 토글을 제거합니다. 쿼리 라우팅은 검출된 의도에 따라 Q&amp;A 및 PLP 플로우들 사이에서 변경될 수 있다.

## 릴리스에서 수정된 버그

**학습자 성적 증명서:** 학습자 성적 증명서 보고서의 날짜는 초 동안 00을 기록하는 데 사용되는 반면 Webhook에서는 초가 두 자리 숫자로 표시되었습니다. 이 문제는 수정되었습니다. 이제 학습자 성적 증명서에 두 자리(초)도 올바르게 표시됩니다.

**인스턴스:** Microsoft Teams 또는 Adobe Connect VC 세션이 인스턴스 페이지에서 취소된 경우 페이지를 새로 고친 후에도 일부 연결된 세션 세부 정보가 남아 있습니다. 이 문제는 수정되었으며 취소된 세션은 이제 팀 구성 도우미 및 로비 바이패스 설정과 Connect 기본 강사 등 관련된 모든 세부 정보를 올바르게 제거합니다.

**API:** 자동화된 AWS 워크플로의 문제로 인해 잘못 인코딩된 URL을 사용하기 위해 CMS HTTP API에 대한 반복적인 다운로드 요청이 발생하여 간헐적인 HTTP 500 오류가 발생하고 대량의 잘못된 트래픽이 발생합니다. 작업 과정이 다운로드 요청 URL의 스페이스 및 기타 특수 문자를 URL로 올바르게 URL 인코딩하도록 업데이트되었습니다. 결과적으로 계정 에셋 다운로드 요청이 성공적으로 처리되어 오류가 줄어들고 자동화된 에셋 검색의 안정성이 향상됩니다.

**학습자:** 작성 중에 강의의 세부 개요 섹션에 적용된 이탤릭체 서식이 올바르게 표시되지만 게시 후 학습자 대상 강의 개요 페이지에는 표시되지 않는 문제가 발생했습니다. 렌더링 로직이 업데이트되어 강의 콘텐츠를 게시할 때 이탤릭체 텍스트 서식이 유지됩니다. 따라서 세부 개요 섹션에 적용된 이탤릭체는 학습자가 게시된 강의 개요 페이지에서 올바르게 일관되게 표시됩니다.

**사용자 지정 인증:** SVG 이미지가 성취 인증서의 인증서 배경으로 업로드되면 인증서 미리 보기에 빈 흰색 배경이 표시됩니다. 업로드된 SVG 배경 이미지를 올바르게 지원하고 표시하도록 인증서 미리 보기 렌더링 프로세스가 업데이트되었습니다. 결과적으로 이제 인증서 미리 보기가 구성된 SVG 배경을 예상대로 렌더링하여 최종 인증서를 정확하게 나타냅니다.

**작성자:** 긴 대화형 비디오에 대한 VTT 파일 생성이 상당히 지연될 수 있으며, 일부 업로드에서는 처리 시간이 2시간을 초과합니다. 이 문제는 대형 비디오 파일의 VTT 생성 성능과 안정성을 개선하도록 조사 및 최적화되었습니다. 따라서 이제 VTT 파일이 더 효율적으로 생성되어 대화형 비디오 업로드를 위한 빠르고 일관된 캡션 작업 과정을 제공합니다.

**API:** 완료 데이터가 백엔드에 있고, 지정된 날짜 범위 및 사용자 그룹 필터와 일치하며, 모든 보고서 조건을 충족했는데도 보고서 내보내기 API에서 일부 학습자 완료 레코드가 반환되지 않았습니다. 적격한 완료 기록이 API 내보내기에 일관되게 포함되도록 여러 작업 및 데이터 검색 로직에서 학습자 완료에 대한 보고서 내보내기 프로세스가 개선되었습니다. 보고서 내보내기 API 응답은 완료 타임스탬프가 요청된 날짜 범위 및 필터 범위에 속하는 사용자에 대한 완전한 학습자 완료 데이터를 반환하여 보다 정확하고 안정적인 보고 결과를 제공합니다.

**Webhook:** 학습자가 나중에 취소된 세션에 등록한 경우 동일한 강의의 다른 세션에 등록하면 학습자가 취소된 세션에서 먼저 등록 취소되지 않는 한 새 등록 Webhook 이벤트가 트리거되지 않습니다. 이로 인해 다운스트림 시스템은 취소되었거나 이전에 완료된 인스턴스에 대한 유효한 등록 업데이트를 놓치게 되었습니다. 이전 세션이 취소되거나 완료된 후 학습자가 동일한 강의의 다른 세션에 등록할 때 새 등록 이벤트가 트리거되어 등록 활동이 일관성 있게 보고되도록 Webhook 흐름이 업데이트되었습니다.

**API:** 광범위한 관련 데이터(예: 인스턴스, 등록, 리소스, 등급)를 포함하는 학습 경로에 대한 관리자 API 요청에 대해 상당한 성능 저하가 발생했으며 응답 시간이 일반적인 5초에서 8초에서 45~60초로 증가하여 통합 워크플로 및 다운스트림 시스템에 영향을 미쳤습니다. 성능 최적화는 데이터 검색 효율성을 개선하기 위해 과도한 학습 객체 요청을 포함하도록 구현되었습니다. 복잡한 중첩 관계를 포함하는 학습 경로 API 호출은 이제 결과를 보다 일관되게 반환하고 응답 시간을 향상시켜 안정적인 통합 성능을 보장합니다.

**전자 메일 및 알림:** 반복 인증 강의를 완료한 학습자가 원래 강의에서 완료 전자 메일 템플릿을 비활성화한 경우에도 강의 완료 전자 메일을 받았습니다. 이는 반복 인증이 원래 강의 레벨 알림 설정을 복사하지 않고 새 강의와 인스턴스를 만들어 중복 강의가 기본 전자 메일 구성을 사용하도록 했기 때문에 발생했습니다. 강의를 복제할 때 강의 알림 설정을 유지하도록 되풀이 프로세스가 업데이트되었습니다. 따라서 완료 전자 메일은 이제 원래 강의 구성에서 명시적으로 활성화된 경우에만 전송됩니다.

**학습자:** 비디오로 구성된 발행인란 공지는 학습자 홈페이지의 초기 비디오 프레임에만 표시되며 재생이 예상대로 자동으로 시작되지 않았습니다. 공지가 로드될 때 지원되는 발행인란 비디오가 올바르게 자동 재생되도록 비디오 재생 비헤이비어가 업데이트되었습니다. 이제 학습자는 수동으로 재생할 필요 없이 비디오 기반의 발행인란 공지를 볼 수 있어 더욱 매력적인 경험을 제공할 수 있습니다.

**학습자:** **네트워크 트렌딩** 위젯이 양쪽 가로 행에 **학습 시작** 빈 카드를 잘못 표시했습니다. 이 문제는 각 행에 적절한 빈 상태 카드를 렌더링하여 해결되었습니다. 첫 번째 행에는 이제 **카탈로그로 이동** 링크가 표시되지만 두 번째 행에는 의도한 대로 **학습 시작** 카드가 계속 표시됩니다.

**API:** 학습자가 이름을 기준으로 카탈로그를 오름차순(A-Z) 순서로 정렬하면 공용 API에서 대소문자를 구분하는 정렬을 사용하여 결과를 반환하여 대문자 코스 이름(AA, BB, CC)이 표준 알파벳 순서로 정렬되지 않고 소문자 이름(aa, bb) 앞에 표시됩니다. 대소문자를 구분하지 않는 비교를 사용하도록 정렬 로직이 업데이트되었습니다. 이제 카탈로그 결과가 문자의 대/소문자에 관계없이 예상된 알파벳 순서로 표시되므로 일관된 강의 이름 순서를 유지할 수 있습니다.

**CR-VC:** VC 강의에 새 강사가 추가되었을 때 &quot;강사/주최자/공동 주최자로 추가되었습니다.&quot; 전자 메일 템플릿을 사용하지 않도록 설정한 경우 세션 초대 전자 메일이 전송되지 않아 강사가 세션 세부 정보를 받지 못했습니다. 템플릿과 별도로 강사 세션 초대가 생성되도록 알림 로직이 수정되었습니다.

**클라이언트 공통:** AET(자동 등록 트리거) 학습 계획에 강의를 추가한 후 활동 모듈의 기간이 업데이트되면 학습자 미리 보기는 기본 강의 인스턴스에서 업데이트된 기간 대신 AET 인스턴스에서 업데이트된 기간을 계속 표시합니다. 이 문제는 학습자 미리 보기가 올바른 강의 인스턴스에서 모듈 지속 기간을 검색하도록 하여 수정되었습니다. 이제 학습자 미리 보기에는 강의 콘텐츠에 대한 업데이트를 반영하여 게시된 최신 모듈 지속 기간이 정확하게 표시됩니다.

**학습자:** 순서가 지정된 인증에서 학습자는 실패한 첫 번째 강의를 건너뛰고 필수 조건을 완료하여 잠긴 두 번째 강의에 액세스할 수 있습니다. 그러면 요구 사항이 하나의 강의로 설정되면 인증이 완료로 표시됩니다. 강의 주문 및 규칙 잠금이 일관되게 적용되도록 유효성 검사가 업데이트되었습니다. 이제 학습자는 정의된 순서로만 인증 요구 사항을 완료할 수 있으므로 잠긴 강의로 인해 인증 완료가 이루어지지 않습니다.

**API:** 리소스가 설명 없이 강의에 추가되었을 때 GET /learningObject/{id} API는 리소스가 나중에 업데이트된 경우 새로 추가된 설명을 반환하지 않았습니다. 이로 인해 부실 리소스 메타데이터가 API를 통해 노출되었습니다. 동기화 문제는 수정되었으며, 이제 API는 언제 추가되었는지에 관계없이 최신 리소스 설명을 반환합니다.

**API:** 모듈이 설명과 함께 마이그레이션되고 설명이 나중에 업데이트되면 업데이트된 값이 모듈 테이블에 올바르게 저장되지만 UI에는 반영되지 않습니다. 수정 중에 업데이트되지 않은 content_group 레코드에서 소스를 가져오므로 UI에 이전 설명이 계속 표시됩니다. 이 동기화 문제는 해결되었으며 이제 업데이트된 모듈 설명이 마이그레이션 후 UI에 일관되게 반영됩니다.

**전자 메일 템플릿 및 구성:** 관리자가 전자 메일 템플릿 설정에서 전자 메일 배너를 업데이트하면 새 배너가 Outlook 데스크탑에 올바르게 표시되지만 Outlook Web에는 표시되지 않아 클라이언트 간에 일관되지 않은 전자 메일 환경이 형성되었습니다. 전자 메일 템플릿 렌더링이 업데이트되어 Outlook 플랫폼 간의 배너 호환성이 보장됩니다. 이제 업데이트된 전자 메일 배너가 Outlook 데스크탑 및 Outlook Web 버전 모두에서 올바르게 표시됩니다.

## 릴리스의 알려진 문제

### 강사가 모듈 간 가상 강의실에 참여할 수 없음

강사가 할당된 강의 모듈의 예약된 시간을 초과하는 강의실에 들어가려고 하면 가상 강의실에 참여할 수 없습니다.

Adobe Learning Manager은 과정 내의 모든 모듈에 단일 가상 강의실을 사용합니다. 강사가 룸을 시작하면 세션은 해당 강사의 참여에 연결된 모듈과 연결됩니다
URL입니다. 이후의 모든 조인 요청은 룸과 연결된 활성 모듈과 일치해야 합니다. 다른 강사가 다른 모듈이 활성화된 상태에서 다른 모듈의 조인 링크를 사용하여 참여하려고 하면
룸이 거부되었습니다. 한 번에 하나의 모듈만 공유 룸에서 활성화할 수 있습니다.

이 동작은 현재 릴리스에서 예상됩니다. 활성 모듈과 관계없이 강의 강사가 강의실에 참여할 수 있도록 개선한 내용은 향후 릴리스에서 확인할 예정입니다.

### macOS에서 마이크와 스피커가 다를 때 오디오가 중단됩니다

선택한 마이크와 스피커가 다른 디바이스인 경우 macOS의 가상 강의실 세션 중에 오디오가 간헐적으로 중단되거나 사운드가 왜곡될 수 있습니다.

이 동작은 마이크의 AirPods 및 재생용 내장 스피커와 같이 오디오 입력 및 출력에 다른 장치를 사용하는 경우 발생할 수 있습니다. 각 디바이스에는 고유한 오디오 지연, 에코가 도입되기 때문입니다.
취소는 효과가 떨어지며 배경 노이즈 억제는 때때로 음성 부분을 노이즈로 오인할 수 있습니다. 이로 인해 오디오가 잠시 중단될 수 있습니다. 이 문제는 다음과 같은 경우에 더 두드러집니다.
마이크가 더 멀리 위치할 때와 같이, 스피커의 음성은 낮은 음량으로 캡처된다.

이는 알려진 플랫폼 제한이며 Adobe Learning Manager에만 국한되지 않습니다. 다른 회의 응용 프로그램에서도 유사한 동작이 발견되었습니다. 이 릴리스에는 수정 사항이 없으며 향후 릴리스에 대해 문제를 평가하고 있습니다.

이 비헤이비어를 방지하려면 마이크와 스피커 모두에 동일한 장치를 사용합니다.

### 포커스가 손실된 후 공유 비디오가 Chrome에서 멈춤

강사가 비디오를 재생하는 Chrome 창을 공유한 다음 포커스를 전환하면 참석자가 공유 콘텐츠가 정지하거나 검은색 화면을 표시하는 것을 볼 수 있습니다.

강사를 위해 비디오가 로컬로 계속 재생되지만 공유 창의 포커스가 맞지 않는 동안에는 원격 참석자가 콘텐츠 업데이트를 볼 수 없습니다. 동작은 운영 체제에 따라 다릅니다.

&#x200B;- Windows에서 참석자에게 검은색 화면이 표시됩니다.
&#x200B;- macOS에서 참석자는 마지막으로 표시된 비디오 프레임을 볼 수 있습니다.

포커스가 공유 브라우저 창으로 돌아가면 참석자에 대한 비디오 재생이 일반적으로 다시 시작됩니다.

이는 브라우저 및 운영 체제 제한이며 Adobe Learning Manager에만 국한되지 않습니다. 이 릴리스에 사용할 수 있는 제품별 수정 사항이 없습니다.

비디오 콘텐츠를 표시하는 동안 공유 브라우저 창에 포커스를 유지합니다. 재생 중에 가상 교실 창으로 다시 전환하지 마십시오.

### 강사가 이미 사용 중인 모듈에 참여할 수 없습니다.

강사가 동일한 가상 강의실을 사용하는 두 개의 강의 모듈을 동시에 실행하려고 하면 첫 번째 강사만 정상적으로 강의실에 입장할 수 있습니다.

Adobe Learning Manager은 강의 인스턴스에 단일 공유 가상 강의실을 사용합니다. 첫 번째 강사가 강의실에 참여하면 강의실이 해당 강사 모듈과 연결됩니다. 추가 조인 요청은 다음과 같습니다
활성 모듈에 대해 검증되었습니다. 다른 강사가 다른 모듈 링크를 사용하여 참가를 시도하면 액세스가 거부됩니다. 따라서 동시 강사 주도 세션은 동일한 세션 내에서 수행할 수 없습니다
룸을 공유하는 별도의 모듈을 사용하는 강의 인스턴스

이 동작은 현재 릴리스에서 예상되며 기존 회의실 아키텍처와 일치합니다. 이 기능은 고객 피드백을 기반으로 향후 개선을 위해 검토 중입니다.

### 공유 콘텐츠가 라이브 허브 창으로 돌아온 후 터널링 효과를 표시합니다

강사가 전체 화면을 공유한 후 라이브 허브 창으로 돌아가면 참여자는 공유된 내용에 터널링 효과를 경험할 수 있다. 전체 화면을 공유하면 공유 피드에는 데스크탑에 표시된 모든 내용이 포함됩니다. Live Hub 창으로 다시 전환하면 공유 콘텐츠가 Live Hub 인터페이스를 반복적으로 캡처하고 표시되므로 예기치 않은 시각적 동작이 발생할 수 있습니다. 이는 전체 화면 공유의 알려진 한계이며 향후 릴리스에 대해 평가되고 있습니다.

전체 화면 대신 특정 애플리케이션이나 창을 공유합니다. 예를 들어 참가자가 볼 프레젠테이션이나 문서 창을 공유할 수 있습니다. 단일 응용 프로그램을 공유하면 공유 콘텐츠가 격리된 상태로 유지되며 터널링 효과가 방지됩니다.

### 종료된 세션에 다시 참여한 후 즉시 세션 대시보드 보고서를 사용할 수 없습니다.

종료된 세션에 다시 참여한 후 즉시 세션 대시보드 보고서를 사용할 수 없습니다.
강사가 세션 종료 후 세션 대시보드를 본 다음 URL을 사용하여 동일한 세션에 다시 참여하면 기존 세션 대시보드 보고서를 더 이상 사용할 수 없습니다. 보고서를 새로 고치는 데 약 5~7분이 소요될 수 있으며 이전 세션 인스턴스가 처리되는 동안 다시 사용할 수 있게 됩니다. 알려진 제한 사항이며 향후 릴리스에 대해 평가되고 있습니다.

세션을 닫은 후 5~7분 정도 기다렸다가 대시보드 URL을 다시 엽니다. 이렇게 하면 이전 세션 인스턴스가 처리를 완료할 수 있으며, 그 후 세션 대시보드 보고서를 다시 사용할 수 있게 됩니다.

### 열 이름이 내보낸 강의실 위치 CSV에서 지역화되지 않습니다.

UI 로케일이 영어 이외의 언어로 설정된 경우 강의실 위치 페이지에서 내보낸 CSV 파일에는 선택한 언어가 아닌 영어로 열 이름(머리글 행)이 표시됩니다.

이 동작은 책임자 프로필 > 설정 > 강의실 위치에서 강의실 위치를 내보내는 동안 발생합니다. 파일 내의 위치 데이터가 올바르게 반환되지만 열 헤더는 관리자가 선택한 UI 로케일과 일치하도록 변환되지 않습니다. 따라서 영어 이외의 로케일로 작업하는 책임자는 지역화된 환경에서 영어 열 이름을 볼 수 있습니다.

머리글 행만 영향을 받고 내보낸 파일의 기본 위치 데이터는 영향을 받지 않습니다. 이 릴리스에는 수정 사항이 없으며 향후 릴리스에 대해 문제를 평가하고 있습니다.

### 크레딧 기간 필드 대문자 입력: 9999.99

사용자가 세션 편집 양식에서 9999.99보다 큰 크레딧 기간 값(예: 999999)을 입력하면 값이 저장 시 자동으로 9999.99로 제한되며 검증 오류나 경고가 사용자에게 표시되지 않습니다. 이 필드에는 9999.99로 설정된 HTML 최대 속성이 있습니다. 이 속성은 입력이 거부되거나 수정되었음을 사용자에게 알리지 않고 브라우저 수준에서 이 cap를 적용합니다. 이 동작이 설계된 대로 작동하는지 확인되므로 이 릴리스에 대한 수정 사항이 적용되지 않았습니다.

### 오류 메시지가 CSV 업로드에 누락된 기본 언어 행을 나타내지 않음

구조화된 위치 CSV 업로드에 fr, de와 같이 기본값이 아닌 로케일의 행만 있고 계정의 기본 언어에 행이 없는 경우 CSV 행 1의 로케일이 반환되고, 열 2가 잘못되었거나 지원되지 않는 오류 메시지가 표시됩니다. 그러나 실제 문제가 누락된 기본 언어 행임을 나타내지는 않습니다. 이렇게 하면 사용자가 다중 로케일 CSV를 업로드하고 오류 메시지만으로 문제를 진단할 수 없습니다. 이 릴리스는 수정 사항이 적용되지 않아 이 문제가 삭제되었습니다.

### 인원 제한 필드에서 최대 4,294,967,295의 비현실적인 값을 사용할 수 있습니다.

강의실 위치 모달의 인원 제한 필드는 최대 4,294,967,295명(43억 ~의 값)의 모든 값을 수용하며, 이는 현실적인 비즈니스 제약 조건이 아닌 기본 데이터 유형에 의해 기술적으로 실행되는 한도입니다. 사용자는 비즈니스 수준의 유효성 검사 거부 없이 1,000,000,000과 같이 비현실적인 시트 수를 입력하고 저장할 수 있습니다.
이 릴리스에 대해 수정 사항이 적용되지 않았습니다. 실제 비즈니스 최댓값이 아직 합의되거나 적용되지 않았습니다.

+++

+++업데이트 109: Adobe Learning Manager 2026년 7월 릴리스

출시일: 2026년 7월 16일

## 수정된 버그

**전자 메일 템플릿:** 강의 인스턴스에 등록된 학습자에게 예정된 세션에 대한 미리 알림 전자 메일이 세션 날짜 및 시간 정보 없이 전송되었습니다. 이 문제는 이메일 템플릿에서 세션 예약 세부 정보를 올바르게 채우도록 수정되었습니다. 이제 학습자는 관련 세션 날짜 및 시간이 포함된 예정된 세션 미리 알림 전자 메일을 수신하여 전체 예약 정보를 제공합니다.

**커넥터:** 작성자가 커넥터 기반 강의실/가상 강의실 강의를 복제하거나 편집할 때 간헐적으로 HTTP 400 오류가 발생했습니다. 일단 트리거되면 문제가 지속되어 세션을 새로 고칠 때까지 후속 강의 생성 또는 편집 작업을 차단했습니다. 이 문제는 커넥터 기반 강의의 안정적인 복제 및 편집을 위해 수정되었습니다. 작성자는 이제 반복되는 400개의 오류 없이 강의실/가상 강의실 강의를 복제, 생성 및 수정할 수 있습니다.

**CMS API 요청 인코딩:** 자동화된 플랫폼 워크플로에서 잘못 인코딩된 URL을 사용하여 CMS HTTP API에 대한 반복적인 다운로드 요청을 생성하고 있으므로 HTTP 500 오류가 간헐적으로 발생하고 초당 수천 개의 잘못된 요청이 발생합니다. 요청이 전송되기 전에 공백과 같은 특수 문자가 포함된 에셋 경로가 올바르게 인코딩되도록 URL 인코딩 로직이 수정되었습니다. 이제 NetStorage에 대한 다운로드 요청이 성공적으로 실행되어 반복되는 API 오류와 요청 스파이크가 사라집니다.

**번역 워크플로 복원:** 기본 번역 서비스에서 언어 쌍을 지원하지 않기 때문에 영어에서 노르웨이어 부크말로 번역 요청을 보내지 못해 전체 번역 워크플로가 중단되었습니다. 지원되지 않는 언어 쌍을 적절하게 관리하고 개별 번역 오류를 격리하도록 번역 처리 로직이 향상되었습니다. 지원되지 않는 언어에 대한 번역 요청은 이제 건너뛰고 적절하게 보고되지만 나머지 지원되는 모든 대상 언어에 대한 번역은 계속 성공적으로 처리됩니다.

+++

+++업데이트 108: Adobe Learning Manager 2026년 6월 릴리스
...
**반복 인증에서 획득한 강의의 가용성:** 이전에 획득한 강의는 영구 인증에만 추가할 수 있었습니다. 반복 인증에서는 획득한 강의를 강의 선택 패널에 표시하지 않아 수신자 계정에서 정기 갱신이 필요한 프로그램에 공유 콘텐츠를 사용할 수 없습니다. 획득한 강의는 이제 영구 인증과 반복 인증 모두 사용할 수 있습니다. 자세한 내용은 [받는 사람 응용 프로그램의 공유 카탈로그 기능](/help/migrated/administrators/feature-summary/catalogs.md#sharedcatalogfeaturesinreceiverapplication)을 참조하세요.

**VILT 세션을 Adobe Learning Manager으로 마이그레이션:** Adobe Learning Manager에서는 이제 CSV 파일을 통해 VILT(가상 강사 주도 교육) 세션 데이터의 대량 마이그레이션 및 업데이트를 지원합니다. 이 워크플로를 사용하여 인스턴스 시작 날짜를 구성하고, 학습 경로 인스턴스를 강의 인스턴스와 연결하며, Microsoft Teams, Adobe Connect 및 Zoom에 대한 가상 강의실 세션을 설정합니다. 자세한 내용은 [마이그레이션 설명서](/help/migrated/integration-admin/feature-summary/migration-manual.md#migrationofviltsessiontoalm)를 참조하십시오.

## 이번 릴리스의 개선 사항

**리커트 척도 순서 반전:** Adobe Learning Manager에서 리커트 척도는 학습 개체에 대한 학습자의 경험을 평가하는 데 사용됩니다. 학습자 경험 피드백 양식의 리커트 척도가 표준 왼쪽에서 오른쪽 순서로 역전되었습니다. 이전에는 리커트 척도가 표준적인 왼쪽에서 오른쪽 순서와는 반대로 오른쪽에서 왼쪽 순서였다. 이제 점수는 왼쪽에 [강력하게 반대]로 시작하고 오른쪽에 [강력하게 동의]로 끝납니다.

**강사가 학습자에게 보내는 임시 전자 메일:** 이전에는 강사가 받는 사람의 전자 메일 주소를 복사할 수 있도록 모든 사람에게 전자 메일 보내기 옵션만 활성화했습니다. 이제 강사가 동일한 옵션을 사용하여 학습자에게 특별 전자 메일을 직접 보낼 수 있습니다. [전자 메일 복사] 옵션은 수신자의 전자 메일 주소를 복사하는 경우에만 사용할 수 있습니다.

### 단계

1. 강사로 로그인합니다.
2. 왼쪽 탐색 패널에서 과거 세션으로 이동합니다. 페이지에 과거 세션 목록이 나타납니다.
3. 과거 세션 목록에서 세션 이름을 선택합니다.
...
1. 번역 품질 점수가 낮은 로케일은 VTT 자막 파일을 자동 생성할 때 더 이상 캡션 생성 모달에서 표시되지 않습니다. 이로 인해 부정확한 한자 렌더링 등 품질이 낮은 번역문이 학습자에게 전달되지 않도록 한다.

2. 이제 시작 이메일(내부) 템플릿에는 Adobe ID 및 SSO 로그인 방법에 대한 독립적인 구성이 있습니다. 이전에는 두 로그인 방법 모두 단일 템플릿 채널을 공유하여 템플릿을 비활성화한 후 SSO가 구성되면 템플릿이 다시 활성화되므로 의도하지 않은 환영 전자 메일이 학습자에게 전송되었습니다.

**인증**

반복 인증은 이제 원래(루트) 버전이 아닌 최신(상위) 버전의 인증에서 강의를 가져옵니다. 이렇게 하면 업데이트된 강의 콘텐츠가 각 인증 주기에 올바르게 포함됩니다.

**콘텐츠 라이브러리 및 사용자 지정 역할**

1. 선택한 콘텐츠 폴더로 액세스가 제한된 사용자 정의 작성자는 이제 &quot;사용할 수 없는 콘텐츠&quot; 오류를 받지 않고 학습자로 미리 보기 옵션을 사용하여 AICC 및 Tincan 강의 모듈을 미리 볼 수 있습니다.

2. 선택한 콘텐츠 폴더 및 카탈로그에 대한 액세스 권한이 있는 사용자 정의 작성자는 이제 403 액세스 오류가 발생하지 않고 학습자 보기로 강의 리소스를 열 수 있습니다.

**데이터 및 보고**

데이터 파이프라인에서 데이터베이스 커넥터 오류와 레코드 페이로드를 확인하여 등록 데이터 동기화가 안정되었습니다. 등록 기록은 이제 다운스트림 보고 시스템으로 안정적으로 스트리밍됩니다.

**게시 및 API**

이제 공용 **검색** 및 **범주** API에서 올바른 숫자 형식의 카탈로그 ID를 반환합니다. 이전에는 카탈로그 ID가 예상 숫자 ID 대신 잘못된 형식의 문자열 형식 &quot;catalog123&quot;으로 반환되었습니다.

**UI 및 관리**

1. 관리자 지정 강의의 강의 인스턴스 페이지에 관리자 및 사용자 정의 관리자 사용자에 대한 끊어진 레이아웃이 더 이상 표시되지 않습니다. 래핑 없이 요소가 오버플로우되는 CSS 회귀가 수정되었습니다.

2. 전자 메일에서 VILT 세션 초대 링크를 클릭하는 외부 사용자는 이제 내부 SSO 로그인 페이지로 잘못 라우팅되지 않고 구성된 로그인 방법(예: Adobe ID)으로 올바르게 리디렉션됩니다.

**AI 도우미**

이제 AI 어시스턴트는 인증에 학습 경로를 추가할 수 있는지 묻는 질문에 올바른 응답을 제공합니다. 이전에는 도우미가 부정확한 긍정 답변을 반환했습니다. 이제 도우미는 Adobe Learning Manager의 인증에 강의만 추가할 수 있다고 올바르게 설명합니다.

## 알려진 문제

**QR 기반 강의실 출석을 위해 학습자 성적 증명서에 소요된 시간:** QR 코드를 통해 출석을 자체 표시한 학습자는 계획된 대로 출석한 것으로 확인되어 0분으로 표시되는 데 소요된 시간입니다. 학습자 성적 증명서에 기록된 세션 기간은 완료 시 방출된 xAPI 데이터에서 파생됩니다. 세션 스케줄 데이터가 처리 완료 시점에 데이터베이스에 아직 커밋되지 않은 경우, 0의 지속기간이 기록된다. 이 릴리스에 적용된 수정 사항이 없습니다.

+++

+++업데이트 107: Adobe Learning Manager 2026년 5월 릴리스

출시일: 2026년 5월 26일

## 이번 릴리스의 개선 사항

**인스턴스 시작 날짜:** 관리자 또는 작성자는 인스턴스를 만들거나 편집할 때 인스턴스의 시작 날짜를 포함할 수 있습니다. 시작 날짜는 API를 통해서만 학습자 인터페이스에 표시될 수 있습니다.
...
Adobe Learning Manager의 2026년 4월 릴리스에는 학습자 경험, 관리, 콘텐츠 제작 및 보고 기능이 개선되었으며 현재 Beta 버전에서 일부 기능이 변경될 예정입니다.

**학습자 경험 개선 사항**

* &quot;다음 모듈&quot; 라벨링 및 전용 종료 동작으로 더 나은 강의 흐름을 위해 Fluidic Player 탐색이 개선되었습니다.
* 작업 지원, 체크리스트 및 비디오 캡션까지 확장된 다국어 지원
* 학습 경험에 통합된 AI 어시스턴트를 통해 상황에 맞는 도움을 제공할 수 있다.
* 통합된 목차, 완료 표시기 및 개선된 노트 내보내기로 향상된 Captivate 콘텐츠 경험입니다.
* 일관된 크로스 플랫폼 경험을 위해 LTI를 통한 플레이어 언어 설정 지원.

**관리자 및 작성자 업데이트**

* 이제 Zoom 커넥터가 여러 개의 동시 가상 세션을 지원하여 예약 효율성을 개선합니다.
* API를 통해 모듈 시작 시간 및 학습 개체 만료의 가시성을 제한하는 기능이 추가되었습니다.
* 가중치 적용 점수, 다국어 지원 및 검토자 피드백 옵션으로 향상된 체크리스트 모듈.
* 드래그 앤 드롭 편집기, 동적 필드 및 AI 생성 배경으로 업그레이드된 사용자 정의 인증서입니다.
* 로그인하지 않은 Experience Builder를 도입하여 공개 학습 페이지를 만듭니다.
* 이제 공유 강의에 원본 작성자가 올바르게 표시됩니다.

**강사 기능**

* 세션 등록 및 출석 추적을 위한 QR 코드 생성.
* 체크리스트 평가 중 주석 및 피드백 제공 기능.

**보고 및 분석 개선 사항**

* 여러 퀴즈 시도를 캡처하도록 SCORM 보고가 향상되었습니다.
* 학습자 성적 증명서에서 학습 시간을 보다 정확하게 계산합니다.
* 관리자용 성적 증명서 보고서 업데이트.
* 향상된 고급 검색 기능.

**해당 항목 및 대체 항목**

* 이와 동일한 항목 및 대체 항목을 개선하면 강의를 보다 정확하게 대체하고 매핑할 수 있어 학습 경로의 유연성이 향상됩니다.

**주요 영향**
이러한 업데이트는 AI 지원 학습, 다국어 지원, 간소화된 가상 교육 관리 및 보다 정확한 보고에 중점을 두고 사용성, 확장성 및 자동화를 향상시킵니다.

자세한 내용은 [Adobe Learning Manager 2026년 4월 릴리스의 새로운 기능 및 변경된 기능](/help/migrated/whats-new.md)을 참조하세요.

## 이번 릴리스에서 수정된 버그

**고급 검색:** 카탈로그 액세스가 누락된 경우 고급 검색에서 사용자 등록을 기준으로 LO 액세스를 허용하지 않았습니다. 이제 고급 검색은 어휘 검색과 동일한 등록 기반 액세스 검사를 적용하여 등록 사용자가 카탈로그 액세스 없이 LO에 액세스할 수 있도록 합니다.

**Adobe Experience Manager:** 카탈로그 위젯 제목이 선택한 카탈로그를 유지하지 않았으며 필터링된 카탈로그 대신 모든 학습 개체가 있는 카탈로그 페이지를 열었습니다. 이제 카탈로그 위젯 머리글을 클릭하면 해당 카탈로그가 미리 선택되고 학습 개체가 해당 카탈로그로 올바르게 필터링된 카탈로그 페이지가 열립니다.

**장치 앱 모바일:** 모바일에서 학습자는 순서가 지정된 학습 경로의 첫 번째 섹션에 있는 모든 항목을 완료할 수 있지만 완료한 후에도 두 번째 섹션의 잠금을 해제하지 못했습니다. 모바일에서 섹션의 모든 항목이 완료되면 순서가 지정된 학습 경로의 다음 섹션이 학습자에게 올바르게 잠금 해제됩니다.

**LMS 마이그레이션:** 마이그레이션을 통해 생성된 강의 인스턴스에 영숫자 인스턴스 ID가 있는 경우, 마이그레이션을 다시 실행하여 VC 세션을 업데이트하면 기존 세션이 업데이트되지 않고 새 세션이 생성됩니다. 이제 마이그레이션이 중복 세션을 만드는 대신 영숫자 ID를 사용하여 강의 인스턴스에 대한 기존 VC 세션을 올바르게 업데이트합니다.

**강의 기간:** VILT 강의의 경우, 기본 인스턴스의 정적 LO_DURATION에서 강의 레벨 기간을 항상 가져왔습니다. 이는 세션이 중지되고 실제 인스턴스 모듈에서 다시 계산되지 않을 때 0이 되었습니다. 이제 VILT 강의 지속 시간은 기본 인스턴스에서 오래된 LO_DURATION 값 또는 0에만 의존하는 대신 실제 인스턴스 모듈을 반영합니다.

**커넥터:** Ember 모듈 세부 정보 컨트롤러가 모듈 간에 다시 사용되었기 때문에 vcHostingSystem이 일관되게 업데이트되지 않아 VC/F2F 동작이 혼합되어 vcHostingSystem만 기반으로 instructorId를 잘못 지웁니다. 이제 모듈 처리로 vcHostingSystem이 일관되게 유지되고 실제 VC 세션에 대해서만 instructorId가 지워지므로, 변환된 F2F 세션에 대해 강사가 올바르게 업데이트됩니다.

**LMS 마이그레이션:** 과정 순서에 대한 LearningProgramCourse CSV 열이 백엔드에서 인식되지 않으므로 orderEnforced가 true인 경우에도 LP 섹션의 과정 순서가 무시됩니다. 백엔드 동작과 일치하고 CSV를 통한 미지원 주문이 암시되지 않도록 강의 주문에 대해 사용되지 않은 CSV 열이 제거되었습니다.

**성능:** AWS 자동화 워크플로에서 인코딩되지 않은 공간이 있는 잘못 인코딩된 다운로드 요청을 Akamai CMS HTTP API로 반복적으로 전송하여 500개의 오류가 발생하고 잘못된 트래픽이 과도하게 발생합니다. 이제 자동화된 워크플로우에서 다운로드 URL의 공간을 URL로 올바르게 인코딩하여 유효하지 않은 요청을 방지하고 연관된 500개의 오류 스파이크를 제거합니다.

**등록 및 등록 취소:** 학습 프로그램, 인증 및 강의에 대한 일부 관리자 등록 플로우에서 등록 소스가 ADMIN_ENROLL 대신 SELF_ENROLL으로 잘못 기록되었습니다. 이제 관리 중심 등록은 영향을 받는 플로우 전반에서 소스 ADMIN_ENROLL으로 일관되게 저장됩니다.

**등록 및 등록 취소:** 반복 인증 체인에 임계값을 기준으로 등록하는 경우 학습자가 이전 반복을 완료하지 않았더라도 나중에 다시 반복에 등록되었습니다. 이제 임계값 기반 등록은 되풀이 체인을 존중하고, 학습자가 필요한 이전 되풀이를 완료하지 않는 한 이후 인증에 등록되지 않습니다.

**관리자 설정:** 관리자가 하위 도메인이 문자로 시작해야 하는 유효성 검사 규칙 때문에 숫자로 시작된 피어 계정을 추가할 수 없습니다. 숫자로 시작하는 하위 도메인이 있는 피어 계정은 허용된 문자 규칙을 충족하는 한 이제 수락됩니다.

**과정 - 퀴즈 점수:** 헤드리스 실행에서 책임자가 퀴즈 모듈을 재설정한 후 플레이어에서 실제 3개 질문 대신 총 12개의 잘못된 질문을 표시했습니다. 이제 헤드리스 실행에서 모듈이 재설정된 후 퀴즈 플레이어에 올바른 총 질문 수가 표시됩니다.

**작성자:** 콘텐츠 그룹이 SCORM에서 CAPI로 업데이트될 때 보고 정보 및 완료 조건이 새로 고쳐지지 않고 콘텐츠 그룹은 완료 조건 없이 유지됩니다. 콘텐츠 그룹을 SCORM에서 CAPI로 업데이트하면 이제 보고 정보가 올바르게 업데이트되고 콘텐츠 그룹에 대한 적절한 완료 조건이 설정됩니다.

**장치 앱 모바일:** Captivate Prime 모바일 앱에서 target=&quot;_blank&quot;를 통해 새 탭에서 열린 다운로드 가능한 리소스가 탭했을 때 다운로드되지 않았습니다. 모바일 앱에서 다운로드 가능한 리소스를 탭하면 이제 콘텐츠가 target=&quot;_blank&quot; 링크를 사용하는 경우에도 다운로드가 올바르게 시작됩니다.

**서식 있는 텍스트 편집기:** 서식 있는 텍스트 LO 설명에서 줄바꿈이 작성자 보기에서 제거되었지만 학습자는 줄바꿈을 계속 볼 수 있습니다. 이제 서식 있는 텍스트 LO 설명이 작성자 및 학습자 보기 모두에서 줄바꿈을 올바르게 유지하고 표시합니다.

## 알려진 문제/디자인

### 두 번째 자연 자원에 대한 버전 관리 동작

Adobe Learning Manager(ALM)는 편집 시나리오 중에 두 번째 자연(SN) 리소스의 버전을 증가시키지 않습니다. 이 동작은 ALM이 작성 선택이 아닌 SN 콘텐츠 업데이트를 처리하는 방식을 반영합니다.

Second Nature는 소스 시스템에서 버전 관리를 지원하지 않습니다. 작성자가 SN 포털에서 시나리오 데이터를 업데이트하면 ALM 리소스 및 연결된 학습 개체(LO)는 새 버전을 만들지 않고도 업데이트된 콘텐츠를 자동으로 참조합니다.

이에 비해 ALM은 파일 기반 및 LTI 기반 리소스에 대한 버전 제어를 지원하므로, 작성자가 파일을 바꾸거나 시작 링크를 업데이트할 수 있습니다. 이러한 컨트롤은 SN 통합에 사용할 수 없으므로 ALM은 Second Nature 콘텐츠에 대한 버전 범핑을 지원하지 않습니다.


+++


+++업데이트 104: Adobe Learning Manager의 학습자용 AI 도우미

출시일: 2026년 3월 2일

## 학습자용 AI 지원

자연어 쿼리를 사용하여 응답, 리소스 및 관련 강의를 즉시 찾을 수 있도록 설계된 GenAI 기반 대화 도구입니다.

**주요 하이라이트**

* RAG(Retrieval-Augmented Generation)를 사용하여 강의 콘텐츠, 작업 지원 및 학습 자료 전반을 지능적으로 검색하여 정확하고 상황에 맞는 응답을 제공합니다.
* 맥락을 이해하고, 이전의 상호 작용을 유지하며, 진리의 근원으로서 인용 기반의 답변을 제공하므로, 이는 필요에 따라 나중에 사용할 수 있습니다.
* 학습자가 개인별 학습 요구 사항에 맞게 개인화된 스크립트, 토킹 포인트 및 요약을 요청할 수 있습니다.
* PDF, DOCX, PPTX, 오디오, 비디오 및 SCORM 1.2 및 2004를 포함하여 지원되는 모든 콘텐츠 형식을 검색합니다.

자세한 내용은 [학습자용 AI 도우미](/help/migrated/learners/feature-summary/learner-ai-assistant.md)를 참조하십시오.

+++

+++업데이트 103: Adobe Learning Manager 2025년 11월 릴리스
...
이제 모든 Adobe Learning Manager 계정에서 Experience Builder를 사용할 수 있으므로 고객이 로우 코드 구성 요소와 유연한 레이아웃을 사용하여 완전히 브랜딩된 학습 경험을 만들 수 있습니다. 관리자는 Experience Builder를 사용하여 완전히 모바일 반응형 맞춤형 학습 포털 및 페이지를 만들 수 있습니다.

기존 계정의 경우 Experience Builder 업데이트가 자동으로 실행됩니다. 이전 UI 또는 클래식 UI를 계속 사용하는 계정에는 Experience Builder 기능을 사용할 수 없습니다.

자세한 내용은 [Experience Builder](/help/migrated/administrators/feature-summary/experience-builder/overview.md)를 참조하세요.

### Adobe Learning Manager 모바일 웹 인터페이스

모바일 웹 인터페이스는 최신 코드베이스에 재구축되어 로드 시간이 단축되고 응답성이 향상되었으며 여러 장치에서 보다 직관적인 학습자 경험을 제공합니다.

업데이트된 모바일 웹 경험은 기본적으로 모든 새 계정에 대해 활성화됩니다. 또한 기존 계정은 소셜 학습이 활성화된 경우를 제외하고 자동으로 업그레이드를 받습니다. 이러한 경우 소셜 학습이 새로운 호환 가능한 프레임워크로 마이그레이션될 때까지 이전 모바일 경험이 활성 상태로 유지됩니다.

>[!NOTE]
>
>새로운 Adobe Learning Manager 모바일 웹 인터페이스에서는 소셜 학습이 지원되지 않습니다.

### Fluidic Player 개선 사항

이전에는 Fluidic Player의 다음 단추에 예정된 모듈의 제목이 표시되지 않아 학습자가 학습 과정에서 다음 단계를 예측하기 어려웠습니다.

향상된 기능을 통해 이제 다음 버튼에 다음 모듈의 제목이 포함되어 학습자는 다음을 수행할 수 있습니다.

* 계속하기 전에 콘텐츠 내용이 어떻게 표시되는지 파악합니다.
* 더욱 명확하고 신뢰할 수 있는 모듈 탐색.
* 과정 흐름 전체에 걸쳐 보다 나은 컨텍스트를 유지합니다.

![](assets/fluidic-player-next.png)
_이제 Fluidic Player에 예정된 모듈의 제목이 있는 다음 단추가 표시됩니다_

### L1 피드백 양식

L1 피드백은 개별 학습 개체 내에서만 구성할 수 있으므로 유연성이 제한되고 관리자가 카탈로그에서 다른 양식을 사용할 수 없습니다. 이제 관리자는 여러 L1 피드백 양식을 만들고 사용할 수 있으며 각 피드백 양식을 특정 강의, 학습 경로 및 인증에 할당할 수 있습니다. 이 양식은 이제 강의 효과, NPS, 리커트 척도 및 자유 텍스트 질문과 같은 여러 질문 유형을 지원합니다.

피드백 양식은 모든 로케일에서 동일한 질문 구조를 유지하면서 추가 언어로 번역할 수 있습니다. 이제 보고서에는 각 응답을 캡처한 양식을 식별하는 데 도움이 되는 피드백 양식 이름 및 피드백 양식 버전 이 포함됩니다.

**사용 사례**

* 맞춤형 피드백 양식을 다양한 교육 프로그램에 할당합니다. 예를 들어, 영업 강의에서는 NPS 중심 질문을 사용할 수 있고, 규정 준수 강의에서는 강의 효과 및 리커트 규모 질문을 사용하여 명확성과 정책 이해도를 측정할 수 있습니다.
* 학습 모듈에 따라 별도의 피드백 양식을 사용합니다. 예를 들어, 혼합 모듈, 자가 진행식 모듈 및 강의실 모듈의 세 가지 형태가 있습니다.

자세한 내용은 [L1 피드백 양식](/help/migrated/administrators/feature-summary/l1-feedback-form.md)을 참조하세요.
...
* 학습자가 수강한 교육에 따라 &#39;이 교육 평가&#39; 문자열의 이름을 &#39;이 강의 평가&#39;, &#39;이 학습 경로 평가&#39; 또는 &#39;이 인증 평가&#39;로 변경했습니다. 교육 유형에 따라 학습자는 문자열을 적절히 볼 수 있습니다.
...
* 많은 수의 레코드가 있는 CSV를 처리 중인 경우 상당한 시간이 소요되며, 이 기간 동안 사용자가 로그인하여 속성 중 하나의 값을 입력하면 CSV 오류가 발생할 수 있는 새 사용자 그룹이 생성될 수 있습니다. 이 문제를 해결하려면 CSV 가져오기가 진행 중일 때 활성 필드 속성 팝업 메시지가 비활성화되고 CSV 업로드가 완료되면 다시 활성화됩니다.
...
* 다른 컴퓨터에서 앱이 제거되면 활동 탭에 알림이 표시되지 않습니다.
...
학습자 일괄 등록을 수행하면 각 학습자에 대해 로그 파일이 생성되지 않습니다.
...
* GetNotificationData를 GET에서 POST로 변경합니다. 원래 구현에서 **IllegalArgumentException: 요청 헤더가 너무 큼** 오류가 발생하여 알림이 실패했습니다.
+++
...
이 업데이트에서는 계정의 모든 작업 지원 목록을 나열하는 새로운 작업 지원 보고서를 제공합니다.
...
* 새로운 열( Completed by )이 LT 보고서에 추가되었습니다. 이를 통해 관리자가 LO 소스 완료를 식별할 수 있습니다.
...
**참고:** 이 Adobe Learning Manager 릴리스에서는 비활성 계정이 있는 사용자가 더 이상 하위 도메인을 사용하여 계정에 액세스할 수 없습니다. 해당 계정은 계정 id 또는 acapindex.html 페이지를 사용하거나 이메일 id를 입력하여 액세스할 수 있습니다.
...
* 사용자 정의 역할 권한의 사용자가 비공개 컨텐츠 폴더에서 컨텐츠 파일을 검색할 수 없었습니다. 이 문제가 수정되었습니다.
...
* 이전에는 가상 교실에서 VC 회의 시스템을 선택하지 않고도 모듈을 생성할 수 있어 새 인스턴스의 VC URL에 필수 정보가 없었습니다. 이제 모듈을 저장하기 전에 모듈 생성 단계에 VC 회의 시스템을 지정하도록 요청하는 오류 메시지가 표시되므로 이 문제를 방지할 수 있습니다.
...
* 책임자의 인터페이스 및 컨텐츠 언어가 독일어 또는 스페인어로 설정된 경우 공지 시작 날짜와 종료 날짜가 저장되지 않았습니다. 이 문제는 수정되었습니다.
...
* 외부 학습자가 같은 계정의 내부 학습자로 변환되면 학습자 앱에서 게임화 리더보드에 액세스할 수 없었습니다. 이 문제는 수정되었습니다.
...
>[!NOTE]
>
>학습 경로의 확장된 기능을 활성화할 수 있는 옵션이 **설정 > 일반** 페이지에 새로 추가되었습니다. 이 옵션이 활성화되면 다른 학습 경로에서 학습 경로를 추가할 수 있습니다. 일단 활성화된 옵션은 변경할 수 없습니다.

학습 경로는 학습 프로그램의 기존 기능을 대체합니다. 학습 프로그램이 기존의 기능성을 고스란히 유지한 채 더 유용한 기능을 갖추게 된 셈입니다. 또한 이 기능은 학습 경로라는 이름으로 새롭게 태어났습니다.
...
* CSV에 정의된 최신 제품 용어는 강의 등록 취소, 학습 프로그램 등록 취소 또는 인증 등록 취소 같은 전자 메일 템플릿에 반영되지 않습니다. 이제 전자 메일 템플릿의 기본 텍스트가 사용자 정의 용어를 지원합니다.
* Learning Manager에 게시 워크플로에서 Learning Manager의 사용자 언어가 지원되지 않습니다. 사용자 언어가 영어 이외의 언어로 설정된 경우에도 &#39;Learning Manager에 게시&#39;는 영어로 진행됩니다.
...
* 관리자 역할을 한 사용자에서 다른 사용자로 변경할 때 이전 사용자의 관리자 역할이 여전히 UI에 반영되는 경우가 있었습니다. 해당 문제는 수정되었습니다. 이 문제는 내부 사용자에게만 나타나고, 외부 사용자에게는 나타나지 않았습니다.
...
* 사용자가 AEM 사용자 노드를 사용하지 않고도 자체 스토리지 백엔드를 구현할 수 있도록 사용자 전자 메일/토큰 가져오기 및 저장이 외부화되었습니다.
...
* **콘텐츠 마켓플레이스:** Learning Manager에서는 이제 기술, 관리, 리더십 등과 같이 다양한 도메인에서 70,000개 이상의 강의를 제공합니다.
* **접근성 지원 개선:** 학습자 역할을 위한 접근성 지원은 향상된 키보드 탐색, 화면 판독기 기능과 대비율 준수를 통해 강화됩니다.
* **서식 있는 텍스트 서식:** Learning Manager에서는 이제 강의, 프로그램, 인증 및 작업 지원의 설명에 관한 서식 있는 텍스트 편집을 제공합니다. 이것을 통해 작성자는 일반 텍스트가 아닌 하이퍼링크, 이미지 및 기타 텍스트 서식 옵션과 같은 서식 있는 텍스트에 설명을 지정할 수 있습니다.
* **평가 등급:** 학습자는 이제 5점 척도로 강의를 평가할 수 있습니다. 책임자는 기존 효과 등급 또는 별 5개 등급 중에서 하나를 선택할 수 있습니다.
* **Badgr 통합:** 학습자는 이제 Learning Manager에서 획득한 배지를 각자의 소셜 네트워크에 공유할 수 있는 Badgr 계정으로 배지를 자동 푸시하도록 Learning Manager를 승인할 수 있습니다.
...
* 책임자와 관리자가 승인 보류 상태의 강의를 확인할 수 있습니다. 따라서 관리자가 직원의 학습 및 교육 상황을 추적할 수 있고, Learning Manager 책임자는 필요에 따라 강의 등록을 승인할 수 있습니다.
...
* userGroupInfo API는 모든 앱에서 학습자 프로그램을 실행하는 동안에는 표시되지 않습니다.
...
* 다중 VC 공급업체 통합을 설정했으나 작동하지 않는 경우 VC 선택 드롭다운이 빈 목록으로 표시됩니다. 이 문제는 수정되었습니다. 남은 VC 통합이 이제 올바르게 나열됩니다.
...
최신 2020년 8월 릴리스에서 Learning Manager를 사용하면 책임자가 모든 질문이 필수 질문이 되도록 L1 피드백을 구성할 수 있습니다. 이제 모바일 앱의 학습자 원근감에서 동일하게 지원됩니다.
...
* 사용자 정의 관리자는 대시보드 보고서에서 학습 프로그램 또는 인증을 검색할 수 없습니다.
* 사용자 정의 관리자는 대시보드에서 관리자를 검색할 수 없습니다.
...
이번 업데이트에서는 학습 객체의 배너 이미지를 변경할 수 있습니다.
...
관리자는 카탈로그 및 사용자 그룹을 기반으로 사용자 정의 역할을 제한할 수 있습니다. 이러한 역할에 속하는 모든 사용자는 해당 범위의 카탈로그에서 학습 객체만 볼 수 있습니다. 해당 사용자는 자신의 사용자 그룹 범위에서 정의된 작업만 수행할 수 있습니다.
...
* **보고서 추가** 대화상자에서 **시간 범위** 및&#x200B;**필터** 선택기가 별도 섹션으로 나타나며 기본적으로 접힌 상태로 표시됩니다. 자세한 내용은 [***보고서 만들기***](../administrators/feature-summary/reports.md#report)&#x200B;를 참조하십시오.
...
학습자 성적 증명서의 시간 열에서 분은 가장 가까운 분으로 반올림되며 초 값은 00입니다. 자세한 내용은 [***시간 열***](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)&#x200B;을 참조하십시오.

### 이번 릴리스에서 수정된 문제 {#Issuesfixedinthisrelease-3}

**학습자 대시보드**

* 이전에는 관리자가 아직 등록을 승인하지 않은 경우에도 학습 일정에 상태가 **세션 등록됨**&#x200B;으로 표시되었습니다. 이제 관리자가 등록을 승인할 때까지 학습자에게 상태가 **보류 중**&#x200B;으로 올바르게 표시됩니다.
...
사진, 문서 또는 오디오나 비디오 파일을 게시판에 공유하면 다른 게시판 팀원이 게시물을 보고 상호 작용을 시작할 수 있습니다.  자세한 내용은 [***게시물 공유***](../learners/feature-summary/ipad-android-tablet-users.md#socialmobile)&#x200B;를 참조하십시오.
...
소셜 학습을 사용하면 학습자가 게시한 콘텐츠를 **큐레이션 안 함** 및 **수동 큐레이션**&#x200B;의 두 가지 방식으로 큐레이션할 수 있습니다. 이번 릴리스에서 Adobe Learning Manager는 AI 지원 자동 큐레이션 기능을 제공하여 소셜 학습 성능을 높여줍니다. 콘텐츠가 게시되면 해당 콘텐츠를 분석하여 게시된 목적의 스킬에 속하는지 여부를 파악합니다. 신뢰 점수를 기반으로 콘텐츠가 실시간으로 게시되거나 수동 큐레이션을 위해 전송됩니다. 자세한 내용은 *[**&#x200B;자동 지원 큐레이션&#x200B;**](../administrators/feature-summary/social-learning-configurations-as-an-admin.md#autocuration)**.***을 참조하세요.

**스킬 도메인으로 스킬 매핑**

Learning Manager LMS에 있는 스킬 도메인으로 계정의 스킬을 매핑하십시오. Learning Manager가 자동 지원 큐레이션을 지원하는 스킬 도메인과 계정 스킬을 연결하는 데 도움이 됩니다. 자세한 내용은 [***도메인으로 스킬 매핑***](../administrators/feature-summary/curation-skills.md)&#x200B;을 참조하십시오.
...
* 관리자가 **이번 분기** 옵션을 선택하면 학습 객체의 등록, 진행, 완료 세부 정보가 표시되지 않았습니다. 이번 릴리스에서는 이러한 세부 정보가 예상대로 표시됩니다.
...
* 이제 강사가 검토 보류 중인 세션 및 모듈을 쉽게 식별할 수 있습니다. 자세한 내용은 [***보류 중인 검토***](../instructors/feature-summary/learners.md#pending)&#x200B;를 참조하십시오.

* 이제 스킬에서 소수점 형식으로 점수를 수여할 수 있습니다. 이를 통해 작성자는 특정 강의에 점수를 소수점 수준으로 수여할 수 있습니다. 자세한 내용은 [***소수점 지원***](../administrators/feature-summary/skills-levels.md#decimal)&#x200B;을 참조하십시오.
* 사용자 정의 역할 생성을 자동화합니다. 자세한 내용은 [***CSV 파일을 통해 역할 구성***](../integration-admin/feature-summary/configure-role-csv-files.md)&#x200B;을 참조하십시오.
* 이제 외부 인증에 필요한 제출 및 활동 모듈이 선택 사항입니다. 따라서 관리자와 강사가 제출 없이 평가를 수행할 수 있습니다. 자세한 내용은 [***선택적 제출***](../managers/feature-summary/learning-objects.md#optional)&#x200B;을 참조하십시오.
* 삭제된 사용자의 학습자 성적 증명서 다운로드 자세한 내용은 [***학습자 성적 증명서***](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)&#x200B;를 참조하십시오.
* 다음 언어 지원:
...
* **QR 코드 스캔으로 등록:** QR 코드 기능이 향상되었습니다. QR 코드를 이용한 출결 표시 지원은 물론 이제 QR 코드를 이용한 강의 등록 및 완료를 지원합니다.
  ...
  강의를 등록 및 완료하려면 관리자가 제공하는 QR 코드를 스캔하면 됩니다. <!-- For more information on scanning QR codes in the web version of Learning Manager, see  [***Scan QR code***](https://helpx.adobe.com/captivate-Learning>Manager/whats-new.html#QRcodetoenrollcompleteenrollcompleteacourse) -->.

* **강의에서 다중 시도:** Learning Manager 앱에서 학습자는 다중 시도를 통해 강의를 수료할 수 있습니다. <!-- For more information on setting up multiple attempts, see  [***Multiple attempts***](<https://helpx.adobe.com/captivate-Learning>Manager/authors/feature-summary/courses.html#Multiattempts). -->
...
* 사용자 그룹에서 사용자를 이동할 때 사용자 등록에 문제가 생겼습니다. 이 문제는 수정되었습니다.
...
* 관리자 응용 프로그램과 데이터 API 목록에는 표시되지만 대시보드 보고서 드롭다운에 인증이 나타나지 않습니다. 이 문제는 수정되었습니다.
...
* 사용자 알림이 실패한 경우에 이와 관련된 전자 메일을 받지 못하였습니다. 이 문제는 수정되었습니다.
...
* 등록/완료와 같은 활동 정보는 관리자가 다운로드받은 학습자 성적 증명서에 나타나지 않았습니다. 이 문제는 수정되었습니다.
...
* 이번 Adobe Learning Manager 릴리스에서는 새로운 기능과 개선 사항을 발표합니다. 사용자 정의 역할 생성, 카탈로그 레이블 추가, 사용자 추출, 태그 관리, 학습 객체 이름 변경, Slack 통합, 새로운 커넥터 통합, xAPI 지원 등을 수행할 수 있습니다. 새로운 기능 및 개선 사항에 대한 자세한 내용은 [새로운 기능 요약](../whats-new.md#main-pars_text)을 참조하세요.
...
* 학습자를 검색 및 선택하여 출석 표시하면 Learning Manager에서 모든 학습자에게 출석을 표시할 수 있었습니다. 이 문제는 수정되었습니다.
* Learning Manager에서는 24시간 형식으로 전자 메일 시간을 표시하였습니다. 이 문제는 수정되었습니다. 지금은 12시간 형식으로 시간을 표시합니다.
...
* 강사는 학습 프로그램/인증의 일부인 세션에서 한 번에 모든 사용자에 대한 출석을 표시할 수 없었습니다. 이 문제는 수정되었습니다.
...
* 모바일 앱 사용자는 L1 피드백을 푸시 알림으로 수신하지 못합니다. 이 문제는 수정되었습니다.
...
* 원본 계정의 작성자가 강좌를 자동으로 저장하여도 변경된 내용이 공유된 계정에 전달되지 않습니다. 이 문제는 수정되었습니다.
...
* 알림이 없는 전자 메일 등 Learning Manager의 특정 전자 메일이 사용자에게 전달되지 않습니다. 예시- 외부 프로필 생성과 활성화 관련하여 받은 전자 메일이나 연결 계정 구성 관련하여 받은 전자 메일. 이 문제는 수정되었습니다.
...
* 경우에 따라 카탈로그에 학습 프로그램을 추가한 후에도 추가 옵션이 여전히 활성화할 수 있도록 설정되어 있습니다. 이 문제는 수정되었습니다.
* 디바이스 브라우저에서 Learning Manager를 열면 디바이스 앱에서 Learning Manager를 경험할 수 있는 옵션이 표시됩니다. 예를 클릭하면 앱이 설치되어 있지 않은 경우 Play Store(Android)를 시작하고, (Android 및 iOS에서) 설치되어 있는 경우 앱을 실행해야 합니다. 위 작업은 문제가 발생하여 수정되었습니다.
...
* 목차에서 클릭하면 자동으로 닫히지 않을 수도 있으며, 내용을 보려면 수동으로 창을 닫아야 했습니다. 이 문제는 수정되었습니다.
...
* 사용자가 작업 지원을 다운로드하지 않으면 오프라인 모드에서 작업 지원 탭을 클릭하였을 때 iOS 앱이 충돌합니다. Adroid에서는 데이터를 로드하는 동안 오류가 발생했다는 메시지가 표시됩니다. 이 문제는 수정되었습니다.
...
* 특정 경우에 작업 지원 탭에서 이미 추가된 작업 보조 장치를 제거하려고 하면 오류 메시지가 표시됩니다. 이 문제는 수정되었습니다.
...
작업 지원은 강의 또는 학습 프로그램과 같은 특정 학습 대상에 등록하지 않고도 학습자가 접속할 수 있는 교육 콘텐트입니다. 이 기능을 통해 관리자는 작업 지원 보고서를 추출하고 다운로드할 수 있습니다. 책임자는 발송한 모든 공지사항에 대한 보고서를 생성할 수도 있습니다. 책임자와 관리자는 등록되지 않은 학습자의 보고서도 추출할 수 있습니다.

**Learning Manager 커넥터**

이제 데이터 내보내기 옵션을 사용하여 사용자 스킬을 FTP 위치로 내보내어 타사 시스템과 통합할 수 있습니다. 통합에 대한 연결 이름을 지정할 수 있습니다. 그리고 내부 사용자를 가져올지 또는 필요에 따라 사용자 스킬을 구성하거나 가져와서 내보낼지를 선택할 수 있습니다.
...
사용자가 학습 프로그램, 강의실이 있는 인증서 및 연결 세션에 등록하여도 일정 초대 파일(.ics)을 받지 못하는 경우가 있었습니다. 이 문제는 수정되었습니다. 이제 사용자는 등록 전자 메일과 함께 세션 세부사항을 안내하는 .ics 파일을 받을 것입니다.
...
이제 책임자나 작성자는 학습 계획에 학습 객체를 여러 개 부여할 수 있습니다. 학습 객체의 인스턴스 중 하나가 만료되면 전체 학습 계획이 자동으로 비활성화됩니다. 향상된 기능을 통해 학습 객체 고유 ID를 사용하여 **[!UICONTROL 완료한 학습]**&#x200B;과 **[!UICONTROL 학습 할당]** 영역을 검색할 수도 있습니다.
...
* 경우에 따라 사용자가 숨겨진 프레임/슬라이드가 있는 모듈 콘텐츠를 업로드할 때 왼쪽 창의 목차에서 숨겨진 프레임/슬라이드가 나타났습니다. 이 문제는 수정되었습니다.
...
* 책임자가 학습자의 강의 효과를 활성화하지 않아도 학습자 역할에서 강의 효과를 **분류** 필터 옵션 중 하나로 표시합니다. 이 문제는 수정되었습니다.
...
* 책임자가 모든 강의를 가져오려고 하면 강의 초안도 반환됩니다.이 강의 초안은 작성자 개인의 것으로 되어 있습니다. 이 문제는 수정되었으며, 이제 강의 초안은 반환되지 않습니다.
...
또한, 다음과 같은 추가 기능으로 기존 사용자 모델을 개선하였습니다.
...
* 작성자가 AIC 또는 Tin Can 콘텐츠로 강의를 만들고 내용을 미리 보려 하면 콘텐츠가 재생되지 않습니다. 이 문제는 수정되었습니다.
...
Learning Manager는 조직의 Learning Manager 학습자 성적 증명서를 생성하는 기능을 제공합니다. 자세한 내용은 [학습자 성적 증명서 기능 도움말 내용](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md)을 참조하십시오.
...
* Learning Manager에 외부 사용자 그룹을 추가한 후, 외부 사용자 등록 프로세스를 일시 중지할 수도 있습니다. 언제든지 &#39;다시 시작&#39; 옵션을 사용하여 차단(일시 중지)을 취소할 수 있습니다.
...
* 강의, 학습 프로그램 또는 인증 보고서를 구독할 수 있습니다. 또한, 퀴즈 점수 및 학습자 상태와 같은 데이터가 있는 개별 강의 보고서도 구독할 수 있습니다. 구독한 내용은 Learning Manager 계정에 등록된 전자 메일 ID로 받을 수 있습니다. 전자 메일 ID 변경도 가능합니다.
* 인증 등록 보고서를 내보내면 **예정 날짜**&#x200B;라는 이름의 새 열도 같이 내보냅니다. 이 열 데이터를 통해 관리자는 학습 객체 이용 마감 시간을 놓친 학습자를 알 수 있습니다.
...
* 제목에 아포스트로피(&#39;)가 포함된 프리젠터 강의를 Learning Manager에서 게시하고 시청하면 물음표(???)가 표시됩니다 목차에 나타납니다. 이 문제는 수정되었습니다.

**인증**

* 인증이 다시 실행될 때 카탈로그와 연결되어 있으면 관련된 모든 카탈로그에 인증이 표시됩니다. 이전에는 사용자가 카탈로그에서 반복 인증을 볼 수 없는 경우가 있었습니다.
* 인증을 생성하는 동안 책임자가 인증 유효 기간과 같거나 큰 **완료까지 남은 기간** 값을 입력하면 경고 메시지가 나타납니다. 이전에는 책임자가 경고 메시지를 받지 못했습니다.
...
* **먼저 사용자를 등록한 후 이후 로그인에 사용할 사용자 이름 암호를 만들어야 하므로** 문자 메시지가 외부 사용자 등록 페이지에 포함됩니다.

**사용자 알림**

* 외부 학습자가 강의 재방문 전자 메일 알림에서 **노트 열기**&#x200B;링크를 클릭하면 플레이어는 열리지만 노트 패널이 작동하지 않습니다. 이 문제는 수정되었습니다.
* 외부 학습자가 강의 재방문 전자 메일 공지에서 **&#39;노트 열기&#39;** 링크를 사용하여 사전 작업 또는 테스트 모듈을 열려고 하면 노트 내용이 표시되지 않습니다. 이 문제는 수정되었습니다.
...
&#39;CSV 업로드 기능&#39;의 도움말 URL이 영어 도움말 콘텐츠로 이동합니다. 이 문제는 수정되었습니다.

**강의 미리 보기 및 게시**

작성자 로그인에서는 프리젠터 10 및 11 TinCan API(SWF/HTML) 콘텐츠를 미리 볼 때 콘텐츠가 작동하지 않습니다. 이 문제는 수정되었습니다.
...
작업 지원은 그 어떤 등록이나 완료 기준 없이 학습자가 접근할 수 있는 교육 콘텐츠 저장소입니다. 학습자는 이러한 작업 지원을 참조하여 조직 내 활동이나 작업 수행을 지원받을 수 있습니다. 책임자는 작업 지원당 다운로드 수를 추적할 수 있습니다.

이 기능에 대한 자세한 내용은 [작업 지원 도움말](../learners/feature-summary/job-aids.md)을 참조하십시오.

### 공지

관리자가 정의한 사용자 그룹에 멀티미디어 메시지(텍스트, 이미지 또는 비디오)로 공지를 보낼 수 있습니다. 공지를 이용하여 학습의 교육 동기를 부여하고 그에 따른 학습 문화를 형성합니다.
...
* 책임자 역할에서 **&#39;학습 객체 추가&#39;** 대화 상자를 사용하여 카탈로그를 생성하는 동안 중지된 강의가 강의 목록에 나타나지 않습니다.

**기타 개선 사항**

* 책임자 역할에서 학습자의 전체 이름(이름과 성)이 **&#39;학습자&#39;** 탭에 표시됩니다. 이전에는 학습자의 이름만 나타났습니다.
...
* 책임자 및 작성자로 강의, 카탈로그 및 학습 프로그램에 접속하면 탭 순서가 **&#39;게시&#39; - &#39;초안&#39; - &#39;전체&#39; - &#39;중단&#39;**&#x200B;으로 변경됩니다. 기본 선택은 **게시**&#x200B;입니다.
...
* Learning Manager에
  ...
  * CP 8 버전 콘텐트 업로드 지원 시간이 단축됩니다(CP803 패치 필요).
