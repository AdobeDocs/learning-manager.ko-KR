...
완료한 강의의 콘텐츠 모듈이 최신 버전으로 업데이트되면 학습자의 흰색 화면이 표시됩니다. 이 문제는 작성자가 아직 강의를 시작하지 않은 학습자에게만 새로운 콘텐츠를 적용하기로 선택했을 때에도 발생했습니다. 이 문제는 사용자 경험에 영향을 미쳤으며 이전 학습자가 재생 문제에 봉착하여 강의를 다시 방문하려고 할 때 혼란을 야기했습니다.

### 기존 동작

작성자가 강의 콘텐츠를 새 버전(V2)으로 업데이트하고 &quot;아직 시작하지 않은&quot; 학습자에만 적용하면, Adobe Learning Manager은 백그라운드에서 완료한 학습자의 모듈 버전을 업데이트했으며 이들 학습자는 업데이트된 모듈이 더 이상 유효하지 않아 콘텐츠에 다시 액세스하려고 할 때 흰색 화면을 경험했습니다.

### 예:

* 완료한 학습자가 업데이트 후 강의를 다시 방문하려고 하면 흰색 화면이 나타납니다.
* 진행 중인 학습자가 강의를 완료했지만 콘텐츠 버전이 자동으로 업데이트된 후 흰색 화면이 나타납니다.

### 변경된 내용

Adobe Learning Manager은 콘텐츠를 업데이트할 때 작성자를 위한 보다 명확한 버전 제어 옵션을 제공합니다. 이제 작성자는 콘텐츠 버전 업데이트 중에 세 가지 잘 정의된 옵션을 볼 수 있습니다.
...
(*) 버전 업데이트 시 모듈이 재설정됨을 나타냅니다.

**[!UICONTROL 업데이트 시작되지 않음]**&#x200B;에서 완료한 학습자는 기존 콘텐츠 버전(V1)을 계속 보고 예기치 않은 흰색 화면 문제를 해결합니다.

* **[!UICONTROL 지금 업데이트]**: 모든 학습자에 대한 콘텐츠 업데이트 적용(시작되지 않음, 진행 중 및 완료 학습자가 이제 새 콘텐츠 버전으로 이동)
* **[!UICONTROL 최종 업데이트]**: 모든 학습자에 대한 콘텐츠 업데이트를 최종 적용합니다(시작되지 않음, 완료한 학습자는 이제 새 콘텐츠 버전으로 이동, 진행 중인 학습자는 완료 후 이동)
* **[!UICONTROL 업데이트 시작되지 않음]**: 시작되지 않은 학습자에 대해서만 콘텐츠 업데이트 적용(진행 중 및 완료한 학습자는 기존 콘텐츠 버전에 남음)
