...
앱을 다운로드하고 설치하고 나서 학습자 자격 증명으로 앱에 로그인합니다. Learning Manager 모바일 앱을 열면 기본적으로 다음 화면이 표시됩니다.
...
피드백은 익명으로 수집되며 앱 설정의 구성을 통해 **사용 데이터 공유** 옵션을 사용하도록 설정한 사용자에게만 표시됩니다.

## 배지 다운로드 {#downloadbadge}

학습자는 PDF와 이미지 형식으로 배지를 다운로드할 수 있습니다. 사용자 프로필 섹션에서 아래로 스크롤하고, 아무 링크나 클릭하여 배지를 다운로드합니다. 다운로드한 배지는 기본 장치 폴더에 저장됩니다. 앱에서 다운로드가 완료된 직후에 폴더 또는 파일을 열 수 있습니다.
...
사용자가 모바일 장치에서 ALM 링크를 열거나 ALM 웹을 사용할 때 더 나은 환경을 위해 앱을 다운로드하라는 메시지가 표시됩니다.
...
기존에 모바일 몰입형 앱에서 지원되지 않던 몇 가지 기능이 포함됩니다. 그 기능은 다음과 같습니다.

| **영역** | **지원되지 않음** |
|---|---|
| 게시판 | 게시판 생성, 게시판 편집, URL 복사, 삭제, 보고서, 정렬, 필터링 |
| 게시 | 투표 유형 게시물 생성, 편집, 게시물에 다중 첨부, URL 복사, 상단에 고정 |
| 의견/ 답변 | 편집, 정답으로 표시, 의견 또는 답변에 이미지/비디오/오디오 업로드 |
| 미디어 | 오디오 캡처, 오디오 파일만 업로드 |
| 기타 | 알림, 게시물 보기, 소셜 순위표, 팔로우한 사람, 사용자 프로필 |

## 학습자가 작업 지원을 다운로드할 수 있도록 활성화 {#download-job-aid}

모바일 몰입형 모드에서 학습자는 작업 지원을 다운로드할 수 있습니다. **설정**&#x200B;을 탭한 다음 **카탈로그**&#x200B;를 탭합니다. 표시되는 목록에서 작업 지원을 다운로드합니다.
...
강의를 탭하여 강의 세부 정보와 완료 날짜(해당하는 경우)를 확인합니다. 이 화면에서 완료 상태에 따라 강의를 계속하거나 재방문할 수 있습니다.
...
화면 하단의 책 아이콘 옆에 있는 아이콘을 탭합니다. 모든 관련 학습 객체가 있는 카탈로그를 즉시 볼 수 있습니다. 격자 레이아웃으로 보거나 세부 보기를 선택할 수 있습니다. 학습 기록 기준으로 강의 추천을 보려면 추천을 선택합니다.
...
프로필, 알림, 계정 설정을 확인하려면 화면 왼쪽 상단 모서리의 ![](assets/hamburger.jpg) 메뉴를 탭합니다.

또한 이 메뉴에서 강의의 출석을 표시하는 QR 코드를 스캔할 수 있습니다.

**QR 코드 스캔** 옵션을 탭하고 코드를 스캔하기만 하면 됩니다.
...
게시물을 만들려면 화면 오른쪽 하단의 ![추가 아이콘](/help/migrated/learners/feature-summary/assets/prime-add.png) 아이콘을 탭합니다.
...
1. **수료 증명**&#x200B;을 누르고 파일(예: pdf 또는 이미지)을 업로드합니다.
...
* 상태가 **제출 대기 중 및 거부됨**&#x200B;인 경우에만 파일을 제출할 수 있습니다.
* 상태가 **승인 보류 중 및 승인됨**&#x200B;으로 표시되는 경우 파일을 제출할 수 없습니다.
...
*LinkedIn Learning에 강의 등록*

LinkedIn 학습 과정에서 소요된 학습 시간은 LinkedIn 콘텐츠/LinkedIn 플랫폼에서 Learning Manager 학습 플랫폼으로 전달합니다.

LinkedIn 학습에서 학습 시간을 전송하지 않는 경우 학습 플랫폼에서 기록할 수 없습니다. 이러한 경우 Learning Manager에서 표시한 소요 학습 시간은 0입니다.
...
학습자가 강의를 시작하면 아래의 스크린샷에 표시된 것처럼 파일 업로드 링크와 함께 **파일 제출** 상태가 보류 중임을 확인할 수 있습니다.
...
파일을 업로드하면 상태가 **승인 보류 중**(으)로 변경됩니다. 강사는 [보류 중인 제출](/help/migrated/instructors/feature-summary/learners.md)의 목록을 보고 강사 웹 앱을 통해 제출을 승인하거나 거부합니다.
...
학습 및 카탈로그 기능으로 이동하려면 **메뉴** 드롭다운을 탭한 다음 알맞은 옵션을 선택합니다.
...
iPad와 Android 태블릿에서 Learning Manager 응용 프로그램을 오프라인 상태로 사용할 수 있습니다. 강의 및 학습 프로그램을 다운로드할 수 있습니다. 과정을 수강하고 오프라인 모드에서 학습 프로그램을 수료한 다음 네트워크에 연결되면 콘텐츠를 온라인 앱과 다시 동기화할 수 있습니다.
...
사용자는 녹색 막대의 **온라인 콘텐츠 보기** 단추를 클릭하여 온라인 모드로 다시 전환할 수 있습니다. 사용자가 온라인 모드로 전환하면 장치 내 콘텐츠가 동기화됩니다.

## 모바일 몰입형 앱에서 여러 항목 카트 지원

학습자는 이제 여러 강의, 학습 경로 또는 인증을 장바구니에 추가하고 함께 구매할 수 있습니다. ALM에서 [다중 항목 카트 보기](/help/migrated/learners/feature-summary/multi-item-cart.md)
