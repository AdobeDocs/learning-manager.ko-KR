...
자세한 내용은 [Microsoft Teams용 Adobe Learning Manager 앱](/help/migrated/adobe-learning-manager-app-microsoft-teams.md)을 참조하십시오.
...
* 스킬 페이지에 강사라는 새로운 열이 있습니다. 열에는 스킬에 대한 강사 수가 표시됩니다. 강사 열의 숫자를 클릭하면 스킬에 할당된 강사 수를 표시하는 팝업 창으로 리디렉션됩니다.
...
강사 및 강의실 또는 가상 강의실 예약의 충돌을 관리합니다. 강사를 강의에 할당하기 전 강사가 가능한 시간과 날짜를 알고 싶으시다면 스케줄링 도우미를 사용하십시오.
...
Google Play Store 및 Apple App Store에서 앱을 다운로드하십시오. 이전 버전의 앱 사용자에게는 새 앱을 다운로드하고 설치하라는 메시지가 표시됩니다.
...
* 검색 범위를 지정하는 강의 메타데이터, 태그 및 스킬과 같이 검색 매개 변수를 지정할 수 있는 검색 옵션.
...
학습자가 링크 또는 카탈로그로 이동 버튼을 클릭하면 학습자는 강의 카탈로그 페이지로 리디렉션됩니다. 학습자는 카탈로그 및 내 학습 페이지에서 사용자 그룹을 기반으로 필터링할 사용자 그룹 목록을 확인합니다.
...
학습자에게 충돌 세션이 있는 경우, 개요 페이지에 등록 팝업 창이 표시됩니다. 카탈로그 페이지 또는 개요 페이지에서 등록한 경우 학습자에게 이미 충돌 세션이 있으면 충돌 세션 세부 정보가 있는 충돌 팝업이 트리거됩니다. 학습자가 충돌하는 세션이 있더라도 새 세션에 등록할 수 있습니다.
...
![스위치 인스턴스 선택](assets/switch-instances-select.png)
_인스턴스 프롬프트 전환_

학습자 또는 책임자 앱에서 강의의 인스턴스를 완료할 때까지 인스턴스를 전환할 수 없습니다.
...
자세한 내용은 [강사 활용 보고서](/help/migrated/administrators/feature-summary/reports.md#instructor-utilization-report)를 참조하십시오.
...
공지사항 생성 대화 상자에서 인스턴스와 함께 전자 메일 및 교육으로 입력 옵션이 기본적으로 선택됩니다. 제목을 지정하고 메시지를 입력한 다음 저장을 클릭합니다.
...
자세한 내용은 [공지사항](/help/migrated/administrators/feature-summary/announcements.md#as-email)을 참조하십시오.
...
| 도시 검색 및 필터링 | filter.cityName=city_name<br>쉼표로 구분된 도시 목록도 지원합니다. | GET /primeapi/v2/search |
...
신규 쿼리 매개 변수: suggestionType 허용되는 값은 다음과 같습니다.
...
현재 및 이전 릴리스의 Learning Manager 웹 앱 및 장치 앱과 관련된 정보는 [출시 안내](/help/migrated/release-note/release-notes.md)를 참조하십시오.
