...
기업에는 Learning Manager와 통합해야 하는 다른 응용 프로그램과 시스템이 있습니다. 커넥터는 외부 시스템에서 Learning Manager로 데이터를 가져오는 등 데이터 기반 통합을 수행하는 데 도움이 되는 유틸리티입니다.  그에 더하여, Learning Manager에서 외부 시스템으로 데이터를 내보냅니다.
...
Salesforce와 Learning Manager을 통합하기 위해 다음 과정을 배워봅니다.
...
1. 매핑 페이지의 왼쪽에는 Learning Manager의 열이 표시되고 오른쪽에는 Salesforce 열이 표시됩니다. Learning Manager의 열 이름에 매핑할 적절한 열 이름을 선택합니다.
   ...
   *맵 속성*
   ...
   >[!NOTE]
   >
   >왼쪽에 표시된 Learning Manager의 열 데이터를 활성 필드에서 가져옵니다. **관리자** 필드는 이메일 주소 유형의 필드에 매핑되어야 합니다. 커넥터를 사용하기 전에 모든 열을 매핑하는 것은 필수입니다.
...
1. **[!UICONTROL 예]**&#x200B;를 클릭하는 경우 다음을 수행할 수 있습니다.
   ...
   * 가져오기를 시작하려면 **[!UICONTROL 저장]**&#x200B;을 클릭합니다.

1. **[!UICONTROL 아니요를 클릭하는 경우 모든 연락처 가져오기]**, 연락처를 필터링하지 않고 직접 필드를 매핑할 수 있습니다. 여기에서는 Salesforce에서 모든 연락처를 가져옵니다.
1. 가져오기를 시작하려면 **[!UICONTROL 저장]**&#x200B;을 클릭합니다.
...
1. 사용자 정의 개체를 만들기 위해 패키지를 다운로드 및 설치합니다.
   ...
   * [패키지 1](https://login.salesforce.com/packaging/installPackage.apexp?p0=04tDb000000HciS)
   * [패키지 2](https://login.salesforce.com/packaging/installPackage.apexp?p0=04tDb000000HciX)
   * [패키지 3](https://login.salesforce.com/packaging/installPackage.apexp?p0=04tDb000000Hcic)
...
**이벤트 연결:**&#x200B;사용자와 연락처 중 어떤 섹션을 내보낼 지 선택합니다. 연락처 개체를 선택하면 Salesforce에는 없지만 Learning Manager에는 있는 사용자가 Salesforce에서 생성됩니다.
...
### Learning Manager 앱 설정 {#setup-in-learning-manager-app}
...
1. OAuth 코드 가져오기 섹션에서 클라이언트 ID 및 범위를 &quot;- admin:read,admin:write&quot;(으)로 설정해야 합니다. **[!UICONTROL 전송]**&#x200B;을 클릭합니다.
...
1. 로그인한 후 Salesforce URL(예: site.lightning.force.com)을 메모합니다.
...
1. 패키지 URL을 입력하고 **[!UICONTROL 계속]**&#x200B;을 클릭합니다. 설치 페이지에 &#39;책임자 전용 설치&#39; 옵션이 선택되어 있어야 합니다. 이 옵션을 변경하지 마십시오.
1. **[!UICONTROL 설치]**&#x200B;를 클릭합니다. 패키지가 설치되면 **[!UICONTROL 완료]**&#x200B;를 클릭합니다. &#39;설치된 패키지&#39; 페이지로 이동하면 설치된 Adobe Learning Manager 패키지를 확인할 수 있습니다.
1. &#39;설정&#39; 옆에 있는 &#39;앱 시작 관리자&#39;로 이동하여 &#39;Adobe Learning Manager&#39;를 검색합니다.
1. 앱을 구성하려면 **[!UICONTROL 구성]**&#x200B;을 클릭합니다.
...
1. 오른쪽 상단에 있는 **[!UICONTROL 설정]**&#x200B;을 클릭합니다.
1. &#39;사용자 정의 알림&#39;을 검색합니다.
...
책임자 계정에서 학습 경로 옵션이 활성화되고, 학습자가 등록된 경우 &#39;유형&#39; 열이 학습 경로로 채워집니다.
...
책임자 계정에서 학습 경로 옵션이 비활성화된 경우, 교육 보고서는 다음 열로 구성되나 데이터는 포함되지 않습니다.
...
1. 왼쪽 창에서 **[!UICONTROL 구성]**&#x200B;을 클릭합니다. 다음 중 하나를 수행합니다.
...
Learning Manager에는 두 가지 레벨의 계층구조가 있는 반면, Workday에는 네 가지 레벨이 있습니다. Workday 레벨은 스킬 프로필 카테고리, 스킬 프로필, 스킬 항목 카테고리, 스킬 항목, 이렇게 네 가지로 구성되어 있습니다. 스킬 이름과 레벨은 Workday에 스킬 항목으로 같이 매핑됩니다.
...
사용자가 달성한 모든 스킬을 Learning Manager에서 Workday로 내보낼 수 있습니다. 모든 활성 사용자 스킬만 내보내고 Learning Manager는 비활성화된 스킬을 내보내지 않습니다. 또한 여러 Learning Manager 계정을 동일한 Workday 커넥터에 연결할 수도 있습니다. 두 Learning Manager 계정에서 스킬 이름이 동일한 경우, Workday에서 동일한 스킬에 매핑됩니다. Workday에서 스킬을 업데이트하기 전에 두 Learning Manager 계정이 동일한 Workday 계정을 사용하는 경우 모든 Learning Manager 계정의 스킬 이름을 업데이트하는 것이 좋습니다.
...
Learning Manager와 Zoom 커넥터를 통합하여 클래스를 호스트할 수 있습니다.  이 커넥터를 통해 학습자와 화상 회의/클래스를 설정할 수 있습니다.
...
1. Learning Manager에서 사용자에게 Box 첫 액세스 전, 암호를 재설정하라는 메시지가 있는 이메일을 전송합니다. 사용자는 Learning Manager Box 계정에 액세스하기 위해 암호를 재설정해야 합니다.
   ...
   개요 페이지에서 통합에 대한 커넥션 네임을 지정할 수 있습니다. 다음 옵션에서 원하는 작업을 선택합니다.
...
1. 구성 페이지에서 구성 목록에서 사용 가능한 기존 구성을 선택하여 CSV에서 xAPI 명령문을 가져옵니다. &#39;편집&#39; 또는 **&#39;새 구성 추가&#39;** 링크를 클릭하여 xAPI 명령문 구성 소스 파일 가져오기 구성 페이지로 이동합니다.
...
LinkedIn Learning 커넥터는 학습자가 Learning Manager에서 과정을 확인하고 이용하길 원하는 LinkedIn 기업용 고객이 사용합니다. API 키를 사용하면 과정을 정기적으로 가져오도록 커넥터를 구성할 수 있습니다. Learning Manager에서 과정을 한 번 생성하고 나면 사용자는 생성한 과정을 찾거나 수료할 수 있습니다. 그런 다음 Learning Manager에서 학습자 진행 상황을 확인할 수 있습니다.
...
>[!NOTE]
>
>연결을 재승인하려면 다른 Power App을 생성하고 리브랜딩된 리디렉션 URL을 지정해야 합니다.
...
Learning Manager는 Power BI에 사용할 하나의 데이터 세트로서 사용자 데이터, 학습자 대본, 게임화, 피드백 보고서 등과 같은 보고서 조합을 내보내는 방법을 제공합니다.
...
Learning Manager는 준비된 Power BI 템플릿도 제공합니다. 이러한 템플릿은 Adobe Learning Manager 계정 책임자에게 더 나은 분석 기능을 제공합니다.
...
이를 통해 사용자는 Power BI 응용 프로그램에서 이러한 템플릿을 다운로드하고 사용하며 추가적으로 사용자 정의하고 뛰어난 스토리를 전달하는 보고서를 만들 수 있습니다.
...
책임자자 계정에서 학습 경로 옵션이 활성화된 경우 보고서에는 학습 경로에 등록된 모든 학습자에 대한 열 유형 학습 경로(상위 레벨)가 포함됩니다.
...
* **기본:** 기본 인증에서는 FTP 도메인 URL, 사용자 이름 및 암호만 제공하면 됩니다. 세부 사항을 제공한 후 &#39;연결&#39;을 클릭합니다.
...
1. Adobe Connect 구성 섹션에서 **지금 구성**&#x200B;을 클릭합니다.
...
Connect 퀴즈 데이터를 Learning Manager로 가져와 기존 보고 워크플로우와 통합하여 퀴즈가 있는 자가 진행식 모듈에 사용할 수 있는 방식과 같이 Learning Manager 사용자가 보고서 내의 Adobe Connect 세션에서 퀴즈 데이터, 사용자 반응 및 점수를 가져올 수 있도록 합니다.
...
* 스케줄이 트리거되고 처리가 완료될 때까지 기다리십시오. 통합 책임자 측면에서 처리 상태를 확인하려면 Adobe Connect 커넥터 내의 **실행 상태**&#x200B;를 통해 진행 상황을 볼 수 있습니다. 실행이 성공하면 상태가 **완료됨**&#x200B;으로 변경됩니다.

* 그러면 책임자가 이전에 생성된 Learning Manager 과정을 선택합니다. 관리자는 다음 사항을 확인합니다.
...
Marketo Engage 커넥터는 새 사용자가 Learning Manager 계정에 추가될 때 Marketo Engage 데이터베이스에 잠재 고객을 추가(또는 업데이트)하도록 고안되었습니다. 또한 Learning Manager 앱에서 사용자의 학습 동작(과정 등록, 과정 완료, 스킬 할당 및 스킬 성과)도 사용자 정의 개체로써 Marketo Engage에서 해당하는 잠재 고객과 연결합니다. 이를 통해 마케터는 이 정보를 사용하여 Learning Manager에서 포착한 학습 동작을 기반으로 하여 대상을 타켓팅하고 &quot;스마트 목록&quot;과 같은 Marketo Engage의 기능을 사용할 수 있습니다.
...
Marketo Engage 연결을 생성할 때 다음의 세부 사항을 제공해야 합니다.
...
Learning Manager 앱에서 Marketo Engage 연결의 **통합 보고** 섹션에서 다음을 기반으로 캠페인을 생성할 수 있습니다.
...
사용자 보고서에서 필드는 열 Adobe Learning Manager 아래에 나열됩니다. Marketo 열 아래의 필드는 Marketo에서 제공하는 항목입니다. 두 열을 모두 사용하여 Marketo에서 Learning Manager에 있는 필드를 매핑할 수 있습니다. Learning Manager 열의 Marketo에서 관련된 열에 참여합니다. 열에 참여한 후 잠재 고객 데이터베이스가 생성됩니다.
...
### 학습자가 가상 수업에 입장할 때 Microsoft Teams로 인증할 수 있음
...
임시 방문 모드에서는 임시 방문 사용자용 학습 환경을 만들 수 있습니다. 예를 들어 임시 방문 경험은 마케팅 캠페인의 시작 페이지 역할을 하여 등록을 독려합니다.
...
프리미엄 제품은 사용자가 **[!UICONTROL 교육 데이터 액세스]** 커넥터로 구성된 헤드리스 인터페이스를 빌드하는 데 도움이 됩니다. 혼합 학습 시나리오의 경우 실시간 인원 제한, 사용 인원, 대기자 명단 제한 및 대기자 명단 수도 표시됩니다. 고객은 이러한 API를 사용하여 검색 및 필터링 기능과 로그인하지 않은 학습자를 위한 전체 강의 요약을 만들 수 있습니다.
...
1. 페이지에서 모든 비동기 API를 활성화합니다. 데이터 양이 많을 수 있으므로 내보내기가 비동기적으로 진행됩니다. 요청 페이로드라고 하는 Adobe Commerce의 API가 전송됩니다. 요청은 소비자가 있는 대기열에 메시지를 푸시합니다. 대기열에서는 이러한 메시지를 처리하고 상거래 측에서 제품을 생성합니다. Adobe Commerce에서는 이러한 비동기 처리를 기본적으로 제공하지 않습니다. 이 옵션을 반드시 활성화해야 하는 이유가 여기에 있습니다.
1. 결제 성공 페이지에서 ALM으로 돌아갈 수 있는 링크를 추가합니다. 반환 URL은 반드시 Adobe Commerce에서 구성해야 합니다. 링크에 사용되는 URL은 다음과 같습니다. -  `https://learningmanager.adobe.com/app/learner#/postPayment`
...
### Adobe Commerce의 기본 Learning Manager {#learning-manager-with-commerce}
...
### Adobe Commerce의 AEM 사이트 {#aem-sites-with-adobe-commerce}
...
#### 필수 구성 요소 {#pre-requisite}

책임자는 책임자 앱의 **설정 > 일반**&#x200B;에서 **교육 가격 활성화** 확인란을 활성화해야 합니다. 옵션이 활성화되어 있으면 작성자는 교육에 대한 가격을 지정할 수 있습니다. Adobe Commerce 연결을 추가하면 이 확인란이 자동으로 선택되고 적용됩니다
...
#### 헤드리스 - 로그인하지 않음 {#headless-non-logged-in}
...
Learning Manager와 Adobe Commerce 간의 동기화는 매일 두 번씩 이뤄집니다. 책임자가 전자상거래용 계정을 활성화한 후 **이 연결을 사용하여 교육 메타데이터 내보내기 활성화** 옵션을 활성화하면 강의, 학습 경로 및 수료증의 이미지가 공개 CDN에 저장됩니다.

데이터를 동기화하지 않으면 학습자에게 가격 정보가 표시되지 않습니다.

기본 Learning Manager에서 전자상거래를 활성화하고 Learning Manager와 Adobe Commerce 간의 동기화를 완료하면 학습자가 무료 또는 유료 교육 내용을 열람하거나 검색할 수 있습니다.
