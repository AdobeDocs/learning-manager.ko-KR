...
기업에는 Learning Manager와 통합해야 하는 다른 응용 프로그램과 시스템이 있습니다. 커넥터는 외부 시스템에서 Learning Manager로 데이터 가져오기와 같은 데이터 기반 통합을 수행하는 데 도움이 되는 유틸리티입니다.  또한 Learning Manager에서 외부 시스템으로 데이터 내보내기를 수행합니다.
...
>[!NOTE]
>
>Adobe Learning Manager의 2022년 11월 릴리스에서 Zoom은 2023](https://marketplace.zoom.us/docs/guides/auth/jwt/)년 6월까지 JWT 인증을 더 이상 사용하지 않습니다[. 따라서 JWT가 지원되는 Zoom 커넥터는 언급한 날짜까지 작동하지만 사용자가 서버간 Oauth 앱을 생성하여 계정에서 해당 기능을 대체하는 것이 좋습니다. 모든 새로운 연결은 기본적으로 Zoom Oauth 인증을 거치게 됩니다.

## Salesforce 커넥터 {#sfconnector}

Salesforce 커넥터는 Learning Manager와 Salesforce 계정을 연결하여 자동으로 데이터를 동기화합니다. Salesforce 커넥터 기능은 다음과 같습니다.
...
자동 예약 기능과 함께 자동 사용자 가져오기 기능을 사용하면 효과적일 수 있습니다. Learning Manager 책임자는 조직 필요에 따라 일정을 설정할 수 있습니다. Learning Manager 응용 프로그램의 사용자는 일정에 따라 최신 상태를 유지할 수 있습니다. Learning Manager 응용 프로그램에서 매일 동기화를 수행할 수 있습니다.
...
Salesforce를 Learning Manager와 통합하려면 다음 프로세스를 학습합니다.

#### 필수 구성 요소 {#prerequisites}

Salesforce 조직 URL이 있는지 확인하십시오. 예를 들어 조직 이름이 myorg **인 경우 Salesforce URL`https://myorg.salesforce.com`은**. Salesforce 계정과 Learning Manager 계정 연결은 URL만 있으면 가능합니다.
...
1. Org-url을 입력하라는 대화 상자가 나타납니다. URL을 제공한 후 연결을&#x200B;]**클릭합니다**[!UICONTROL .
1. 연결이 성공하면, 개요 페이지가 나타납니다.

### 맵 속성 {#mapattributes}

연결이 성공적으로 설정되면 Salesforce 열을 Learning Manager의 해당 속성에 매핑할 수 있습니다. 이 과정은 꼭 필요합니다.

1. 매핑 페이지에서 왼쪽에는 Learning Manager의 열이 표시되고 오른쪽에는 Salesforce 열이 표시됩니다. Learning Manager의 열 이름에 매핑되는 적절한 열 이름을 선택합니다.
   ...
   *맵 속성*
   ...
   >[!NOTE]
   >
   >왼쪽에 표시된 Learning Manager의 열 데이터는 활성 필드에서 가져옵니다. **관리자** 필드는 이메일 주소 유형의 필드에 매핑되어야 합니다. 커넥터를 사용하기 전에 모든 열을 매핑하는 것은 필수입니다.

1. 매핑을 완료한 후 [저장&#x200B;]**]을 클릭합니다**[!UICONTROL .
...
Salesforce 커넥터 페이지에서 Salesforce URL을 입력하고 인증을 완료합니다. 인증이 완료되면 사용자 또는 연락처 가져오기를 진행할 수 있습니다. [연락처] 옵션을 선택하는 경우 가져올 연락처의 하위 집합을 지정합니다.

Salesforce 열을 선택하고 해당 Learning Manager의 그룹화 가능 속성에 매핑합니다. 매핑을 완료하면 다음 사용자가 가져와도 동일한 매핑이 사용됩니다.

1. Salesforce에 로그인합니다.
1. 연결 페이지에서 내부 사용자&#x200B;]**가져오기를 클릭합니다**[!UICONTROL .

   ![](assets/image048.png)
   *내부 사용자 가져오기*

1. **사용자** 가져오기 페이지에는 새 옵션인 연락처가 있습니다. 연락처&#x200B;**라디오 버튼을**&#x200B;클릭하면 다음 옵션이 표시됩니다.

   ![](assets/image050.png)
   *연락처 속성 매핑*

1. [예&#x200B;]**]를 클릭하면**[!UICONTROL &#x200B;다음을 수행할 수 있습니다.

   * **연락처 열 선택:** Learning Manager로 가져올 필드를 선택합니다.
   * **값 지정:** 선택한 필드를 나타내는 값을 선택합니다.

   ![](assets/image053.png)
   *값을 지정합니다*

   * Salesforce 열을 Learning Manager의 열과 매핑합니다.
   * 가져오기를 시작하려면 [저장&#x200B;]**]을 클릭합니다**[!UICONTROL .

1. 아니오를 클릭 **[!UICONTROL 하면. 모든 연락처를]** 가져오면 연락처를 필터링하지 않고 필드를 직접 매핑할 수 있습니다. 여기에서 Salesforce에서 모든 연락처를 가져옵니다.
1. 가져오기를 시작하려면 [저장&#x200B;]**]을 클릭합니다**[!UICONTROL .

## 학습 기록 내보내기

Learning Manager는 성적 증명서, 사용자 보고서, 기술 보고서와 같은 학습 기록을 Salesforce로 내보내는 기능을 제공합니다. 내보낸 데이터를 Salesforce의 &#39;사용자&#39; 테이블 또는 &#39;연락처&#39; 테이블과 연결해야 하는지 결정할 수 있습니다.
...
Learning Manager에서 학습 기록을 내보내기 전에 Salesforce에서 사용자 정의 개체를 만들어야 합니다. 사용자 지정 개체는 회사 또는 산업과 관련된 정보를 저장하기 위해 만드는 개체입니다. 자세한 내용은 [Salesforce 사용자 정의 개체](https://trailhead.salesforce.com/en/content/learn/modules/data_modeling/objects_intro)를 참조하십시오.
...
**이벤트 연결:** 내보낼 섹션(사용자 또는 연락처)을 선택합니다. 연락처 개체를 선택하면 Learning Manager에는 있지만 Salesforce에는 없는 사용자는 Salesforce에서 생성됩니다.
...
>[!NOTE]
>
>Salesforce의 실행 상태 페이지에서 처리된 레코드 개수는 Salesforce에서만 확인할 수 있습니다. Learning Manager는 처리된 모든 레코드에 부분적인 내보내기 또는 실패가 있는 경우에도 상태를 완료됨으로 표시합니다.
...
1. Learning Manager 패키지 URL](https://login.salesforce.com/packaging/installPackage.apexp?p0=04t1k0000008WOQ)을 [시작합니다.
...
1. 설치를&#x200B;]**클릭합니다**[!UICONTROL . 패키지가 설치되면 [완료&#x200B;]**]를 클릭합니다**[!UICONTROL . &#39;설치된 패키지&#39; 페이지로 이동하면 설치된 Adobe Learning Manager 패키지를 확인할 수 있습니다.
...
1. 빠른 찾기&#x200B;]**에서**[!UICONTROL &#x200B;원격 사이트 설정을 검색합니다.
...
1. 오른쪽 상단에서 [설정&#x200B;]**]을 클릭합니다**[!UICONTROL .
1. &#39;사용자 정의 알림&#39;을 검색합니다.
...
**[!UICONTROL 새로 구성]**> **[!UICONTROL 클릭하고]** 다음을 추가합니다.
...
관리자 계정에서 학습 경로 옵션을 사용하지 않도록 설정하면 보고서에 행과 열이 추가되지 않습니다.

관리자 계정에서 학습 경로 옵션이 활성화된 경우 학습자가 학습 경로에 등록되어 있는 경우 &quot;유형&quot; 열이 학습 경로로 채워집니다.

>[!NOTE]
>
>플래그를 사용할 수 있고 기존 연결을 사용하는 경우 몇 가지 레코드가 누락될 수 있습니다.

#### 새 연결

관리자 계정에서 학습 경로 옵션이 비활성화된 경우 교육 보고서는 다음 열로 구성되지만 데이터는 포함되지 않습니다.

* **포함된 경로:** 학습 프로그램의 이름이 표시됩니다
* **포함된 경로 ID:** 학습 프로그램의 ID가 표시됩니다.
* **포함된 코스 ID:** 학습 경로 내에 있는 코스의 ID를 표시합니다.

또한 학습 경로가 활성화된 계정의 새 연결은 세 개의 새 열이 표시되고 모든 데이터가 플로우합니다.

또한 보고서에는 학습 경로에 등록한 모든 학습자에 대한 열 유형 학습 경로(상위 수준)가 포함됩니다.

유형 열에서 학습 프로그램의 이름이 학습 경로로 바뀝니다. 기존 연결에서는 변경되지 않습니다.

## Learning Manager FTP 커넥터 {#ftpconnector}

Learning Manager와 임시 외부 시스템을 통합하는 FTP 커넥터를 사용하여 데이터를 자동으로 동기화할 수 있습니다. 외부 시스템에서 CSV 형식으로 데이터를 내보낼 수 있으며, Learning Manager FTP 계정의 적절한 폴더에 배치할 수 있습니다. FTP 커넥터 기능은 다음과 같습니다.

Box 커넥터를 사용하여 데이터 마이그레이션, 사용자 가져오기 및 데이터 내보내기를 수행할 수도 있습니다. 자세한 내용은 Box 커넥터를 참조하세요.
...
FTP 커넥터를 Learning Manager와 통합하려면 프로세스를 학습합니다.
...
1. 이메일 아이디를 입력하라는 대화 상자가 나타납니다. 조직의 Learning Manager FTP 계정 관리를 담당하는 사람의 이메일 ID를 제공합니다. 이메일 ID를 제공한 후 연결을&#x200B;]**클릭합니다**[!UICONTROL .
1. Learning Manager에서 사용자에게 FTP 첫 액세스 전에 암호를 재설정하라는 이메일을 전송합니다. 사용자는 Learning Manager FTP 계정에 액세스하기 위해 암호를 재설정해야 합니다.
   ...
   개요 페이지에서 통합에 대한 연결 이름을 지정할 수 있습니다. 다음 옵션 중에서 수행할 작업을 선택합니다.
...
내부 사용자 가져오기 옵션을 사용하면 요청 시 또는 예약 시 csv에서 Learning Manager로 사용자를 가져올 수 있습니다.
...
1. 맵 속성 페이지에서 왼쪽에는 Learning Manager의 예상 열이 표시되고 오른쪽에는 CSV 열 이름이 표시됩니다. 처음에 오른쪽에는 빈 선택 상자가 표시됩니다. 파일&#x200B;**선택을 클릭하여**&#x200B;템플릿 CSV를 가져옵니다.
1. 위의 단계에서는 모든 CSV 열 이름으로 오른쪽 선택 드롭다운 목록을 채웁니다. Learning Manager의 열 이름에 매핑되는 적절한 열 이름을 선택합니다.
...
1. 외부 시스템의 CSV 파일은 다음 경로에 배치해야 합니다.

   `code $OPERATION$/$OBJECT_TYPE$/$SUB_OBJECT_TYPE$/data.csv`

   >[!NOTE]
   >
   >2016년 7월 릴리스에서는 사용자 가져오기만 허용됩니다. 따라서 FTP 커넥터를 사용하려면 CSV 파일이 다음 폴더에 있어야 합니다.

   `code Home/import/user/internal/*.csv`

1. FTP 커넥터는 CSV 파일에서 모든 행을 가져옵니다. 하나의 CSV 사용자에 해당하는 행이 다른 CSV에 나타나지 않는 것이 중요합니다.
1. 모든 CSV에는 매핑에 지정된 열이 포함되어야 합니다.
1. 프로세스가 시작되기 전에 필요한 모든 CSV가 폴더에 있어야 합니다.

>[!NOTE]
>
>사용자를 Learning Manager로 가져오는 동안 책임자는 사용자가 Learning Manager에서 어떻게 관리되는지 알아야 합니다. 자세한 내용은 사용자 관리 도움말을](migration-manual.md#usermanagement) 참조하십시오[.
...
1. 구성 페이지의 CSV에서 xAPI 문을 가져오기 위해 구성 목록에서 사용할 수 있는 기존 구성을 선택합니다. 편집을 클릭하거나 **새 구성** 링크를 추가하여 Import-Sources 구성 페이지로 이동합니다.
...
1. 왼쪽 창에서 [요청 시 실행&#x200B;]**]을 클릭합니다**[!UICONTROL .
...
사용자 기술 보고서를 내보내는 두 가지 옵션이 있습니다.

**[!UICONTROL 사용자 기술 - 주문형]**: 시작 날짜를 지정하고 옵션을 사용하여 보고서를 내보낼 수 있습니다. 보고서는 입력된 날짜로부터 현재 날짜까지 추출됩니다.
...
내보낸 파일이 있는 Export 폴더를 열려면 아래와 같이 사용자 기술 페이지에 제공된 FTP 폴더 링크를 엽니다.
...
자동으로 내보낸 파일은 skill_achievements_date from to date to.csv라는 제목&#x200B;**으로&#x200B;_&#42;사용할 수 있습니다&#42;.&#42;_&#42;**
...
내보낸 파일이 FTP 위치에 있는 내보내기 폴더를 열려면 아래와 같이 학습자 성적 증명서 페이지에 제공된 FTP 폴더 링크를 엽니다

자동으로 내보낸 파일은 Home/export/&#42;FTP_location 위치에 **있습니다.&#42;**

자동 내보낸 파일은 learner_transcript_date from &#42;_to date to.csv&#42;라는 제목&#x200B;**으로_&#42;사용할 수 있습니다&#42;.**
...
>[!NOTE]
>
>매핑하는 동안 csv 활성 필드에 대해 DontImportFromSource **옵션을**&#x200B;선택하면 이 필드가 시스템에서 삭제됩니다.
...
1. &#39;저장&#39;을 클릭합니다.
   ...
   구성이 완료되었으며 내 계정에 Lynda가 추가되었습니다. 이제 홈 페이지에서 연결 관리를 클릭하고 언제든지 구성을 편집할 수 있습니다.
...
1. 왼쪽 창에서 [구성&#x200B;]**]을 클릭합니다**[!UICONTROL . 다음 중 하나를 실행합니다.
   ...
   *Learning Manager용 Lynda 커넥터 구성하기*
...
1. 또한 언제든지 왼쪽 창에서 실행 상태를 눌러 이 커넥터에 대한 모든 실행의 요약을 시간 순서대로 볼 수도 있습니다. 동기화 시작 날짜 및 기간, 동기화 유형(온디맨드 동기화인지 여부) 및 동기화 상태(동기화가 진행 중인지 완료되었는지 여부)를 볼 수 있습니다.
   ...
   [getAbstract 사용자 피드 샘플 엑셀 시트](assets/report-export-20170401175342.xlsx)
...
1. 이미 연결이 설정된 경우 홈 페이지에서 &#39;Harvard ManageMentor&#39; > &#39;연결 관리&#39;를 클릭합니다. 기존 구성을 수정할 연결을 클릭합니다.
   ...
   *Learning Manager용 HarvardManage Mentor 커넥터 구성하기*
...
1. 또한 언제든지 왼쪽 창에서 실행 상태를 눌러 이 커넥터에 대한 모든 실행의 요약을 시간 순서대로 볼 수도 있습니다. 동기화 시작 날짜 및 기간, 동기화 유형(온디맨드 동기화인지 여부) 및 동기화 상태(동기화가 진행 중인지 완료되었는지 여부)를 볼 수 있습니다.
   ...
   * [Harvard ManageMentor 커넥터에 대한 과정 메타데이터 파일](assets/hmm12-metadata.xlsx)
...
* 직원의 UUID, 이메일 주소 및 이름이 여러 Workday Integration에서 고유한지 확인합니다. 값이 올바르지 않으면 연결 오류가 발생합니다.
* Workday를 통해 채워진 UUID 필드는 LMS 관리자에게 표시되는 클라이언트에서 삭제할 수 없습니다. 값을 변경하려면 Adobe Learning Manager 온보딩 또는 지원 팀에 문의하십시오.
* 사용자 제거는 실행당 50명의 사용자만 제거할 수 있으므로 사용자 제거 옵션도 작동하지 않을 수 있습니다. UUID를 통해 사용자를 업로드하는 동안 각별히 주의하십시오.
...
>[!PREREQUISITES]
>
>조직의 Workday 관리자에게 ISU_Permissions 문서에 정의된 권한을 가진 ISU(통합 시스템 사용자)를 만들도록 요청합니다. 아래 링크에서 복사본을 다운로드합니다.

[ISU(통합 시스템 사용자) 보안 사본을 다운로드합니다.](assets/isu-permissions-v1.pdf) Workday 커넥터를 Learning Manager와 통합하려면 프로세스를 학습합니다.

1. Learning Manager 홈 페이지에서 Workday 타일 위로 마우스를 가져다 댑니다. 그러면 메뉴가 나타납니다. 메뉴에 있는 **[!UICONTROL 연결]**&#x200B;항목을 클릭합니다.
...
1. 새 연결에 대한 자격 증명을 입력하라는 대화 상자가 나타납니다. 연결 전에 다음 필드를 입력합니다.
   ...
   * 테넌트: 테넌트는 회사 내부에 있습니다. Workday 책임자가 테넌트 세부 정보를 제공합니다.
   * 사용자 이름 및 암호: Workday 관리자는 필요한 보안 권한이 있는 ISU(통합 시스템 사용자)를 만들고 통합 관리자와 공유합니다.

>[!NOTE]
>
>   Learning Manager는 Workday API 버전 40.1을 사용합니다.
...
>[!NOTE]
>
>현재 Learning Manager는 Workday에서 69개의 사용자 특성 가져오기를 지원합니다. Learning Manager의 Active Field(활성 필드)를 사용하여 속성을 추가합니다.

![](assets/workday.png)
*지도 속성*

**비정규직 근로자** 제외 확인란을 선택하여 관리자 아래에서 사용할 수 있는 임시 근로자를 가져오지 못하도록 합니다.

Learning Manager에는 두 가지 레벨의 계층구조가 있는 반면, Workday에는 네 가지 레벨이 있습니다. Workday의 네 가지 수준은 기술 프로필 범주, 기술 프로필, 기술 항목 범주 및 기술 항목입니다. Learning Manager의 스킬 이름과 레벨이 함께 Workday의 스킬 항목 아래에 매핑됩니다.
...
wd:Worker_ID
매니저
wd:Personal_Data.wd:Name_Data.wd:Preferred_Name_Data.wd:Name_Detail_Data입니다.@wd:Formatted_Name
wd:Personal_Data.wd:Name_Data.wd:Legal_Name_Data.wd:Name_Detail_Data입니다.@wd:Formatted_Name
...
wd:Personal_Data.wd:Contact_Data.wd:Address_Data.0입니다.@wd:Formatted_Address
wd:Personal_Data.wd:Contact_Data.wd:Address_Data.0.wd:Postal_Code
wd:Personal_Data.wd:Contact_Data.wd:Email_Address_Data.0.wd:Email_Address
wd:Personal_Data.wd:Contact_Data.wd:Address_Data.0.wd:Country_Region_Descriptor
wd:Personal_Data.wd:Contact_Data.wd:Phone_Data.0입니다.@wd:Formatted_Phone
wd:Personal_Data.wd:Contact_Data.wd:Phone_Data.0.wd:Country_ISO_Code
wd:Personal_Data.wd:Contact_Data.wd:Phone_Data.0.wd:International_Phone_Code
wd:Personal_Data.wd:Contact_Data.wd:Phone_Data.0.wd:Phone_Number
wd:Personal_Data.wd:Primary_Nationality_Reference.wd:ID.1입니다.$
wd:Personal_Data.wd:Gender_Reference.wd:ID.1입니다.$
wd:Personal_Data.wd:Identification_Data.wd:National_ID.0.wd:National_ID_Data.wd:ID
wd:Personal_Data.wd:Identification_Data.wd:Custom_ID.0.wd:Custom_ID_Data.wd:ID
wd:User_Account_Data.wd:Default_Display_Language_Reference.wd:ID.1입니다.$
wd:Role_Data.wd:Organization_Role_Data.wd:Organization_Role.0.wd:Organization_Role_Reference.wd:ID.1입니다.$
wd:Employment_Data.wd:Worker_Job_Data.0.wd:Position_Data.wd:Position_Title
wd:Employment_Data.wd:Worker_Job_Data.0.wd:Position_Data.wd:Business_Title
wd:Employment_Data.wd:Worker_Job_Data.0.wd:Position_Data.wd:Business_Site_Summary_Data.wd:이름
wd:Employment_Data.wd:Worker_Job_Data.0.wd:Position_Data.wd:Business_Site_Summary_Data.wd:Address_Data.@wd:Formatted_Address
wd:Employment_Data.wd:Worker_Job_Data.0.wd:Position_Data.wd:Job_Classification_Summary_Data.0.wd:Job_Classification_Reference.wd:ID.1입니다.$
wd:Employment_Data.wd:Worker_Job_Data.0.wd:Position_Data.wd:Job_Classification_Summary_Data.0.wd:Job_Group_Reference.wd:ID.1입니다.$
wd:Employment_Data.wd:Worker_Job_Data.0.wd:Position_Data.wd:Work_Space__Reference.wd:ID.1입니다.$
...
계정을 동일한 Workday 커넥터에 연결합니다. 두 Learning Manager 계정에서 스킬 이름이 동일한 경우 Workday의 동일한 스킬에 매핑됩니다. Workday에서 스킬을 업데이트하기 전에 두 Learning Manager 계정이 동일한 Workday 계정을 사용하는 경우 모든 Learning Manager 계정에서 스킬 이름을 업데이트하는 것이 좋습니다.
...
통합 책임자는 miniOrange 속성을 선택하여 해당 Learning Manager의 그룹화 가능 속성에 매핑할 수 있습니다. 매핑을 완료하면 다음 사용자가 가져와도 동일한 매핑이 사용됩니다. 책임자가 사용자를 가져오기 위해 다른 매핑을 사용하려는 경우 이 매핑을 재구성할 수 있습니다.

#### 자동으로 사용자 가져오기

사용자 가져오기 프로세스를 통해 Learning Manager 책임자는 miniOrange에서 직원 세부 정보를 가져와 Learning Manager로 자동으로 가져올 수 있습니다.
...
1. Learning Manager 홈 페이지에서 miniOrange 카드/썸네일 위로 마우스를 가져갑니다. 그러면 메뉴가 나타납니다. 메뉴에서 연결&#x200B;]**옵션을 클릭합니다**[!UICONTROL .

   ![](assets/miniorange-tile.png)

   *miniOrange 커넥터 타일*

1. 연결을&#x200B;]**클릭하여**[!UICONTROL &#x200B;새 연결을 설정합니다. miniOrange 커넥터 페이지가 나타납니다. 매핑할 계정의 세부 정보를 입력합니다.

   ![](assets/establish-connection.png)

   *연결 만들기*

1. miniOrange 사용자를 Learning Manager 내부 사용자로 직접 가져오려면 내부 사용자&#x200B;]**가져오기 옵션을 사용합니다**[!UICONTROL .

   ![](assets/import-users.png)

   *내부 사용자 가져오기*

1. 매핑 페이지의 왼쪽에는 Learning Manager의 열이 있고 오른쪽에는 miniOrnage 열이 있습니다. Learning Manager의 열 이름에 매핑되는 적절한 열 이름을 선택합니다.
...
1. 데이터 소스를 보고 편집하려면 책임자 권한으로 **[!UICONTROL &#39;설정&#39; > &#39;데이터 소스&#39;]**&#x200B;를 클릭합니다.
   ...
   설정된 miniOrange 소스가 나열됩니다. 필터를 편집하려면 편집을&#x200B;]**클릭합니다**[!UICONTROL .
...
## 확대/축소 커넥터 {#zoom-connector}

Learning Manager를 Zoom 커넥터와 통합하고 이를 사용하여 수업을 호스팅할 수 있습니다.  커넥터를 사용하면 학습자와 화상 회의 회의/수업을 설정할 수 있습니다.

커넥터를 설정하고 사용하려면 다음 단계를 수행합니다.

1. Learning Manager 홈 페이지에서 확대/축소 축소판 위에 마우스를 올려 놓습니다. 그러면 메뉴가 나타납니다. 메뉴에서 연결&#x200B;]**옵션을 클릭합니다**[!UICONTROL .

   <!-- ![](assets/connectors.png)

   *Zoom connector tile* -->

1. 확대/축소 커넥터 페이지가 열립니다. 계정의 세부 정보를 각 필드에 입력하여 사용자 피드를 통합하고 동기화합니다. 자세한 내용은 커넥터 계정 책임자에게 문의하십시오.
...
1. 연결이 설정되면 작성자는 Zoom을 회의 시스템으로 사용하여 VC 과정을 만듭니다.

   <!-- ![](assets/vc.jpg)
   
   *Create a VC course* -->

1. 책임자, 관리자 및 학습자는 생성된 강의에 학습자를 등록할 수 있습니다. 등록 후에 학습자는 이메일을 받습니다. 학습자는 Learning Manager 계정에 로그인하여 프로그램 세부 정보를 보고 과정을 수강할 수 있습니다.
1. 과정을 마치면 완료 보고서가 Learning Manager로 전송됩니다. 책임자는 완료 보고서를 보고 학습자의 출석 및 점수를 확인할 수 있습니다.

   ![](assets/attendence-and-scoringreport.png)
   *출석 및 채점 보고서*

### Zoom Server-to-Server OAuth 앱 만들기

Adobe Learning Manager에서 사용할 Zoom Server-to-Server OAuth 앱을 만들 때 연결을 만드는 동안 Adobe Learning Manager에 필요한 범위를 추가해야 합니다.
...
Learning Manager와 임시 외부 시스템을 통합하는 Box 커넥터를 사용하여 데이터를 자동으로 동기화할 수 있습니다. 외부 시스템은 데이터를 CSV 형식으로 내보내고 Learning Manager Box 계정의 적절한 폴더에 배치할 수 있어야 합니다. 박스 커넥터 기능은 다음과 같습니다.

데이터 마이그레이션, 사용자 가져오기 및 데이터 내보내기에도 FTP 커넥터를 사용할 수 있습니다. 자세한 내용은 [Learning Manager FTP 커넥터](connectors.md#main-pars_header_1427405935)를 참조하십시오.
...
통합 책임자는 CSV 열을 선택하고 이를 Learning Manager의 그룹화 가능 속성에 매핑할 수 있습니다. 이 매핑은 일회성 작업입니다. 한번 매핑을 완료하면 다음 사용자를 가져와도 동일한 매핑을 사용할 수 있습니다. 책임자가 사용자를 가져오기 위해 다른 매핑을 사용하려는 경우 매핑을 재구성할 수 있습니다.
...
Box 커넥터를 Learning Manager와 통합하려면 프로세스를 학습합니다.

1. Learning Manager 홈 페이지에서 Box 카드/썸네일 위에 마우스를 가져다 댑니다. 그러면 메뉴가 나타납니다. 메뉴에서 항목 연결을 클릭합니다.

   ![](assets/screen-shot-2017-10-25at54426pm.png)

   *Box에 연결*

1. 이메일 아이디를 입력하라는 대화 상자가 나타납니다. 조직의 Learning Manager Box 계정 관리를 담당하는 사람의 이메일 ID를 제공합니다. 이메일 ID를 제공한 후 연결을 클릭합니다.
1. Learning Manager에서 사용자에게 Box 첫 액세스 전, 암호를 재설정하라는 메시지가 있는 이메일을 전송합니다. 사용자는 암호를 재설정하고 Learning Manager Box 계정에 액세스하는 데 사용해야 합니다.

   >[!NOTE]
   >
   >지정된 Learning Manager 계정에 대해 하나의 Learning Manager Box 계정만 만들 수 있습니다.
...
연결이 성공적으로 설정되면 Box 폴더에 있는 CSV 파일의 열을 Learning Manager의 해당 속성에 매핑할 수 있습니다. 이 과정은 꼭 필요합니다.
...
1. 구성 페이지의 CSV에서 xAPI 문을 가져오기 위해 구성 목록에서 사용할 수 있는 기존 구성을 선택합니다. edit 또는 A **dd a new Configuration(새 구성** 링크)을 클릭하여 Import xAPI Statements-Configuration-Source File(xAPI 명령문-구성-소스 파일 가져오기) 페이지로 이동합니다.
...
1. 외부 시스템의 CSV 파일은 다음 경로에 배치해야 합니다.
   ...
   >[!NOTE]
   >
   >2016년 7월 릴리스에서는 사용자 가져오기만 허용됩니다. 따라서 Box 커넥터를 사용하려면 CSV 파일이 다음 폴더에 있어야 합니다.

   `code Home/import/user/internal/*.csv`

1. Box 커넥터는 CSV 파일에서 모든 행을 가져옵니다. 하나의 CSV 사용자에 해당하는 행이 다른 CSV에 나타나지 않는 것이 중요합니다.
1. 모든 CSV에는 매핑에 지정된 열이 포함되어야 합니다.
1. 프로세스가 시작되기 전에 필요한 모든 CSV가 폴더에 있어야 합니다.
...
사용자 스킬 - 온디맨드: 이 옵션을 사용하여 시작 날짜를 지정하고 보고서를 내보낼 수 있습니다. 입력한 날짜부터 현재까지 보고서가 추출됩니다

**[!UICONTROL 사용자 스킬 - 구성]**: 이 옵션을 사용하면 보고서 추출을 예약할 수 있습니다. &#39;예약 사용&#39; 확인란을 선택하고 시작 날짜와 시간을 지정합니다. 보고서를 생성하고 전송하는 시간 간격을 지정할 수도 있습니다.

+++

Box 위치에서 내보낸 파일이 있는 Export 폴더를 열려면 아래와 같이 User Skills(사용자 기술) 페이지에 제공된 Box 폴더 링크를 엽니다.
...
>[!NOTE]
>
>고객은 Learning Manager 팀이 공유하는 Box 폴더의 액세스 권한 및 콘텐츠를 관리합니다.  또한 폴더의 콘텐츠는 프랑크푸르트 지역에 물리적으로 저장됩니다.
...
>[!NOTE]
>
>LinkedIn Learning 커넥터에서 Adobe Learning Manager로 가져온 모든 과정에 대해 고유한 LO ID를 받게 됩니다.

>[!NOTE]
>
>LinkedIn 학습 과정에서 소요된 학습 시간은 LinkedIn 콘텐츠/LinkedIn 플랫폼에서 Learning Manager 학습 플랫폼으로 전달합니다. LinkedIn 학습에서 학습 시간을 전송하지 않는 경우 학습 플랫폼에서 기록할 수 없습니다. 이 경우 Learning Manager에 표시되는 학습 소요 시간은 0입니다.

### LinkedIn Learning 포털 설정 구성 {#configuresettingsinlinkedlnlearningportal}

1. Linkedln Learning LMS에 책임자로 로그인합니다.
1. 상단 내비게이션 패널에서 admin(관리자&#x200B;]**)을 클릭합니다**[!UICONTROL .
1. 다음 창에서 **[!UICONTROL 설정]**&#x200B;을 클릭합니다.
1. 왼쪽 탐색 패널에서 [재생 통합&#x200B;]**]을 선택한**[!UICONTROL &#x200B;다음 [통합&#x200B;**] 탭을**&#x200B;클릭합니다.
1. LMS 콘텐트 시작 설정을&#x200B;]**클릭하여**[!UICONTROL &#x200B;해당 설정을 확장합니다.
...
1. 통합 관리 대시보드에서 LinkedIn Learning]을 클릭합니다[!UICONTROL . 시작, 연결, 연결 관리. 이렇게 세 가지 옵션을 볼 수 있습니다.
1. LinkedIn Learning 커넥터를 처음 구성하는 경우 연결을] 클릭합니다[!UICONTROL .
...
1. 연결 페이지에서 커넥터 이름을 입력합니다. 연결을 위해 Appkey와 Secret Key를 입력합니다.
   ...
   >[!NOTE]
   >
   >엔터프라이즈 관리자는 LinkedIn Learning Admin 포털에서 새 응용 프로그램을 생성하여 Appkey 및 Secret 키를 가져올 수 있습니다.

1. **[!UICONTROL 저장]**&#x200B;을 클릭합니다.

   구성이 완료되었으며 내 계정에 LinkedIn Learing 연결이 추가되었습니다. 이제 홈 페이지에서 연결&#x200B;]**관리를 클릭하고**[!UICONTROL &#x200B;언제든지 구성을 편집할 수 있습니다.

1. 이미 연결을 설정한 경우 연결&#x200B;]**관리를 클릭하여 모든 연결 보기를 확인합니다**[!UICONTROL .
...
1. 왼쪽 창에서 [구성]을 클릭합니다. 다음 중 하나를 실행합니다.
   ...
   * 이 창에서 계정의 세부 정보 및 동기화 예약을 보거나 편집합니다. **[!UICONTROL 이 계정을 사용하려면 연결]** 사용 확인란을 선택합니다.
   * [편집&#x200B;]**]을 클릭하고**[!UICONTROL &#x200B;자격 증명을 편집합니다. 이 영역에 대한 업데이트를 취소하려면 &#39;재설정&#39;을 클릭합니다.
   * Enable Schedule(일정&#x200B;]**사용)을 클릭하여**[!UICONTROL &#x200B;동기화를 예약합니다. 시작 시간과 날짜를 입력한 다음 동기화 예약 빈도를 일 단위로 입력할 수 있습니다. 예를 들면, 3일마다 동기화가 되도록 설정할 수 있습니다.

   변경 사항을 저장하려면 **[!UICONTROL 저장]**&#x200B;을 클릭합니다.

1. 왼쪽 창에서 [On-Demand Execution ]**]을 클릭합니다**[!UICONTROL . 이 옵션을 사용하면 LinkedIn Learning에서 사용자 피드 및 기타 관련 데이터를 가져올 수 있습니다. 요청 시 실행의 시작 날짜를 입력하고 실행을 클릭하여 동기화를 실행합니다. 시작일부터 현재까지의 모든 데이터를 가져옵니다.

   * 동기화 중에 응용 프로그램이 다운타임되는 경우 실행 중에 Learning Manager에 대한 액세스&#x200B;]**비활성화를 클릭**[!UICONTROL &#x200B;할 수 있습니다.
   * 실행 중에 Learning Manager에 대한 액세스&#x200B;]**활성화를 클릭하면**[!UICONTROL &#x200B;동기화 중에 서비스가 중단되지 않습니다.
...
* **LinkedIn Learning의 라이브러리를 기반으로 합니다.**
...
컨텐츠는 마이그레이션 후에만 저장됩니다. 콘텐츠는 각 카탈로그에 저장됩니다.
...
커넥터를 **[!UICONTROL 구성하려면 커넥터]** 페이지에서 Power BI ]**타일 위로 마우스를**[!UICONTROL &#x200B;가져간 다음 연결을&#x200B;]**클릭합니다**[!UICONTROL . Power BI 페이지가 열립니다. 연결을 설정하려면 App Client ID, App Client 비밀, 테넌트 이름 및 Workspace ID(선택 사항)를 제공합니다. 이와 같은 자격을 습득하려면 다음 단계를 수행합니다.

![](assets/power-bi-configurepage.png)

*Power BI 커넥터 구성*

1. 를 시작합니다 <https://app.powerbi.com/embedsetup>.
1. 조직에&#x200B;]**대한 포함을 클릭하고**[!UICONTROL  Microsoft 계정에 로그인합니다.
1. 앱의 이름을 입력합니다.
1. 앱 유형 섹션에서 서버 쪽 웹앱 옵션을 선택합니다.
1. **[!UICONTROL 리디렉션 URL]** 섹션에서 사용자 지정 URL **사용(대상 애플리케이션의 URL을 알고 있는 경우 선택) 옵션을**&#x200B;선택합니다. 다음 URL을 입력합니다.

   `https://learningmanager.adobe.com/ctr/app/azure/_callback` (환경에 따라 도메인 업데이트)

1. 홈 URL 필드에 다음 URL을 입력합니다. `https://learningmanager.adobe.com/`
1. 권한 섹션에서 모든 데이터 세트 읽기 및 **모든 데이터 세트****읽기 및 쓰기를 선택합니다**.
...
1. 앱&#x200B;]**등록을 클릭하고**[!UICONTROL &#x200B;클라이언트 ID 및 클라이언트 암호를 저장합니다.

>[!NOTE]
>
>연결 권한을 다시 부여하려면 다른 Power App을 만들고 리브랜딩된 리디렉션 URL을 지정해야 합니다.
...
**내보내기 구성**: 보고서 추출 예약을 원한다면 Enable Schedule **[!UICONTROL (일정]** 사용) 확인란을 선택하고 시작 날짜와 시간을 지정합니다. 보고서를 생성하고 전송하는 시간 간격을 지정할 수도 있습니다.

![](assets/power-bi-configureuserskillpage.png)

*보고서를 예약하도록 구성 내보내기*

**요청 시 내보내기:** 시작 날짜를 지정하고 옵션을 사용하여 보고서를 내보낼 수 있습니다 . 입력한 날짜부터 현재까지 보고서가 추출됩니다.
...
PowerBI-xAPI 기능 페이지에서 xAPI 활동 보고서&#x200B;]**내보내기를 클릭합니다**[!UICONTROL .
...
**&#39;실행 상태&#39;**&#x200B;를 클릭하여 모든 작업의 요약을 시간 순서대로 볼 수 있습니다. 실행되는 동안 경고 신호에 실패가 표시됩니다. 오류 보고서 링크를 클릭하여 오류 보고서를 **CSV** 로 다운로드할 수 있습니다.
...
Learning Manager는 사용자 데이터, 학습자 대화 내용, 게임화, 피드백 보고서 등과 같은 보고서의 조합으로 내보내기를 Power BI에 대한 하나의 데이터 세트로 만드는 방법을 제공합니다.
...
시작 날짜와 종료 날짜를 지정하고 옵션을 사용하여 보고서를 내보냅니다. 보고서는 지정된 날짜 범위에 대해 추출됩니다.
...
* **진행 중:** 상태가 진행 중인 모든 레코드를 내보냅니다.
* **시작되지 않음:** 지정된 날짜 범위에 등록되었지만 보고서를 생성할 때 시작되지 않은 레코드를 제외합니다.
...
Learning Manager는 미리 만들어진 Power BI 템플릿도 제공합니다. 이 템플릿은 Adobe Learning Manager 계정 관리자에게 더 나은 분석 기능을 제공합니다.

템플릿을 다운로드하고, 관련 보고서를 내보내고, 사용 가능한 템플릿을 사용하여 보고서를 쉽게 플롯할 수 있습니다.

![](assets/download-power-bi-template.png)
*Power BI 템플릿 다운로드*

이를 통해 사용자는 이러한 템플릿을 다운로드하여 Power BI 애플리케이션에서 사용하고, 추가로 사용자 지정하고, 보고서에서 설득력 있는 스토리를 전달할 수 있습니다.
...
#### 관리자: 학습 성적 증명서 및 통합 보고서

**기존 연결**

관리자 계정에서 학습 경로 옵션을 사용하지 않도록 설정하면 보고서에 행과 열이 추가되지 않습니다.

관리자 계정에서 학습 경로 옵션이 활성화된 경우 보고서에는 학습 경로에 등록된 모든 학습자에 대한 학습 경로(상위 수준) 열 유형이 포함됩니다.

**새로운 연결**

관리자 계정에서 학습 경로 옵션이 비활성화된 경우 교육 보고서는 다음 열로 구성됩니다.
...
유형 열에서 학습 프로그램은 학습 경로로 이름이 변경됩니다. 기존 연결에서는 변경되지 않습니다. 그러나 새 연결의 경우 변경 내용은 30일 후에 반영됩니다.
...
관리자 계정에서 학습 경로 옵션을 사용하도록 설정하면 보고서에 &quot;유형&quot; 열이 포함됩니다. 이 열에는 새 값 &quot;학습 경로(해당되는 경우 더 높은 수준)&quot;가 포함됩니다.
...
자체 FTP를 구성하려면 통합 관리자로 로그인하고 사용자 정의 FTP ]**>**[!UICONTROL &#x200B;연결을&#x200B;]**클릭합니다**[!UICONTROL .
...
* **기본:** 기본 인증에서는 FTP 도메인 URL, 사용자 이름 및 암호만 제공하면 됩니다. 세부 정보를 제공한 후 연결을 클릭합니다.
* **인증:** 고객 FTP가 인증서 인증을 지원하는 경우 이 옵션을 선택할 수 있습니다. SSH 키 생성을 클릭하면 SSH 키가 로컬 컴퓨터에 다운로드됩니다. 파일을 열면 키는 다음과 같습니다.

![](assets/ssh-public-key.png)
*SSH 공개 키*

아래 세부 정보를 추가하기 전에 이 공개 키를 FTP 서버에 배치해야 합니다. 주어진 키를 FTP의 공개 키로 설정 한 후 FTP 도메인 URL과 사용자 이름을 제공하고 연결&#x200B;**버튼을 클릭하여**&#x200B;연결을 설정하십시오.

연결 설정이 완료되면 가져오기 및 내보내기를 위한 폴더가 ftp 위치에 자동으로 생성됩니다. 그 후 가져오기/내보내기 기능은 사용자 지정 FTP에서 제공됩니다.
...
* 앱을 등록하기 전에 https://portal.azure.com/](https://portal.azure.com/) URL[을 사용하여 Azure 포털에 로그인합니다.
* Azure Active Directory를 엽니다.

## 응용 프로그램을 등록하는 단계입니다. {#stepstoregisteryourapplication}

* Azure Active Directory를 클릭합니다. Add ]**> App registration(앱 등록]**)을 **[!UICONTROL 클릭합니다**[!UICONTROL .
...
1. portal.azure.com **에**&#x200B;로그인합니다. 값은 테넌트 ID 및 기본 도메인 필드에 채워집니다.
...
ADFS 커넥터를 설정하려면 Learning Manager CSM 팀에 문의하십시오.

## ADFS 커넥터 구성 {#configureadfsconnector}

1. Learning Manager 홈 페이지에서 ADFS 카드/썸네일 위에 마우스를 가져다 댑니다. 그러면 메뉴가 나타납니다. 메뉴에서 연결 옵션을 클릭합니다.
...
1. 매핑 페이지의 왼쪽에는 Learning Manager의 열이 있고 오른쪽에는 ADFS 열이 있습니다. Learning Manager의 열 이름에 매핑되는 적절한 열 이름을 선택합니다.

   ![](assets/adfs4.jpg)

   *맵 속성*

1. 데이터 원본을 보고 편집하려면 관리자 권한으로 설정&#x200B;]**>**[!UICONTROL &#x200B;데이터 원본을&#x200B;]**클릭합니다**[!UICONTROL .

   설정된 ADFS 원본이 나열됩니다. 필터를 편집하려면 편집을&#x200B;]**클릭합니다**[!UICONTROL .

   ![](assets/datasource.jpg)
   *데이터 소스 설정*

1. 가져오기가 완료되면 알림이 옵니다. 가져오기 로그를 보거나 편집하려면 사용자&#x200B;]**>**[!UICONTROL &#x200B;가져오기 로그를&#x200B;]**클릭합니다**[!UICONTROL .

### 연결 삭제 {#Deleteaconnection-1}

설정된 miniOrange 연결을 삭제하려면 다음과 같이 하십시오.
...
1. Integrate(통합&#x200B;]**)를 클릭합니다**[!UICONTROL .
...
책임자 로그인 상태에서 VC 강의 이름을 클릭합니다. 왼쪽 창에서 [Instances]와 [Session Details]를 클릭합니다.  세션 세부 정보 페이지의 오른쪽 모서리에 있는 편집 아이콘을 클릭하여 세션 정보를 추가합니다.
...
Connect 퀴즈 데이터를 Learning Manager로 가져와 기존 보고 워크플로우와 통합하면 Learning Manager 사용자는 퀴즈가 있는 자기 주도형 모듈에서 사용할 수 있는 방식과 같이 보고서 내의 Adobe Connect 세션에서 퀴즈 데이터, 사용자 응답 및 점수를 얻을 수 있습니다.
...
* 작성자는 Learning Manager에서 모듈 유형을 **가상 교실로 사용하여 강의를 만듭니다.**
...
* 스케줄이 트리거되고 처리가 완료될 때까지 기다리십시오. 통합 책임자 측면에서 처리 상태를 확인하려면 Adobe Connect 커넥터 내의 **실행 상태**&#x200B;를 통해 진행 상황을 볼 수 있습니다. 실행에 성공하면 상태가 완료&#x200B;**됨으로**&#x200B;바뀝니다.

* 그런 다음 관리자는 이전에 만든 Learning Manager 과정을 선택합니다. 관리자는 다음 사항을 확인합니다.

   * **출석 및 점수** - 최종 퀴즈 점수 및 출석 상태를 표시합니다.
   * **L2 퀴즈 점수**

      * **사용자**&#x200B;별 - 최종 퀴즈 점수를 점수&#x200B;**및**&#x200B;백분율&#x200B;**로**&#x200B;표시합니다.
      * **질문별** - 보고서 차트로 퀴즈 정보를 표시합니다.

## Marketo Engage 커넥터 {#marketo}

Learning Manager는 마케팅 캠페인 실행을 지원하는 마케팅 자동화 소프트웨어인 Marketo Engage와 통합됩니다.

Marketo Engage 커넥터는 새 사용자가 Learning Manager 계정에 추가될 때 Marketo Engage 데이터베이스에 리드를 추가(또는 업데이트)하도록 설계되었습니다. 또한 Learning Manager에서 사용자의 학습 행동(과정 등록, 과정 완료, 기술 할당 및 기술 성취)을 사용자 정의 개체로 Marketo Engage의 해당 리드와 연결합니다. 이를 통해 마케터는 이 정보를 사용하여 Learning Manager에서 캡처한 학습 행동을 기반으로 대상을 타겟팅하고 &quot;스마트 목록&quot;과 같은 Marketo Engage의 기능을 사용할 수 있습니다.
...
Learning Manager를 Marketo 계정과 통합하려면 Marketo 계정에 API로 스키마를 생성할 수 있는 기능이 있어야 합니다.
...
Marketo Engage 연결을 만들 때 다음 세부 정보를 제공해야 합니다.
...
*Marketo에 대한 자격 증명 입력*

>[!NOTE]
>
>Marketo Engage 앱에서 클라이언트 ID 및 시크릿을 가져올 수 있습니다. Marketo 앱의 LaunchPoint **섹션에서 클라이언트 ID와 암호를**&#x200B;가져오고 WebServices **섹션에서 Marketo 도메인을**&#x200B;가져올 수 있습니다.

**Learning Manager 앱에서 Markeo Engage 연결의 통합 보고서** 섹션에서 다음을 기반으로 캠페인을 만들 수 있습니다.

* Learning Manager에 신규 사용자가 추가됨
* 새 사용자가 과정에 등록되었을 때
* 새 사용자가 과정을 완료했을 때
* 학습자가 스킬에 등록되었을 때
* 학습자가 스킬을 달성했을 때
...
열 매핑은 잠재 고객 데이터베이스를 생성하는 데 사용됩니다. 리드는 사용자 보고서에서 내보낸 사용자입니다.

사용자 보고서에서 필드는 열 Adobe Learning Manager 아래에 나열됩니다. Marketo 열 아래의 필드는 Marketo에서 제공하는 항목입니다. 두 열을 모두 사용하여 Learning Manager의 모든 필드를 Marketo의 필드와 매핑할 수 있습니다. Learning Manager 열에서 Marketo의 관련 열에 참여합니다. 열에 참여한 후 잠재 고객 데이터베이스가 생성됩니다.

Marketo에서 내보낸 모든 사용자를 볼 수 있습니다.

Marketo 앱의 **Marketo 사용자 정의 개체** 섹션에서 세 개의 모든 보고서, 학습자 성적 증명서, 사용자 스킬 및 사용자 보고서를 볼 수 있습니다. 이러한 보고서에는 각각 앞에 &quot;cp_&quot;**라는 문자열**&#x200B;이 붙습니다. Marketo로 내보내진 각각의 새 사용자는 잠재 고객으로 간주됩니다.
...
**Microsoft Teams를 사용하여 가상 세션 설정**
...
**Microsoft Teams가 가상 교실에 입장할 때 학습자를 인증하도록 허용**
...
자세한 내용은 Adobe Learning Manager **](install-microsoft-teams-connector.md)에 Microsoft Teams 커넥터 설치를 참조하십시오[**.

## 로그인하지 않은 환경

로그인하지 않은 환경을 사용하면 로그인하지 않은 사용자에 대한 실시간 환경을 만들 수 있습니다. 예를 들어, 로그인하지 않은 경험은 가입을 장려하기 위한 마케팅 캠페인의 랜딩 페이지 역할을 합니다.

Adobe Learning Manager의 로그인하지 않은 환경은 교육 데이터 액세스&#x200B;]**커넥터를 사용하여**[!UICONTROL &#x200B;구성할 수 있습니다. 커넥터는 다음과 같은 제품을 제공합니다.
...
표준 오퍼링은 Adobe Learning Manager의 기본 버전을 빌드하는 것입니다. 사용자는 데모 전용, 로그인하지 않은 헤드리스 환경을 구축할 수 있습니다. 데모 헤드리스 환경은 확장할 수 없으며 프로덕션 환경에서 사용해서는 안 됩니다.

**프리미엄 제품**

프리미엄 제품은 사용자가 학습 데이터 액세스&#x200B;]**커넥터에 의해 구성된 헤드리스 인터페이스를 빌드하는**[!UICONTROL &#x200B;데 도움이 됩니다. 이를 통해 사용자는 이름, 설명, 작성자, 기술, 기간 등과 같은 코스 및 학습 경로 세부 정보에 대한 실시간 데이터를 얻을 수 있습니다. 혼합 학습 시나리오의 경우 실시간 좌석 제한, 점유 좌석, 대기자 명단 제한 및 대기자 명단 수도 얻을 수 있습니다. 고객은 이러한 API를 사용하여 로그인하지 않은 학습자를 위한 검색 및 필터링 기능과 전체 과정 요약을 만들 수 있습니다.

고객은 프리미엄 플랜을 구매하여 확장성이 뛰어난 로그인하지 않은 환경을 구축할 수 있습니다.

>[!NOTE]
>
>프리미엄 플랜을 구매하려면 지원 팀 또는 CSM에 문의하십시오.

사용자가 플랜을 구매하면 CSM 팀이 해당 플랜에 대한 프리미엄 플랜을 활성화합니다. 학습 데이터 액세스 커넥터를 사용하여 사용자는 앞에서 언급한 기능으로 로그인하지 않은 환경을 설정할 수 있습니다.

### 학습 데이터 액세스 커넥터

>[!IMPORTANT]
>
>이 특정 기능은 Adobe Learning Manager가 Adobe Experience Manager에 대한 추가 기능으로 판매되는 경우에만 사용할 수 있습니다. 과정 데이터는 24시간 후에 부실해집니다.

>[!NOTE]
>
>이 섹션에서는 인프라의 작동 방식을 강조 표시하지만 헤드리스 또는 AEM 기반 비로그인 환경을 구축하려면 당사에 문의하십시오. 사용 사례에 따라 올바른 접근 방식을 제안합니다. 이 기능은 현재 셀프 서비스로 사용할 수 없습니다.

**[!UICONTROL 학습 데이터 액세스]** 커넥터를 사용하면 헤드리스 환경을 만들 수 있습니다. 이 환경은 독립형이거나 AEM Sites를 기반으로 하는 사용자 정의 사용자 인터페이스일 수 있습니다. 학습자에게 교육 정보를 검색하고 표시하는 데 도움이 되며 검색 및 필터링이 가능합니다. 데이터 커넥터가 활성화되면 공개 API 세트를 사용하여 인터페이스를 구축할 수 있으며, 여기서 과정/학습 경로 정보가 학습자에게 표시됩니다.

#### 커넥터 구성

**[!UICONTROL 교육 데이터 액세스]** 커넥터를 사용하여 Adobe Learning Manager 계정을 데이터 저장소 및 검색 시스템과 통합할 수 있습니다. 이렇게 하면 AEM Sites 기반 인터페이스에서 교육 데이터를 가져오고, 웹 페이지를 표시하고, 학습자에게 더 나은 검색 옵션을 제공할 수 있습니다.

API를 사용하여 Adobe Learning Manager의 교육 메타데이터를 데이터 검색 및 검색 지원 서비스로 내보냅니다. 또한 이러한 내보내기를 자동화하는 일정을 생성할 수도 있습니다.

학습 데이터 액세스 커넥터를 구성하려면 다음 단계를 수행합니다.

1. 통합 관리 앱에서 교육 데이터 액세스&#x200B;]**>**[!UICONTROL &#x200B;시작하기&#x200B;]**를 선택합니다**[!UICONTROL .
1. 시작&#x200B;]**페이지에서 다음을****선택합니다**[!UICONTROL .
1. 연결 이름과 허용 목록에 추가된 도메인을 입력합니다.

   ![](assets/connection-name-and-domain-name.png)연결 이름 및 도메인 이름을 입력합니다

1. **[!UICONTROL 다음 옵션 중에서 인터페이스]** 유형을 선택합니다.

   * **[!UICONTROL 네이티브 학습 관리자]**: 네이티브 인터페이스에서만 사용할 수 있는 표준 제품입니다.
***[!UICONTROL 헤드리스 인터페이스]**: 로그인하지 않은 환경을 구축하기 위해 API를 노출하는 프리미엄 제품입니다.

   ![](assets/types-of-interface.png)인터페이스 유형

1. 연결을&#x200B;]**선택합니다**[!UICONTROL . 기본 URL과 CDN URL이 자동으로 생성됩니다.
이러한 URL을 사용하여 API를 통해 데이터를 검색할 수 있습니다.

   >[!NOTE]
   >
   >프리미엄 서비스를 사용하는 고객은 표준 서비스를 사용하는 고객과 다른 URL을 받게 됩니다.


1. 커넥터 페이지에서 Export Training Metadata(학습 메타데이터&#x200B;]**내보내기)를 선택합니다**[!UICONTROL .
1. 이 연결을 사용하여 학습 메타데이터 내보내기&#x200B;]**사용을 선택하여**[!UICONTROL &#x200B;학습 데이터를 내보냅니다.
1. 연결을 활성화하면 모든 코스, 학습 경로 및 인증서의 이미지가 CDN으로 마이그레이션됩니다.
1. 과정, 학습 경로 및 인증서의 메타데이터를 검색 및 검색 서비스로 내보냅니다.
1. Enable schedule(일정 사용) 옵션을 선택하여 메타데이터 내보내기를 예약할 수 있습니다. 일정은 프리미엄 플랜의 경우 3시간마다 자동으로 발생합니다.
1. 요청 시 보고서의 경우 주문형으로 **[!UICONTROL 이동하여 시작 날짜를]** 선택한 **[!UICONTROL 다음**[!UICONTROL &#x200B;실행을 클릭합니다&#x200B;]**.]**
실행 상태 페이지에서 보고서 실행 **[!UICONTROL 상태를 확인할 수 있습니다]** .

### AEM에서 웹 사이트 생성

**전제 조건:** GitHub 저장소&#x200B;**](https://github.com/adobe/adobe-learning-manager-reference-site/releases/tag/1.0.0)에서[** AEM 패키지를 설치합니다.

1. 기본 및 검색 URL, 클라이언트 ID, 클라이언트 시크릿 및 책임자 새로 고침 토큰을 사용하고 AEM에서 구성을 생성합니다.
1. AEM 구성 요소를 사용하여 웹 사이트를 생성합니다.
1. 웹 사이트를 게시합니다.

자세한 내용은 이  [**문서를**](../../adobe-learning-manager-integration-aem.md) 참조하십시오.
...
>[!NOTE]
>
>이 특정 기능은 Adobe Learning Manager가 Adobe Experience Manager의 Add-on으로 판매되는 경우에만 사용할 수 있습니다.

>[!NOTE]
>
>이 커넥터는 평가판 계정에도 사용하도록 설정할 수 있습니다.
...
1. RabbitMq](https://devdocs.magento.com/cloud/project/services-rabbit.html) 또는 다른 메시징 브로커를 사용하도록 설정합니다[.
1. CRON을](https://devdocs.magento.com/cloud/env/variables-deploy.html#cron_consumers_runner) 활성화합니다[.
1. 1단계와 2단계에서 다음 파일을 편집합니다.

   1. .magento.app.yaml
   1. .magento/services.yaml
   1. .magento.env.yaml 파일

1. 사용자 정의 모듈을 통해 옵션 제한을 덮어씁니다. 이 단계는 선택 사항이지만 대규모 데이터 세트에는 적극적으로 권장됩니다.
1. 페이지에서 모든 비동기 API를 활성화합니다. 데이터 양이 많을 수 있으므로 내보내기가 비동기적으로 진행됩니다. Adobe Commerce의 API를 요청 페이로드라고 합니다. 요청은 메시지를 큐로 푸시하고 이 큐에는 소비자가 있으며, 이 소비자는 이러한 메시지를 처리하고 상거래 쪽에서 제품을 만듭니다. Adobe Commerce에서는 이러한 비동기 처리를 기본적으로 제공하지 않습니다. 그렇기 때문에 이 옵션을 활성화해야 합니다.
1. 결제 성공 페이지에서 ALM으로 돌아갈 수 있는 링크를 추가합니다. 반환 URL은 반드시 Adobe Commerce에서 구성해야 합니다. 링크에 사용할 URL입니다. -  `https://learningmanager.adobe.com/app/learner#/postPayment`
1. 인덱싱을 &quot;저장 시&quot;에서 &quot;예약됨&quot;으로 변경합니다.  자세한 내용은 이 KB](https://support.magento.com/hc/en-us/articles/360040227191)를 [참조하세요.
1. 다음 패치를 적용합니다. 자세한 내용은 패치 적용을](https://devdocs.magento.com/cloud/project/project-patch.html) 참조하세요[.
1. 빠르게 구성하십시오.  Fastly는 클라우드 인프라에서 Adobe Commerce에 필요하며 스테이징 및 프로덕션 환경에서 사용됩니다. 자세한 내용은 [Fastly 설정](https://devdocs.magento.com/cloud/cdn/configure-fastly.html)을 참조하십시오.

### 커넥터 구성

통합 책임자는 Adobe Commerce 커넥터에서 **[!UICONTROL 연결]**&#x200B;을 클릭합니다.

구성 페이지에서 다음 세부 정보를 입력합니다. 인증 키와 같은 이러한 세부 정보는 Adobe Commerce에서 확인할 수 있습니다. Adobe Commerce에서 통합을 생성하면 여기에서 자격 증명을 사용할 수 있습니다.

![](assets/adobe-commerce-configuration.png)
*Adobe Commerce Connector 구성*
...
>[!NOTE]
>
>Adobe Learning Manager의 2024년 3월 릴리스에서는 Adobe Commerce 2.4.6](https://experienceleague.adobe.com/docs/commerce-operations/release/notes/adobe-commerce/2-4-6.html?lang=en)에 대한 [지원을 도입했습니다.


1. [교육 메타데이터&#x200B;]**내보내기] >**[!UICONTROL  On Demand ]**를 클릭합니다**[!UICONTROL .

1. 날짜를 선택합니다.

1. **[!UICONTROL 실행]**&#x200B;을 클릭합니다. 성공적으로 실행되면 가격이 설정된 모든 과정 또는 학습 경로가 Adobe Commerce로 이동합니다. 학습자는 Learning Manager에서 강의를 구매할 수 있습니다.

### Adobe Commerce의 기본 학습 관리자

#### 학습자

학습자가 과정, 인증 또는 학습 경로를 구매하려면 로그인해야 합니다.

과정을 구매하려면, 지금 구매를 클릭합니다. 구매를 완료하기 위하여 Adobe Commerce로 리디렉션됩니다. 결제가 완료되면 Learning Manager로 돌아가 강의를 시작하라는 메시지가 표시됩니다. 구매를 완료하려면 Adobe Commerce에 별도로 로그인해야 합니다.

ALM Native 또는 AEM에서 강의, 수료증 또는 학습 경로를 구매하면 ALM과 Adobe Commerce에서 이메일을 발송합니다.

또한 Adobe Commerce의 이메일도 활성화/비활성화할 수 있습니다.

### Adobe Commerce를 사용하는 AEM Sites
...
관리자는 Admin 앱의 설정 > 일반에서 **교육** 가격 책정 활성화 확인란을 **활성화합니다.** 이 옵션을 활성화하면 작성자가 교육 가격을 지정할 수 있습니다. Adobe Commerce 연결을 추가하면 이 확인란이 자동으로 선택되고 적용됩니다
...
* 사용자는 Adobe Learning Manager 또는 헤드리스 학습 인터페이스에서 강의를 판매하여 수익을 창출할 수 있습니다.
* 플랫폼에 Adobe Commerce를 통합하여 Learning Manager의 기본 앱과 AEM을 사용하여 강의를 판매할 수 있습니다.
* Learning Manager의 고객이 유료 과정의 형태로 공식 학습을 제공할 수 있도록 합니다.
* 학습자가 교육 구매를 결정하기 전에 과정을 미리 볼 수 있습니다.
...
>[!NOTE]
>
>Adobe Learning Manager와 Adobe Commerce의 로그인 기능은 서로 다릅니다.

### AEM

이 모드에서 학습자는 AEM 기반 템플릿 및 구성 요소를 사용하여 구축된 AEM 기반 사이트에서 과정을 이수합니다.

AEM 사이트에서 학습자는 장바구니, 장바구니에 추가 버튼, 장바구니에서 강의 삭제 등을 지원합니다.
...
Learning Manager와 Adobe Commerce 간의 동기화는 매일 두 번씩 이뤄집니다. 관리자가 전자 상거래에 대한 계정을 활성화한 **후 이 연결을** 사용하여 교육 메타데이터 내보내기 활성화 옵션을 활성화하면 코스, 학습 경로 및 인증서의 이미지가 공용 CDN에 저장됩니다.

데이터를 동기화하지 않으면 학습자에게 가격 정보가 표시되지 않습니다.

기본 Learning Manager의 경우 전자 상거래가 활성화되고 Learning Manager와 Adobe Commerce 간의 동기화가 완료되면 학습자는 무료 또는 유료 교육을 보거나 검색할 수 있습니다.

AEM의 경우 지금 구매가 없고 장바구니&#x200B;**에**&#x200B;추가 버튼만 있습니다. 이 버튼은 동기화가 이뤄지지 않은 경우에도 비활성화된 상태로 유지됩니다.
