...
* OAuth2.0 인증
* API 객체 모델
* 포함, 필드 및 기타 매개 변수를 사용하는 방법을 보여 줍니다.
* 실제 사용 사례에 대한 끝점을 제공합니다.
...
Adobe Learning Manager을 외부 애플리케이션과 통합하여 다용성을 개선합니다. 단계는 통합 책임자 인터페이스에 액세스하고, 응용 프로그램을 등록하고, 클라이언트 ID 및 시크릿을 얻는 것입니다. 인증 및 권한 부여에 OAuth 2.0 프레임워크를 사용함을 강조하여 ALM에서 인증 토큰(OAuth, Refresh 및 Access 토큰)을 생성합니다. 액세스 토큰의 유효 기간은 7일입니다.
...
서드파티 API 테스트 도구를 사용할 수 있지만 Postman을 사용하여 끝점을 테스트합니다. 이 문서의 예시는 끝점 테스트용으로 Postman을 사용합니다.
...
포함 매개 변수를 API URL에 추가하고 포함할 관련 엔터티를 지정합니다.
...
사용자 보고서는 사용자 인터페이스(**[!UICONTROL 관리자]** > **[!UICONTROL 사용자]** > **[!UICONTROL 내부]**)에서 직접 다운로드할 수 있습니다. 그러나 이 보고서는 사용자 ID와 관련 관리자의 세부 정보를 반환하지 않습니다.
관리자용 작업 API를 사용하여 세부 정보를 검색할 수 있습니다.
