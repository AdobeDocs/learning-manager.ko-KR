...
* 샘플 파일: [role.csv](assets/role.zip)
* 샘플 파일: [user_role.csv](assets/user-role.zip)
 ...
 <tbody>
  ...
  <tr>
   <td>
    <p>&lt;Entity&gt;</p></td>
   <td>
    <p>강의, 카탈로그 등의 각 개체 유형에 대한 액세스 유형(전체, 쓰기, 등록, 보고, 없음)을 식별합니다.</p></td>
   <td>
    <p>전체</p>
    <p>없음</p>
    <p>쓰기 | 보고</p>
...
CSV에 의해 생성된 사용자 정의 역할은 CSV 파일 자체를 통해 전적으로 관리될 수 있습니다. 여기에는 역할의 추가, 수정 및 삭제가 포함됩니다.
...
관리자는 증분 사용자를 위해 사용자 정의 역할을 보다 효율적으로 할당할 수 있습니다. 매번 전체 데이터 세트를 다시 업로드할 필요 없이 사용자, 역할 및 사용자 역할 데이터를 업로드할 수 있습니다.

업로드된 각 사용자 가져오기 파일에 대해 FTP에서 다음 구조를 사용하여 별도의 폴더를 만듭니다.
...
* 사용자 가져오기 파일: user1.csv
* 역할 파일: user1_role.csv
* 사용자 역할 매핑 파일: user1_user_role.csv

여기서 [샘플 CSV](/help/migrated/assets/sample-csv-Incremnetal.zip)를 다운로드합니다.

각 사용자 가져오기 파일은 해당 역할 및 사용자-역할 매핑 파일과 직접 연결되어 적절한 증분 처리를 보장합니다.
